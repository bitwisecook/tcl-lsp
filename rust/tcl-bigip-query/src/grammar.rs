...
mod tests {
    ...
    fn grammar_is_non_empty_with_expected_header() {
        ...
        assert!(!g.is_empty(), "grammar reference must not be empty");
        assert!(
            g.starts_with("F5 QUERY DSL — GRAMMAR"),
            "grammar must start with the expected header, got: {:?}",
            &g[..g.len().min(40)]
        );
    }
}
