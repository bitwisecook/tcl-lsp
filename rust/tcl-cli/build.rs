...
/// (`tcl_explorer_wasm.js` + `tcl_explorer_wasm_bg.wasm`), `mermaid.min.js`,
/// and the shared Monaco/LSP bundle, which `make explorer-build` assembles in
/// `gui/`. This walks `gui/` and writes
...
fn embed_gui(manifest: &Path) {
    ...
    let editor_assets = [
        ...
        "editor/assets/monaco-host.css",
        "editor/assets/tcl.tmLanguage.json",
        "editor/assets/onig.wasm",
        "editor/lsp/worker.js",
        "editor/lsp/tcl_lsp_server_wasm.js",
        "editor/lsp/tcl_lsp_server_wasm_bg.wasm",
    ];
    let has_editor = editor_assets
        .iter()
        .all(|required| assets.iter().any(|(name, _)| name == required));
    ...
    let _ = writeln!(out, "pub const GUI_HAS_EDITOR: bool = {has_editor};");
