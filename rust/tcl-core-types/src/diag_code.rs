...
diagnostic_codes! {
    ...
    Bigip6003 => "BIGIP6003", diag(Bigip, true, "Virtual server references an iRule that is not defined in the configuration.");
    Bigip6004 => "BIGIP6004", diag(Bigip, true, "An attached iRule uses HTTP:: or SSL:: commands without the matching virtual-server profile.");
    Bigip6005 => "BIGIP6005", diag(Bigip, true, "Virtual server references a pool that is not defined in the configuration.");
    Bigip6006 => "BIGIP6006", diag(Bigip, true, "Data group is defined but not referenced by any iRule in the configuration.");
    Bigip6007 => "BIGIP6007", diag(Bigip, true, "iRule references an SNAT pool not found in the configuration.");
    Bigip6008 => "BIGIP6008", diag(Bigip, true, "Pool has no members defined.");
    Bigip6009 => "BIGIP6009", diag(Bigip, true, "Virtual server has a duplicate iRule attachment.");
    Bigip6010 => "BIGIP6010", diag(Bigip, true, "An attached iRule uses a persistence profile that is not attached to the virtual server.");
    Bigip6011 => "BIGIP6011", diag(Bigip, true, "IP-type data group contains an invalid IP address or network record.");
    Bigip6012 => "BIGIP6012", diag(Bigip, true, "Attached iRules handle the same event at the same effective priority.");
    Bigip6013 => "BIGIP6013", diag(Bigip, true, "A registry-declared BIG-IP object reference could not be resolved.");
    Bigip6014 => "BIGIP6014", diag(Bigip, true, "A BIG-IP object declaration duplicates another object of the same kind and path.");
    Bigip6038 => "BIGIP6038", diag(Bigip, true, "An iRule event requires a profile that is not active on its virtual server.");
    Bigip6039 => "BIGIP6039", diag(Bigip, true, "A virtual server attaches incompatible profile types.");
    ...
    W200 => "W200", diag(Warning, true, "Unsigned (`u`) modifier on a `binary format`/`binary scan` integer specifier requires a newer Tcl release.");
