//! Depth port for the `tcl-syntax` `expr` stack — the EVALUATION half.
//!
//! Companion to `expr_parser_port.rs` and `expr_parser_coverage_port.rs`, which
//! drive the *parser* / *AST shape* (`parse_expr` → `ExprNode`). This file
//! instead exercises the two evaluation modules that the parse-only ports do
//! not touch:
//!
