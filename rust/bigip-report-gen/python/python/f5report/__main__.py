...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.f5mku_file:
        try:
            with open(args.f5mku_file, encoding="utf-8") as handle:
                master_key = handle.read().strip()
    ...
    if args.copyright_file:
        try:
            with open(args.copyright_file, encoding="utf-8") as handle:
                copyright_notice = handle.read().strip()
        except OSError as exc:
    ...
    if args.front_matter:
        try:
            with open(args.front_matter, encoding="utf-8") as handle:
                front_matter = handle.read()
        except OSError as exc:
