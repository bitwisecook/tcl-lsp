//! Port of the Python conf-wrapped iRules pytests to Rust.
//!
//! Ports the structural (range / boolean / extracted-string) assertions of
//! two suites:
//!
//! * `tests/test_conf_wrapped_irules.py` — conf-wrapped iRules analysis
//!   (`ltm rule` / `gtm rule` mode): conf-wrap detection, embedded-rule
//!   body extraction, rule ranges, multiple rules, nested braces,
//!   gtm vs ltm.
//! * `tests/test_bigip_rule_extract.py` — BIG-IP embedded iRule extraction
//!   ranges: offset → brace-range mapping for embedded iRule navigation.
//!
//! The functions under test live in [`tcl_bigip::rule_extract`]
//! (`is_conf_wrapped_irules`, `find_embedded_rules`, `find_rule_at_offset`,
//! `replace_rule_body`, `EmbeddedRule`), the Rust port of the Python
//! `dialects.f5.bigip.rule_extract` module.
//!
//! GAPs (behaviours the Rust tree does not model — see the `// GAP:`
//! comments inline): the LSP *analysis* layer that the Python suite also
//! exercises (`analyser.conf_wrapped.analyse_conf_wrapped`, the
...
//! `find_enclosing_when_event(embedded_rules=…)`) is not ported to Rust:
...
//! are ported via `find_embedded_rules`.
...
// GAP: test_detect_dialect_returns_irules / test_dialect_directive_takes_priority
// exercise `compiler.registry.dialect.detect_dialect_from_source`, which maps a
...
// -- Analysis: rule count + names (TestAnalysis) -----------------------------
...
// -- Range / offset facts (TestRangeShifting / TestScopeTree) ----------------
...
// layer and not modelled in Rust. The *structural* offset facts those tests
...
// -- find_enclosing_when_event with embedded_rules (TestEnclosingWhenEvent) ---
// GAP: Python's `find_enclosing_when_event(src, line, embedded_rules=rules)`
// scopes the cursor's enclosing `when` event to the rule body it falls in.
// Rust's `tcl_lsp_core::irules_context::find_enclosing_when_event(src, line,
// dialect)` deliberately does NOT take `embedded_rules` (documented: the
// conf-wrapped mode is not modelled). The structural precondition those tests
...
fn no_rules_returns_empty() {
    ...
    assert!(!is_conf_wrapped_irules(src));
    // GAP: the Python test also asserts `len(result.diagnostics) == 0`; the
