//! Port of the Python pytest `tests/test_bigip_link_extract.py`
//! ("BIG-IP linked object graph extraction").
//!
//! The Python suite exercises `dialects.f5.bigip.link_extract.
//! extract_linked_bigip_objects` — a BFS wrapper that, given one or more cursor
//! offsets, resolves the *root* object containing each offset and walks the
//! linked-object graph **bidirectionally** (incoming + outgoing reference
//! edges) up to `max_depth`, annotating each reached node with its BFS `depth`
//! and a `sourceOrigin` tag derived from the config URI's basename.
//!
//! The Rust crate exposes the underlying node/edge graph
...
//! are made directly against the graph, matching the Python `result["nodes"]`
//! / `result["edges"]` shape.
//!
//! Behaviours ported, one `#[test]` per Python test:
//!  * bidirectional BFS reaches incoming (vs→pool) and outgoing (pool↑) links,
//!    with monotone depth (`..._traverses_incoming_and_outgoing_links`);
//!  * cross-`bigip.conf`-file reference resolution (`..._resolves_across_..`);
//!  * `gtm rule`→`gtm pool` stays in the gtm domain, no `ltm_pool` edge
//!    (`..._keeps_gtm_rule_pool_links_in_gtm_domain`);
//!  * multi-cursor seeds merge into one graph with multiple roots
//!    (`..._multi_cursor_merges_graphs`);
//!  * two cursors in one object dedupe to a single root
//!    (`..._multi_cursor_deduplicates_same_object`);
//!  * multi-cursor across files (`..._multi_cursor_across_files`);
//!  * `sourceOrigin` base/synced/script tags (`..._source_origin_annotation`);
//!  * firewall rule-list → address-list ref is graph-visible via the registry
//!    pilot path (`..._finds_firewall_rule_address_list_via_registry`).
...
/// One reached node, mirroring an entry of the Python `result["nodes"]`.
struct ReachedNode<'a> {
    node: &'a ObjectNode,
    depth: usize,
}

/// A seed root, mirroring an entry of the Python `result["roots"]`.
#[derive(Clone)]
struct Root {
    id: String,
    uri: String,
    /// Mirrors Python `roots[].header` / `rootHeader`; carried for result-shape
    /// fidelity though no ported assertion reads it.
    #[allow(dead_code)]
    header: String,
}
...
struct LinkResult<'a> {
    ...
    roots: Vec<Root>,
    /// Reached nodes, sorted by `(depth, id)` as Python does.
...
/// Faithful port of `extract_linked_bigip_objects`: build the graph, resolve a
/// deduplicated root per seed, run a bidirectional BFS bounded by `max_depth`,
/// and return the reached nodes/edges. Returns `None` exactly when Python does
/// (no seeds, an unknown seed uri, or no resolvable root).
fn extract_linked<'a>(
    graph: &'a ObjectGraph,
    seeds: &[(&str, usize)],
    max_depth: usize,
    ...
    // Every seed uri must be present in the built graph (Python checks both
    // `sources` and `configs`; here a uri only appears if it had a config).
    for (uri, _) in seeds {
    ...
    // Reached nodes, sorted by (depth, id) like the Python output.
...
/// default partition, as the Python `parse_bigip_conf` does).
...
/// Byte offset of `needle` within `hay`, plus `add` (mirrors Python's
