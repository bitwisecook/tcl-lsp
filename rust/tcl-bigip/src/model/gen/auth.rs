// @generated by scripts/codegen/gen_bigip_model_rust.py — do not edit.
//! Generated BIG-IP `auth` model structs (port of `dialects/f5/bigip/model/_auth.py`).

#![allow(clippy::struct_excessive_bools)]
#![allow(clippy::derivable_impls)]
#![allow(clippy::default_trait_access)]
#![allow(unused_imports)]

use super::*;

/// Mirrors Python `BigipAuthApmAuth`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthApmAuth {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `profile`
    pub profile: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthApmAuth {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            profile: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthCertLdap`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthCertLdap {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `bind_dn`
    pub bind_dn: String,
    /// `bind_pw`
    pub bind_pw: String,
    /// `bind_timeout`
    pub bind_timeout: String,
    /// `idle_timeout`
    pub idle_timeout: String,
    /// `login_attribute`
    pub login_attribute: String,
    /// `port`
    pub port: String,
    /// `scope`
    pub scope: String,
    /// `search_base_dn`
    pub search_base_dn: String,
    /// `search_timeout`
    pub search_timeout: String,
    /// `servers`
    pub servers: Vec<String>,
    /// `ssl`
    pub ssl: String,
    /// `user_template`
    pub user_template: String,
    /// `version`
    pub version: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthCertLdap {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            bind_dn: String::new(),
            bind_pw: String::new(),
            bind_timeout: String::new(),
            idle_timeout: String::new(),
            login_attribute: String::new(),
            port: String::new(),
            scope: String::new(),
            search_base_dn: String::new(),
            search_timeout: String::new(),
            servers: Vec::new(),
            ssl: String::new(),
            user_template: String::new(),
            version: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthLdap`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthLdap {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `bind_dn`
    pub bind_dn: String,
    /// `bind_pw`
    pub bind_pw: String,
    /// `bind_timeout`
    pub bind_timeout: String,
    /// `check_host_attr`
    pub check_host_attr: String,
    /// `check_roles_group`
    pub check_roles_group: String,
    /// `filter_`
    pub filter_: String,
    /// `group_dn`
    pub group_dn: String,
    /// `group_member_attribute`
    pub group_member_attribute: String,
    /// `idle_timeout`
    pub idle_timeout: String,
    /// `ignore_auth_info_unavail`
    pub ignore_auth_info_unavail: String,
    /// `ignore_unknown_user`
    pub ignore_unknown_user: String,
    /// `login_attribute`
    pub login_attribute: String,
    /// `port`
    pub port: String,
    /// `scope`
    pub scope: String,
    /// `search_base_dn`
    pub search_base_dn: String,
    /// `search_timeout`
    pub search_timeout: String,
    /// `servers`
    pub servers: Vec<String>,
    /// `ssl`
    pub ssl: String,
    /// `ssl_ca_cert`
    pub ssl_ca_cert: String,
    /// `ssl_check_peer`
    pub ssl_check_peer: String,
    /// `ssl_client_cert`
    pub ssl_client_cert: String,
    /// `ssl_client_key`
    pub ssl_client_key: String,
    /// `user_template`
    pub user_template: String,
    /// `version`
    pub version: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthLdap {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            bind_dn: String::new(),
            bind_pw: String::new(),
            bind_timeout: String::new(),
            check_host_attr: String::new(),
            check_roles_group: String::new(),
            filter_: String::new(),
            group_dn: String::new(),
            group_member_attribute: String::new(),
            idle_timeout: String::new(),
            ignore_auth_info_unavail: String::new(),
            ignore_unknown_user: String::new(),
            login_attribute: String::new(),
            port: String::new(),
            scope: String::new(),
            search_base_dn: String::new(),
            search_timeout: String::new(),
            servers: Vec::new(),
            ssl: String::new(),
            ssl_ca_cert: String::new(),
            ssl_check_peer: String::new(),
            ssl_client_cert: String::new(),
            ssl_client_key: String::new(),
            user_template: String::new(),
            version: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthLoginFailures`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthLoginFailures {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthLoginFailures {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthPartition`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthPartition {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `description`
    pub description: String,
    /// `default_route_domain`
    pub default_route_domain: String,
    /// `inherited_traffic_group`
    pub inherited_traffic_group: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthPartition {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            description: String::new(),
            default_route_domain: String::new(),
            inherited_traffic_group: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthPassword`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthPassword {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `expiration_warning`
    pub expiration_warning: String,
    /// `minimum_length`
    pub minimum_length: String,
    /// `policy`
    pub policy: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthPassword {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            expiration_warning: String::new(),
            minimum_length: String::new(),
            policy: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthPasswordPolicy`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthPasswordPolicy {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `expiration_warning`
    pub expiration_warning: String,
    /// `max_duration`
    pub max_duration: String,
    /// `max_login_failures`
    pub max_login_failures: String,
    /// `min_duration`
    pub min_duration: String,
    /// `minimum_length`
    pub minimum_length: String,
    /// `minimum_regular_characters`
    pub minimum_regular_characters: String,
    /// `password_memory`
    pub password_memory: String,
    /// `policy_enforcement`
    pub policy_enforcement: String,
    /// `required_lowercase`
    pub required_lowercase: String,
    /// `required_numeric`
    pub required_numeric: String,
    /// `required_special`
    pub required_special: String,
    /// `required_uppercase`
    pub required_uppercase: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthPasswordPolicy {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            expiration_warning: String::new(),
            max_duration: String::new(),
            max_login_failures: String::new(),
            min_duration: String::new(),
            minimum_length: String::new(),
            minimum_regular_characters: String::new(),
            password_memory: String::new(),
            policy_enforcement: String::new(),
            required_lowercase: String::new(),
            required_numeric: String::new(),
            required_special: String::new(),
            required_uppercase: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthRadius`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthRadius {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `service_type`
    pub service_type: String,
    /// `servers`
    pub servers: Vec<String>,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthRadius {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            service_type: String::new(),
            servers: Vec::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthRadiusServer`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthRadiusServer {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `server`
    pub server: String,
    /// `port`
    pub port: String,
    /// `secret`
    pub secret: String,
    /// `timeout`
    pub timeout: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthRadiusServer {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            server: String::new(),
            port: String::new(),
            secret: String::new(),
            timeout: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthRemoteRole`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthRemoteRole {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `role_info`
    pub role_info: Vec<String>,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthRemoteRole {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            role_info: Vec::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthRemoteUser`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthRemoteUser {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `default_partition`
    pub default_partition: String,
    /// `default_role`
    pub default_role: String,
    /// `remote_console_access`
    pub remote_console_access: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthRemoteUser {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            default_partition: String::new(),
            default_role: String::new(),
            remote_console_access: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthSource`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthSource {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `fallback`
    pub fallback: String,
    /// `type_`
    pub type_: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthSource {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            fallback: String::new(),
            type_: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthTacacs`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthTacacs {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `protocol`
    pub protocol: String,
    /// `secret`
    pub secret: String,
    /// `service`
    pub service: String,
    /// `servers`
    pub servers: Vec<String>,
    /// `accounting`
    pub accounting: String,
    /// `authentication`
    pub authentication: String,
    /// `debug`
    pub debug: String,
    /// `encryption`
    pub encryption: String,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthTacacs {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            protocol: String::new(),
            secret: String::new(),
            service: String::new(),
            servers: Vec::new(),
            accounting: String::new(),
            authentication: String::new(),
            debug: String::new(),
            encryption: String::new(),
            range: None,
        }
    }
}

/// Mirrors Python `BigipAuthUser`.
#[derive(Debug, Clone, PartialEq)]
pub struct BigipAuthUser {
    /// `name`
    pub name: String,
    /// `full_path`
    pub full_path: String,
    /// `description`
    pub description: String,
    /// `partition`
    pub partition: String,
    /// `shell`
    pub shell: String,
    /// `encrypted_password`
    pub encrypted_password: String,
    /// `partition_access`
    pub partition_access: Vec<String>,
    /// `range`
    pub range: Option<crate::range::Range>,
}

impl Default for BigipAuthUser {
    fn default() -> Self {
        Self {
            name: String::new(),
            full_path: String::new(),
            description: String::new(),
            partition: String::new(),
            shell: String::new(),
            encrypted_password: String::new(),
            partition_access: Vec::new(),
            range: None,
        }
    }
}
