...
fn collating_element(chars: &[Chr]) -> Option<Chr> {
    ...
    // A small set of named collating elements used by the test suite.
    let name: String = chars.iter().filter_map(|&c| char::from_u32(c)).collect();
    match name.as_str() {
        "zero" => Some(u32::from(b'0')),
        "one" => Some(u32::from(b'1')),
        "two" => Some(u32::from(b'2')),
        "three" => Some(u32::from(b'3')),
        "four" => Some(u32::from(b'4')),
        "five" => Some(u32::from(b'5')),
        "six" => Some(u32::from(b'6')),
        "seven" => Some(u32::from(b'7')),
        "eight" => Some(u32::from(b'8')),
        "nine" => Some(u32::from(b'9')),
        _ => None,
    }
}
