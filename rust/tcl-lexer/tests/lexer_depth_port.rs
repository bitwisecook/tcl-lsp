//! Depth coverage for the `tcl-lexer` public surface the existing port
...
//!
//! Read alongside `parse_lexer_port.rs`, `parse_expr_port.rs`,
...
// Shared helpers — mirror the sibling port files' `lex` / `kinds` shape.
...
fn backslash_newline_continues_a_comment_swallowing_the_next_line() {
    // tclsh-proven (per parse_lexer_port.rs): a `\` at end of a comment
