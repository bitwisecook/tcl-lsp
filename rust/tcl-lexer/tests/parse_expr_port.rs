//! Port of `tests/test_tcl_parse_expr.py` — itself recreated from Tcl's
//! official `tests/parseExpr.test`. Verifies the Rust `tokenise_expr` lexer
//! classifies expression lexemes the same way the Python `ExprLexer` did.
//!
...
//! `sin(0)`=0.0). The lexer is a tokeniser (not an evaluator), so — as the
