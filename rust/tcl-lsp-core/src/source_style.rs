...
///
/// `decode` is the byte-level decoder's report when the caller read the file
/// itself, and `None` when the text arrived already decoded from an editor —
/// see [`crate::source_decode`] for what that changes (precision, never the
/// verdict).
#[must_use]
