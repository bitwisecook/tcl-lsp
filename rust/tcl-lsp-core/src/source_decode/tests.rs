...
#[test]
fn w107_respects_proof_that_a_genuine_replacement_char_is_content() {
    ...
    // ...whereas the text-only path (an editor `didOpen`, where the bytes were
    // consumed by the editor's own decoder) cannot tell it from a substitution
    // and says so. This is the module's one documented false positive.
    let diags = encoding_integrity_diagnostics(src, None);
    assert_eq!(codes(&diags), ["W107"]);
    assert!(diags[0].message.contains("most likely"));
}

#[test]
fn both_paths_agree_on_which_codes_fire_for_a_broken_file() {
    // The consistency contract: an editor's lossy decode and ours produce the
    // same text, so the same codes fire — the byte path is just more precise.
    ...
    assert_eq!(codes(&byte_diags), codes(&text_diags));
    assert_eq!(codes(&byte_diags), ["W107"]);
    // Precision, not verdict: only the byte path can name offset and class.
    assert!(byte_diags[0].message.contains("overlong encoding"));
    assert!(!text_diags[0].message.contains("overlong encoding"));
}
...
#[test]
fn w109_survives_the_text_only_path_for_lossily_decoded_utf16() {
    // An editor that did *not* recognise the UTF-16 hands us NUL-interleaved
    // text; the density heuristic still catches it with no bytes at all.
    let (text, _) = decode_source(&utf16le(BODY));
    let diags = encoding_integrity_diagnostics(&text, None);
    assert_eq!(codes(&diags), ["W109"]);
