...
//!
//! An editor hands the server text over the wire; the bytes are long gone by
//! then, and VS Code has done its own lossy decode.  So the *reporting* is
//! driven off the decoded text ([`encoding_integrity_diagnostics`]) and both
//! paths raise **the same codes on the same file**.  The byte path additionally
//! passes its [`DecodeReport`], which upgrades the message from a heuristic to
//! a fact: the exact byte offset and the exact malformation class.
//!
//! That is the deliberate difference, and it is one-directional — more bytes
//! means more precision, never a different verdict.  The one place it changes
//! a verdict is the honest one: with the bytes in hand the decoder can *prove*
//! the file was valid UTF-8, so a genuine `U+FFFD` in the source is correctly
//! left alone; without them, a `U+FFFD` in the text is treated as evidence of
//! a failed decode, because from the text alone the two are identical.
//!
//! # Codes
//!
...
//! * **A genuine `U+FFFD` typed by the author**, on the text-only path.  See
//!   above — this is a stated false positive, and the byte path does not have
//!   it.
...
//!   is left to W108.  Only a NUL *density* consistent with UTF-16 trips W109.
...
/// At least this many NUL bytes, and at least this share of the buffer, before
/// [`NonTextEncoding::NulInterleaved`] is claimed.
///
/// Mostly-ASCII UTF-16 is ~50% NUL, so the bar can sit far above anything real
/// text reaches and still catch every practical case. The absolute floor stops
/// a two-byte file from qualifying on ratio alone.
const NUL_RATIO_THRESHOLD: f64 = 0.25;
/// Minimum NUL count for the [`NUL_RATIO_THRESHOLD`] test to apply at all.
...
fn detect_non_text(bytes: &[u8]) -> Option<NonTextEncoding> {
    ...
    #[expect(
        ...
        reason = "a ratio of two counts; f64 has 53 bits of mantissa, so the \
                  comparison is exact for any file that fits in memory"
    )]
    let ratio = nuls as f64 / bytes.len().max(1) as f64;
    (nuls >= NUL_COUNT_FLOOR && ratio >= NUL_RATIO_THRESHOLD)
...
/// ([`decode_source`]), or `None` when the text arrived already decoded (an
/// editor `didOpen`). Passing it upgrades a heuristic into a fact and
/// suppresses the one false positive the text-only path has — see the module
/// docs.
///
...
pub fn encoding_integrity_diagnostics(
    ...
    // From bytes, the BOM / NUL-density verdict. From text alone, the residue
    // a lossy UTF-16 decode leaves behind: NULs at a density no real source
    // has. Both anchor at (0,0) — the finding is about the file.
    let non_text = report.and_then(|r| r.non_text).or_else(|| {
        let nuls = text.chars().filter(|&c| c == '\0').count();
        #[expect(
            clippy::cast_precision_loss,
            reason = "a ratio of two counts — see NUL_RATIO_THRESHOLD"
        )]
        let ratio = nuls as f64 / text.chars().count().max(1) as f64;
        (nuls >= NUL_COUNT_FLOOR && ratio >= NUL_RATIO_THRESHOLD)
            .then_some(NonTextEncoding::NulInterleaved)
    });
    ...
    match report {
        ...
        // Text-only path: U+FFFD is the only evidence available.
        None => {
            ...
            if count > 0 {
                ...
                out.push(StyleDiagnostic {
                    range: first_replacement_range(text),
                    message: format!(
                        "Source contains {count} Unicode replacement character{plural} (U+FFFD) \
                         — the file was most likely not valid UTF-8 and these bytes were \
                         substituted before analysis, so the analysed text is not the file on \
                         disk. Tcl 9 refuses to read such a file at all."
                    ),
                    severity: StyleSeverity::Warning,
                    code: "W107",
                    fix: None,
