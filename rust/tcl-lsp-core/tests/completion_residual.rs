...
//! Residual coverage for `tcl-lsp-core/src/completion.rs` — the
...
fn smoke_completion_binary_exists() {
    // Anchors the test binary so `cargo llvm-cov --test completion_residual`
    // has a target to instrument.
    let src = "pu\n";
...
// Array-element completion — `$arr(` / `$arr(prefix`.
...
// Switch / option completion — the text-edit range and documentation, with
// the `start == 0` guard exercised by the dash-at-col-0 path.
...
// `HTTP::respond <status> content|noserver|…`.
...
// subcommand (not command-level) arg-value path.
...
// proc completion under the iRules dialect.
...
// `workspace_proc_detail` ("1 param" vs "N params").
...
// `is_bare_var_name` edges via observable completion behaviour — the empty-name
// guards and the brace-vs-bare choice for qualified names.
...
// segmentation that only runs for `f5-irules`.
...
// and defaults (the proc fallback).
