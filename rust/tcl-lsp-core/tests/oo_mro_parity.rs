...
fn trio_at(source: &str, (line, character): (u32, u32)) -> Trio {
    ...
    Trio {
        ...
        references: references(source, tcl_dialect::DialectProfile::by_name(D), line, character, &analysis, true),
    }
}
...
fn outline_members(source: &str) -> Vec<(String, String)> {
    ...
    walk(
        "",
        &tcl_lsp_core::document_symbols::document_symbols(source, tcl_dialect::DialectProfile::by_name(D)),
        &mut out,
    );
    out
}
