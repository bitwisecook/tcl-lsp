...
//! `taint_port.rs` (291 tests) leaves thin: the full sink-family matrix
...
//!
//! This file deliberately reuses the `taint_port.rs` harness *shape* —
...
//! position-aware filter that `taint_port.rs` does not already pin (verified by
...
// taint_port.rs (PATH_BOUNDED, HEADER_TOKEN_SAFE survival, the
...
// single taint_port.rs example (stdout/stderr channel literals, both tainted).
...
// `option_scan_region` / `arg_can_be_option` branches beyond taint_port.rs.
...
// via the four quote spellings. taint_port.rs covers `regexp`; this adds
...
// already-stamped colour. taint_port.rs covers URI::encode; this adds the
...
// taint_port.rs's IP/PORT/SNI/URI/HTML set. f5-dialect, IR-gated.
// ===========================================================================
mod irule3003_depth {
    use super::*;
...
// tainted target. taint_port.rs has an irule3004 module; this adds the
...
// taint_port.rs's literal+PATH_PREFIXED cases. f5-dialect, IR-gated.
// ===========================================================================
mod irule3101_depth {
    use super::*;
...
// normalise+`string match` guard suppression. taint_port.rs does not cover W313.
//
...
// result clears every sink. taint_port.rs covers `string length` / `llength`;
...
mod sanitiser_breadth {
    ...
    fn sanitiser_inside_command_sub_clears_argument() {
        ...
        //
        // The `expr` sink already gets this right (taint_port.rs proves
...
// source taints regardless of arguments. taint_port.rs covers the basic
...
mod interproc_depth {
    ...
    fn passthrough_conservatively_loses_source_option_safety_colour() {
        ...
        // T102-clean — taint_port.rs `path_prefixed_copy_suppresses`), but the
