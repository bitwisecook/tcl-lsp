    ...
    let mut entries: Vec<(String, String)> = fs::read_dir(dir)
        ...
        .map(|e| {
            let name = e
                ...
                .unwrap_or("<unknown>")
                .to_string();
            let source = fs::read_to_string(e.path()).unwrap_or_else(|err| {
                panic!("read {}: {err}", e.path().display())
            });
            (name, source)
        })
        .collect();
...
fn divergent_corpus_reports_progress() {
    ...
    if !closed.is_empty() {
        eprintln!(
            "[differential_codegen] these fixtures now match — promote them to matching/:"
        );
        for name in &closed {
            eprintln!("    {name}");
        }
