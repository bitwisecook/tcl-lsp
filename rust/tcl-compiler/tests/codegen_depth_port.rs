...
//! existing `tests/codegen_port.rs` leaves thin:
//!
...
//! ## Method
//!
//! Unlike `codegen_port.rs` (which calls the free `codegen_module` /
...
//!
//! Does NOT duplicate `codegen_port.rs`: that suite covers the main emitter
...
/// `backend.rs` entry point, distinct from `codegen_port.rs`'s free-function
/// `codegen_module`.
