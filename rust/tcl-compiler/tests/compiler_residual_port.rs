...
//! `tests/codegen_port.rs`, `tests/codegen_depth_port.rs`,
...
/// `codegen_depth_port.rs` uses for `try_list_expand_concat` &c.).
...
//
// Disjoint from var_escape_cfg_port.rs (which covers conditional upvar, loop /
