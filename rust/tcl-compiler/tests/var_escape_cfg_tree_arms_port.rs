...
//!
//! Companion to `var_escape_cfg_catch_port.rs`. That file reaches the
...
use tcl_compiler::var_escape::{analyse_cfg_function, EscapeTag};
...
fn catch_literal_return_walks_return_arm() {
    ...
    assert!(!r.dynamic_barrier(), "literal catch'd return is not pessimistic");
}
...
fn catch_while_return_walks_structural_then_return() {
    ...
    assert!(!r.dynamic_barrier(), "catch'd while+return is not pessimistic");
}
...
fn catch_foreach_return_walks_structural_then_return() {
    ...
    assert!(!r.dynamic_barrier(), "catch'd foreach+return is not pessimistic");
}
...
fn catch_switch_expr_eval_walks_expr_arm() {
    ...
    assert!(!r.dynamic_barrier(), "catch'd switch+expr is not pessimistic");
}
...
fn catch_for_body_return_walks_for_arm() {
    ...
    assert!(is_frame(&r, "i"), "for-init write escapes: {:?}", r.name_tags);
    assert!(!r.dynamic_barrier(), "catch'd for+return is not pessimistic");
}
...
fn catch_nested_if_global_then_return() {
    ...
    assert!(is_frame(&r, "deep"), "deeply nested global escapes: {:?}", r.name_tags);
}
