...
//!
//! Two prior depth ports (`var_escape_cfg_port.rs`, `var_escape_typeinfer_port.rs`)
//! and the residual port (`compiler_residual_port.rs`) already cover the *common*
...
// (`compiler_residual_port.rs` covers a simple literal catch upvar; the dynamic
...
// UpFrame) with a frame-crossing command inside — arms `compiler_residual_port.rs`
// (which only does the IR `Return`/`ExprEval`/`namespace upvar` scans) and
// `var_escape_cfg_port.rs` (which does the IR `walk` if/for/switch/try, NOT the
...
// /`[info vars]` shape is found. `compiler_residual_port.rs` covers the head-loop
...
// (`var_escape_cfg_port.rs` drives cfg_result only on flat statements, not eval
// bodies.)
// ════════════════════════════════════════════════════════════════════════════
...
// run `dynamic_name_escape` (resolve-literal-or-spill). `var_escape_cfg_port.rs`
...
// `compiler_residual_port.rs` covers user/qualified callees + the `$cmd` head;
