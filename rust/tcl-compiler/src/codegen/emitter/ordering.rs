...
pub fn reorder_bottom_tested(cfg: &CfgFunction, order: Vec<String>) -> Vec<String> {
    ...
    for (_back_src, header) in back_edges {
        ...
        collect_loop_body(cfg, &body_start, &header, &mut loop_blocks, Some(&exit_block));
...
mod tests {
    ...
    fn linearise_single_block() {
        ...
        cfg.blocks.get_mut("entry_0").unwrap().terminator =
    ...
    fn linearise_diamond() {
        ...
        cfg.blocks.get_mut("join_1").unwrap().terminator =
            Some(Terminator::Return {
                value: None,
                span: None,
                expr: None,
