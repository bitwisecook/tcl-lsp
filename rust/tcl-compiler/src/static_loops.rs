...
pub fn simple_var_ref(text: &str) -> Option<String> {
    let stripped = text.trim();
    let name = if let Some(inner) = stripped.strip_prefix("${").and_then(|s| s.strip_suffix('}')) {
...
fn exec_statement(stmt: &Statement, env: &mut StaticEnv) -> bool {
    match stmt {
        ...
        Statement::AssignExpr { name, expr, .. } => {
            match evaluate_expr_with_constants(expr, env) {
                Some(v) => {
...
pub fn summarise_for_statement(
    ...
    summarise_static_for(init, condition, next, body, initial_constants, max_iterations)
}
...
mod tests {
    ...
    fn summarise_forwards_for_statement_helper() {
        ...
        let env =
            summarise_for_statement(&for_stmt, &StaticEnv::new(), 100).expect("summarised");
