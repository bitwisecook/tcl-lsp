...
pub fn run(ctx: &mut PassContext<'_>, cu: &CompilationUnit) {
    ...
    let mut unused: Vec<&String> = user_procs.iter().filter(|q| !reachable.contains(*q)).collect();
    unused.sort();
...
fn comment_out(text: &str, proc_name: &str) -> String {
    let mut out = format!(
        "# [O124] Unused proc — '{proc_name}' is not called from any event\n",
    );
...
mod tests {
    ...
    fn run_pass(source: &str, dialect: Option<&str>, ip: InterproceduralAnalysis) -> Vec<Optimisation> {
    ...
    fn library_irule_produces_nothing() {
        ...
        let ip = ip_with(&[
            ("::helper", &[], false),
            ("::when::RULE_INIT", &[], false),
        ]);
        let opts = run_pass(source, Some("f5-irules"), ip);
        assert!(opts.is_empty(), "library iRule should not emit O124: {opts:?}");
    }
    ...
    fn reachable_proc_not_flagged() {
        ...
        let opts = run_pass(source, Some("f5-irules"), ip);
        assert!(opts.is_empty(), "reachable proc should not be flagged: {opts:?}");
    }
