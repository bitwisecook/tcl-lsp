...
fn extract_method_def(
    ...
    extract_method_def_in(
        ...
        visibility,
        synthetic_name,
        // dialect-drift-ok: the dialect-less test/utility spelling — no
        // document is in hand, so the C Tcl rules stand in, exactly as the
        // `None` surface query beside them does.
        MemberDialect {
            surface: None,
