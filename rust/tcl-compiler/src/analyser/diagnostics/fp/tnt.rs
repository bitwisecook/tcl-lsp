...
fn fp_tnt_01_direct_operand_still_fires() {
    ...
    let src = "proc f {} {\n    set data [gets stdin]\n    expr {$data + 1}\n}";
    assert!(fires(src, D, "T100"), "FP-TNT-01 TP: direct operand must fire T100; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_01_function_arg_direct_operand_still_fires() {
    ...
    let src = "proc f {} {\n    set data [gets stdin]\n    expr {abs($data)}\n}";
    assert!(fires(src, D, "T100"), "FP-TNT-01 TP: abs($data) must fire T100; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_02_content_arg_still_fires() {
    ...
    let src = "proc f {} {\n    set data [gets stdin]\n    puts $data\n}";
    assert!(fires(src, D, "T101"), "FP-TNT-02 TP: tainted puts content must fire T101; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_02_content_with_channel_still_fires() {
    ...
    let src = "proc f {} {\n    set chan stdout\n    set data [gets stdin]\n    puts $chan $data\n}";
    assert!(fires(src, D, "T101"), "FP-TNT-02 TP: content arg with channel must fire T101; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_03_eval_list_tainted_head_fires_t100() {
    ...
    assert!(fires(src, D, "T100"), "FP-TNT-03 TP: tainted [list ...] head must fire T100; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_03_eval_list_literal_head_no_t100() {
    ...
    let src = "set raw [gets stdin]\neval [list puts $raw]\n";
    assert!(!fires(src, D, "T100"), "FP-TNT-03 TN: [list <known-cmd> $raw] must suppress T100; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_03_uplevel_list_tainted_head_fires_t100() {
    ...
    let src = "set raw [gets stdin]\nuplevel [list $raw]\n";
    assert!(fires(src, D, "T100"), "FP-TNT-03 TP: uplevel [list $raw] must fire T100; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_03_eval_list_via_var_still_fires() {
    ...
    let src = "set raw [gets stdin]\nset lst [list $raw]\neval $lst\n";
    assert!(fires(src, D, "T100"), "FP-TNT-03 TP: propagated [list]-wrapped taint must fire T100; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_03_interp_eval_list_tainted_head_fires_t105() {
    ...
    assert!(fires(src, D, "T105"), "FP-TNT-03 TP: interp eval [list $raw] must fire T105; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_03_interp_eval_list_literal_head_no_t105() {
    ...
    assert!(!fires(src, D, "T105"), "FP-TNT-03 TN: [list puts $raw] interp eval must suppress T105; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_04_late_terminator_does_not_protect_tainted_var() {
    ...
    assert!(fires(src, D, "T102"), "FP-TNT-04 TP: late `--` must not protect earlier candidate; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_04_early_terminator_protects_tainted_var() {
    ...
    let src = "set pat [gets stdin]\nregexp -- $pat subject\n";
    assert!(!fires(src, D, "T102"), "FP-TNT-04 TN: early `--` must protect later candidate; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_04_no_terminator_fires() {
    ...
    let src = "set pat [gets stdin]\nregexp $pat subject\n";
    assert!(fires(src, D, "T102"), "FP-TNT-04 TP: tainted pattern with no `--` must fire T102; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_05_tainted_url_fires_t104() {
    ...
    assert!(fires(src, D, "T104"), "FP-TNT-05 TP: tainted URL must fire T104; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_05_tainted_header_value_no_t104() {
    ...
    assert!(!fires(src, D, "T104"), "FP-TNT-05 TN: tainted header value must NOT fire T104; emitted {:?}", codes(src, D));
}
...
fn fp_tnt_05_socket_tainted_host_and_port_fire() {
    ...
    assert!(fires(src_host, D, "T104"), "FP-TNT-05 TP: tainted host must fire T104; emitted {:?}", codes(src_host, D));
    assert!(fires(src_port, D, "T104"), "FP-TNT-05 TP: tainted port must fire T104; emitted {:?}", codes(src_port, D));
}
