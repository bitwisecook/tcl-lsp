...
/// For one candidate proc not yet a known upvar proc, walk its body for a
/// call to an *already-known* upvar proc where the callee's own upvar-
/// source parameter is fed, at that call site, by a bare `$paramName`
/// reference naming one of the candidate's own parameters unchanged —
/// issue #923 idx 18 (tcllib's `page::util::flow` wrapper idiom: `proc
/// ::page::util::flow {start fvar nvar script} { ... [list
/// ::page::util::flow::iter ... $fvar $nvar $script] }`, where the real
/// upvar/uplevel work happens one proc away, inside the snit type's own
/// constructor). Returns a synthesised [`UpvarInfo`] for the candidate
/// (a `param_targets` map — `caller_side_defs` only reads the *values*,
/// so the keys are placeholders) when at least one such pass-through is
/// found, `None` otherwise.
#[must_use]
pub fn transitive_upvar_info_for<S: BuildHasher>(
    params: &[String],
    body: &Script,
    known: &HashMap<String, UpvarInfo, S>,
    known_params: &HashMap<String, Vec<String>, S>,
) -> Option<UpvarInfo> {
    let mut info = UpvarInfo::default();
    walk_script_for_transitive_calls(&body.statements, params, known, known_params, &mut info);
    (!info.is_empty()).then_some(info)
}

fn walk_script_for_transitive_calls<S: BuildHasher>(
    stmts: &[Statement],
    params: &[String],
    known: &HashMap<String, UpvarInfo, S>,
    known_params: &HashMap<String, Vec<String>, S>,
    ...
    for stmt in stmts {
        walk_stmt_for_transitive_calls(stmt, params, known, known_params, info);
    }
}

fn walk_stmt_for_transitive_calls<S: BuildHasher>(
    stmt: &Statement,
    params: &[String],
    known: &HashMap<String, UpvarInfo, S>,
    known_params: &HashMap<String, Vec<String>, S>,
    ...
    match stmt {
        Statement::Call { command, args, .. } => {
            if let Some(callee_info) = known.get(command.as_str()) {
                let target_params: &[String] = known_params
                    .get(command.as_str())
                    .map_or(&[][..], Vec::as_slice);
                record_transitive_pass_through(args, params, target_params, callee_info, info);
            }
        }
        Statement::If {
            clauses, else_body, ..
            ...
            for c in clauses {
                walk_script_for_transitive_calls(
                    &c.body.statements,
                    params,
                    known,
                    known_params,
            ...
            if let Some(e) = else_body {
                walk_script_for_transitive_calls(&e.statements, params, known, known_params, info);
            }
        }
        Statement::For {
            init, next, body, ..
        } => {
            walk_script_for_transitive_calls(&init.statements, params, known, known_params, info);
            walk_script_for_transitive_calls(&next.statements, params, known, known_params, info);
            walk_script_for_transitive_calls(&body.statements, params, known, known_params, info);
        }
        Statement::While { body, .. }
        | Statement::Foreach { body, .. }
        | Statement::Catch { body, .. }
        ...
        Statement::Switch {
            ...
            if let Some(b) = default_body {
                walk_script_for_transitive_calls(&b.statements, params, known, known_params, info);
            }
        }
        Statement::Try {
            body,
            ...
            if let Some(f) = finally_body {
                walk_script_for_transitive_calls(&f.statements, params, known, known_params, info);
            }
        }
        _ => {}
    }
...
/// If `args` (the call site's own words, positionally aligned with
/// `target_params`, the callee's own parameter list) feeds a plain
/// `$paramName` reference — naming one of `caller_params`, the calling
/// proc's own parameters, unchanged — into one of `callee_info`'s own
/// upvar-source positions, record the caller's parameter as an upvar
/// target too, one frame further out (issue #923 idx 18). A callee whose
/// own upvar target is unresolvable widens the caller the same way, since
/// the caller is reached only through it.
fn record_transitive_pass_through(
    args: &[String],
    caller_params: &[String],
    target_params: &[String],
    callee_info: &UpvarInfo,
    info: &mut UpvarInfo,
) {
    for callee_param in callee_info.param_targets.values() {
        let Some(idx) = target_params.iter().position(|p| p == callee_param) else {
            continue;
        };
        let Some(arg) = args.get(idx) else { continue };
        let Some(caller_param) = is_dollar_param_ref(arg) else {
            continue;
        };
        if !caller_params.iter().any(|p| p == caller_param) {
            continue;
        }
        info.param_targets.insert(
            format!("@transitive@{caller_param}"),
            caller_param.to_string(),
        );
    }
    if callee_info.has_unresolvable_caller_target {
        info.has_unresolvable_caller_target = true;
    }
}
