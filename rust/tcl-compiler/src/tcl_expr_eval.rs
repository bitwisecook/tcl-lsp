...
fn apply_irules_string_op(op: BinOp, left: &str, right: &str) -> Option<TclValue> {
    let res = match op {
        ...
        BinOp::MatchesRegex => {
            return regex_matches(right, left).map(|b| TclValue::Int(i64::from(b)))
        }
...
mod tests {
    ...
    fn irules_matches_regex_are_features_bail_to_none() {
        ...
        assert_eq!(eval_irules(r#""abc" matches_regex "(?=a)abc""#), None);
    }
    ...
    fn irules_matches_regex_invalid_pattern_is_none() {
        ...
        assert_eq!(eval_irules(r#""abc" matches_regex "[unterminated""#), None);
    }
