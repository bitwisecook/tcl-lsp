...
impl InputDocument {
    ...
    ///
    /// Shares its implementation with the LSP server's publish path, so the
    /// CLI and the editor raise the same codes on the same file; the CLI's
    /// messages are simply more precise, because it read the bytes. See
