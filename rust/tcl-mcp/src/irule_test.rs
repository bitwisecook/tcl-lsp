...
fn extract_test_paths(source: &str) -> Vec<PathInfo> {
    ...
    let data = tcl_diagram::diagram_data_for_dialect(source, registry, tcl_lsp_core::profile_for_dialect(IRULES_DIALECT));
...
fn collect_taints(source: &str) -> Vec<Taint> {
    let registry = registry_for_dialect(IRULES_DIALECT);
    let graph = tcl_lsp_core::graphs::dataflow_graph(source, registry, tcl_lsp_core::profile_for_dialect(IRULES_DIALECT));
    graph
        .get("taint_warnings")
        .and_then(Value::as_array)
        .map(|warnings| {
