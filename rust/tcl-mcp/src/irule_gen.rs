...
fn build_test_script(ctx: &ScriptContext) -> String {
    ...
    let _ = writeln!(out, "# test_{test_name}.tcl -- Generated tests for {}", ctx.basename);
    out.push_str("#\n");
...
fn build_all_test_blocks(test_name: &str, ctx: &ScriptContext) -> Vec<String> {
    ...
    for p in ctx.cfg_paths {
        let event = p.get("event").and_then(Value::as_str).unwrap_or("").to_owned();
    ...
    for event_name in &order {
        ...
        for path in &by_event[event_name] {
            ...
            blocks.push(render_test_case(&format!("{test_name}-cfg-{test_idx}.0"), &desc, &body));
        }
    }
    blocks
}
...
fn build_test_description(path: &Value) -> String {
    ...
    for c in conditions.into_iter().flatten() {
        match c.get("kind").and_then(Value::as_str) {
            ...
            Some("switch") => {
                parts.push(format!("{} = {}", cond_field(c, "subject"), cond_field(c, "pattern")));
            }
            _ => {}
        }
    }
...
fn build_assertion(cmd: &str, args: &[String]) -> Vec<String> {
    match cmd {
        "pool" if !args.is_empty() => {
            vec![format!("::orch::assert_that pool_selected equals \"{}\"", args[0])]
        }
        "reject" => vec!["::orch::assert_that decision connection reject was_called".to_owned()],
        "drop" | "discard" => {
            vec![format!("::orch::assert_that decision connection {cmd} was_called")]
        }
        "HTTP::redirect" => {
...
fn build_request_setup(event_name: &str, conditions: &[&Value]) -> Vec<String> {
    ...
    if event_name == "HTTP_REQUEST" || event_name == "HTTP_RESPONSE" || event_name.contains("HTTP")
    {
        ...
        result.push(format!("::orch::run_http_request -host \"{host}\" -uri \"{uri}\""));
...
fn build_http_test_blocks(
    ...
    blocks.push(render_test_case(&format!("{test_name}-1.0"), "basic request routing", &body));
...
fn build_multi_tmm_block(test_name: &str, ctx: &ScriptContext) -> String {
    ...
    if !ctx.objects.pools.is_empty() {
        ...
        for pool in &ctx.objects.pools {
            let _ = writeln!(out, "    ::orch::add_pool {pool} {{{{10.0.0.1:80}} {{10.0.0.2:80}}}}");
        }
        out.push_str("}\n");
    }
    ...
    if let Some(var) = check_var {
        let _ = writeln!(out, "        set val [::orch::tmm_get_static $tmm_id {var}]");
    ...
    let _ = writeln!(out, "        \"{test_name}-multi-1.0: only $active TMMs got traffic\"");
    out.push('}');
    out
}
