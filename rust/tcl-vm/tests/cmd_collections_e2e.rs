...
/// elements; with no pattern removes the whole array (iterating and emptying
/// it instead would leave an empty array present rather than deleting it —
...
///
/// The sentence and the exact-then-unique-prefix scan are
...
/// The native VM exposes `dict info` through the release-aware
...
///
/// `dict` is a `TclMakeEnsemble` command, so the whole sentence, including
/// its `must be` clause, and the
...
/// A dict with a **duplicate key** is canonicalised the way
...
///
/// Decoding the list rep straight into `chunks_exact(2)`
/// pairs and then `find`ing the *first* match would read and rewrite the
/// stale value in three VM paths: the `Op::DICT_GET` opcode, `dict_set_path` (and the
...
fn duplicate_dict_keys_canonicalise_last_value_wins() {
    ...
    // all-literal `[dict create …]` is constant-folded in codegen. Rendering
    // the fold's arguments as a plain list join would freeze the duplicate
    // into the literal even though `dict size`/`dict get` still answer
    // correctly (they re-canonicalise on the way in) — the divergence would
    // show only in the **string representation**, exactly what a `puts` or
    // `string length` sees.
    //
    // The dynamic spelling defeats the fold and always takes the correct path;
    // the two must agree.
...
/// A dict *value-parse* failure is reported as a *dict*, not a
/// list.
///
