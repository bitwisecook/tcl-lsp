//! Port of the C-Tcl-observable parts of:
//!   - `tests/test_command_registry.py`   (registry structure / switches /
//!     options / argument values / hover / subcommands / dialect scoping)
//!   - `tests/test_dialect_profiles.py`   (per-dialect signature membership)
//!   - `tests/test_dialect_helpers.py`    (the `is_irules_dialect` predicate)
//!   - `tests/test_detect_dialect.py`     (see the NOTE below)
//!
//! The pytest suites drive the **Python** `compiler.registry` package. This
//! file asserts the *equivalent behaviour* on the **Rust** `tcl_registry`
//! crate, mapping the Python registry API to its Rust counterpart:
//!
//!   Python                                     Rust
//!   ------------------------------------------ -------------------------------
...
///
/// The Python `option.hover.summary` ("server mode") is registry-internal
/// hover prose; the Rust `OptionSpec` records this as `detail`. We assert the
...
// ===========================================================================
// test_command_registry.py :: TestCommandsForEvent / TestCommandLegality /
// TestLegalityParity  (iRules event ⇄ command cross-product)
// ===========================================================================
...
/// `test_legality_matches_commands_for_event` (parity) — the O(1) legality
/// check agrees with the bulk valid-command listing for every known event.
///
/// f5-dialect.
#[test]
...
fn form_kind_variants_exist() {
    ...
    // Distinctness is the contract the Python `.value` strings encode.
...
// ===========================================================================
// test_command_registry.py :: `when` argument values cover the event corpus
// ===========================================================================

/// `test_when_event_values_cover_event_props` (adapted) — in the Python model
/// `when` arg-0 completion lists every event. In the Rust model event-name
/// completion is driven by `Traits::IS_EVENT_HANDLER` + the `EventRegistry`
/// rather than baked into `when`'s `arg_values`. Assert that linkage: `when`
/// is an event handler and the registry knows the canonical events.
///
...
// ===========================================================================
// test_command_registry.py :: TestRewriteAliasResolution (cfg_rewrite_name)
// ===========================================================================

/// `test_rewrite_alias_is_derived_from_subcommand_property` — `dict for` /
/// `dict map` declare their CFG-rewrite qualified names on the subcommand.
///
...
/// require Tk`), so `button` IS present — this is the documented Rust
/// divergence from the Python lazy loader, where Tk is a separate loader.
#[test]
...
// ===========================================================================
// test_dialect_helpers.py :: is_irules_dialect predicate
// ===========================================================================

/// `test_value_predicate_accepts_canonical_and_legacy_alias` +
/// `test_value_predicate_rejects_other_dialects_and_none` — the iRules
/// predicate accepts `f5-irules` and the legacy `irules` alias, rejects
/// everything else and `None`.
///
...
// ===========================================================================
// test_detect_dialect.py  (the dialect-name surface that lives in this crate)
//
// `detect_dialect_from_source` itself is implemented in tcl-compiler /
// tcl-lsp-core, not tcl-registry, so the source-scanning behaviour cannot be
// ported here. The dialect *vocabulary* it resolves into — and which this
// crate owns — is covered instead.
// ===========================================================================

/// The detection targets (`tcl8.4`, `tcl8.5`, `tcl8.6`, `tcl9.0`, `f5-irules`,
/// `expect`) that `test_detect_dialect.py` expects as outputs are all
...
fn detection_target_dialects_are_known() {
    ...
    // has no parse — the analogue of returning `None`.
...
///
/// NOTE: the strict "absent from tcl8.6" half of the Python intent is
/// deliberately NOT asserted here. The Rust `const` spec is declared
...
// ===========================================================================
// Registry construction basics (mirrors the Python build_default checks)
// ===========================================================================
