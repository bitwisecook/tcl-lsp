...
pub(super) fn specs() -> Vec<CommandSpec> {
    vec![
        ...
        block(
            ...
            "A case list is a *value* (`{pattern body …}` inside one word) rather than a word grammar, which is why it is a separate field from `clause_grammar`. `case_list switch` names the shipped descriptor; the block spells out all seventeen plain-data fields.",
        ),
