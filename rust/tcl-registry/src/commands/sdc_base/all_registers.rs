...
const FORMS: &[FormSpec] = &[
    FormSpec { kind: FormKind::Default, synopsis: "all_registers ?-clock clock_name? ?-rise_clock clock_name? ?-fall_clock clock_name? ?-cells? ?-data_pins? ?-clock_pins? ?-slave_clock_pins? ?-async_pins? ?-output_pins? ?-level_sensitive?" },
];
