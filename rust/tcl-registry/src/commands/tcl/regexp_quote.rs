...
/// Command spec for `regexp::quote`, one of four recognised spellings of the
...
/// spec lets the taint analyser recognise a user proc under this alternate
/// name — or one of its three siblings — as the same REGEX_LITERAL-quoting
/// idiom (`tcl_compiler`'s `regexp_quote_suppresses_t103` test) wherever it is
/// called, standard Tcl or otherwise. `surface: ALL_TCL` (no iRules row) is
/// deliberate, not an oversight: iRules excludes all four spellings, and this
/// group's omission of an iRules row is exactly what enforces that — an
/// `ALL_TCL` surface is simply absent under iRules, so there is no separate
/// disable list. Folding `IRULES` into a dialect union here would re-admit
/// exactly the commands iRules means to exclude.
