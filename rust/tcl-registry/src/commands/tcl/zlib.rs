...
//! Tcl 8.5's zlib.html serves the same soft-404 "URL Not Found" page
//! template `coroutine.rs` documents for `coroutine.n` on the same
//! 8.4/8.5 doc trees (a 200-status page that says "The page you are
//! looking for cannot be found", not a real manual page).
//! Tcl 8.4's zlib.html is unreachable — both it and a known-good control
//! page from the same 8.4 doc tree fail with either a bare connection
//! timeout or a Cloudflare 522, indicating a transient host-side outage
//! of the 8.4 tree specifically rather than a real 404 — so the
//! 8.4-absence conclusion above rests on the 8.5 confirmation and TIP 234
//! landing in 8.6 (years after 8.4's feature-freeze), not on a direct
//! fetch of the 8.4 page itself.
//!
