...
/// `regex::quote` / `regexp::quote`). None of the four is a documented core
/// Tcl command: Tcl ships no metacharacter-quoting builtin, and
/// `regex_quote.html` (`.htm` on the 8.6 tree, whose `.html` redirects to a
/// broken `Location` header) 404s on every tcl-lang.org manpage tree for 8.4,
/// 8.5, 8.6, 9.0, and 9.1 alike; the `TclCmd` alphabetical command index
/// likewise lists no `quote`- or `regex`-prefixed command (only
...
/// three siblings — lets the taint analyser recognise a user proc under this
...
/// iRules, so there is no separate disable list. Folding `IRULES` into a
/// dialect union here would re-admit exactly the commands iRules means to
/// exclude.
