...
mod tests {
    ...
    fn the_point_names_the_catalogue_runtime_base() {
        ...
        for profile in DialectProfile::all() {
            ...
            assert!(!environment.is_lenient(), "{} has no environment", profile.name);
        ...
        assert_eq!(
            resolve_environment("tk").point().and_then(DialectPoint::tcl_version),
            Some(tcl_dialect::TclVersion::V8_6)
        );
        // A non-Tcl family has no rung on the Tcl ladder.
        assert_eq!(resolve_environment("jim").point().and_then(DialectPoint::tcl_version), None);
    }
