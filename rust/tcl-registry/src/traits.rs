...
declare_trait_examples! {
    ...
    LoopListHeader => flow!("set items {alpha beta}\nforeach item $items { puts $item }"; (1, "foreach"); (0, "{alpha beta}", "supplies the list expression"), (1, "foreach item $items", "evaluates the list once in the loop header"), (1, "$items", "is evaluated once in the loop header"), (1, "$item", "receives one element per iteration"));
    ...
    DefinitionBodyMemberOnly => flow!("oo::class create Point {\n    variable x\n    method move {dx} {incr x $dx}\n}"; (1, "method"); (0, "oo::class create Point", "opens a definition body"), (1, "method move", "is legal only as a member of that body"), (2, "incr x $dx", "the member body is ordinary code again"));
