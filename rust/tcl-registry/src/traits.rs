...
declare_traits! {
    ...
    /// Only the zero-argument command or instance-method form returns
    /// attacker-controlled data. Coordinate/limit forms of Tk value getters
    /// derive their result from explicit arguments and must not be overtainted.
    TaintSourceZeroArgs => TAINT_SOURCE_ZERO_ARGS, Security, "a zero-argument taint source";
    /// Values written through registry-declared `VarWrite` positions become
    /// externally controlled.  Tk uses this for editable widgets whose
    /// `-textvariable` / `-variable` is updated by later user interaction;
    /// unlike `TAINT_SOURCE`, it does not taint the constructor's return value.
    TaintsVarWrites => TAINTS_VAR_WRITES, Security, "taints variables written by the command";
    /// An object instance method configures the option table declared by its
    ...
    /// A Tk **geometry manager** — a command that takes over placement of the
    /// widgets given to it (`pack`, `grid`, `place`). Whether it also claims
    /// exclusive propagation ownership of an effective container is a
    /// separate [`crate::tk_geometry::TkGeometryManagerSpec`] fact: `pack`
    /// and `grid` do, while `place` deliberately does not.
    ///
...
declare_trait_examples! {
    ...
    HasBooleanCond => flow!("set user_count [gets stdin]\nif {$user_count > 0} { puts busy }"; (1, "if"); (0, "[gets stdin]", "supplies an external value"), (1, "if {$user_count > 0}", "reads its first argument in boolean expression context"), (1, "puts busy", "runs only when the condition is true"));
    ...
    HasLoopBody => flow!("set items {alpha beta}\nforeach item $items { lappend seen $item }\nputs $seen"; (1, "foreach"); (0, "{alpha beta}", "supplies loop values"), (1, "foreach item $items", "recursively analyses its loop body"), (1, "lappend seen $item", "runs in that loop body"), (2, "$seen", "contains the values produced by each iteration"));
    NeverInlineBody => flow!("set items {alpha beta}\nforeach item $items { puts $item }\nputs done"; (1, "foreach"); (0, "{alpha beta}", "supplies runtime iterations"), (1, "foreach item $items", "owns a distinct loop body"), (1, "{ puts $item }", "must remain a distinct loop body"), (2, "puts done", "runs after the loop completes"));
    LoopListHeader => flow!("set items {alpha beta}\nforeach name $items { puts $name }"; (1, "foreach"); (0, "{alpha beta}", "supplies the list"), (1, "$items", "is substituted once, before the first iteration"), (1, "foreach name", "reads that one list rather than re-evaluating it per pass"), (1, "puts $name", "sees one element each time round"));
    ...
    DynamicEvalBody => flow!("set body [gets stdin]\napply [list {} $body]\nputs done"; (1, "apply"); (0, "[gets stdin]", "supplies a runtime body"), (1, "apply [list {} $body]", "dynamically evaluates the body as Tcl"), (2, "puts done", "runs only if that body completes"));
    ...
    ExpansionEscapeSafe => flow!("set args [list world]\nputs {*}$args\nputs done"; (1, "puts"); (0, "[list world]", "creates the expanded argument"), (1, "puts {*}$args", "expands data without introducing a frame-sensitive command name"), (2, "puts done", "continues in the same frame"));
    ...
    FrameHashBuiltin => flow!("proc readLocal {} {\n    set local value\n    return [set local]\n}\nputs [readLocal]"; (1, "set"); (1, "set local value", "writes the frame's variable table"), (2, "[set local]", "reads it back through the frame-hash builtin, not a namespace lookup"), (4, "[readLocal]", "observes the value that round trip produced"));
    ...
    ScriptConcatenatesArgs => flow!("set command concat\neval $command hello world\nputs done"; (1, "eval"); (0, "command concat", "supplies the first script fragment"), (1, "eval $command hello world", "concatenates trailing words into one script"), (2, "puts done", "runs after the constructed script"));
    ...
    CatchableThrow => flow!("set status [catch {error failure} message]\nputs [list $status $message]"; (0, "catch"); (0, "error failure", "throws a catchable Tcl error"), (0, "catch", "intercepts the error edge"), (1, "$status $message", "observes code 1 and the message"));
    ...
    ProvidesPackage => flow!("namespace eval demo { proc run {} { return ok } }\npackage provide demo 1.0\nputs [demo::run]"; (1, "package"); (0, "demo {", "creates the public namespace"), (1, "package provide demo 1.0", "declares this unit loadable by external callers"), (2, "demo::run", "represents one such caller"));
