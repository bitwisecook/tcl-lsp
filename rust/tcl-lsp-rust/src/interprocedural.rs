...
use tcl_compiler::interprocedural::{
    ConstantReturn, ProcArgTrait, ProcSummary,
};
...
fn summary_to_dict<'py>(
    ...
    d.set_item("constant_return", constant_return_to_py(s.constant_return.as_ref()))?;
    d.set_item(
        "return_depends_on_params",
        PyList::new_bound(py, &s.return_depends_on_params),
    )?;
    d.set_item("return_passthrough_param", s.return_passthrough_param.clone())?;
...
fn constant_return_to_py(cr: Option<&ConstantReturn>) -> Option<(String, String)> {
    match cr {
        ...
        Some(ConstantReturn::Bool(b)) => Some((
            "bool".into(),
