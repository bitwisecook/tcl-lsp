...
//! End-to-end LSP smoke test for `tcl-lsp-server::completion`.
//!
...
//! `textDocument/completion` sequence a real client would.
//! Verifies that proc-name completion at the start of a fresh
//! line returns a user-defined proc.
...
async fn completion_smoke() {
    ...
    assert!(
        ...
        "expected completionProvider capability: {resp}",
    );
    ...
    // didOpen — two procs and an empty third line.
    let did_open = concat!(
    ...
    // Completion at line 2, char 0 (start of empty third line).
    let req = r#"{"jsonrpc":"2.0","id":2,"method":"textDocument/completion","params":{"textDocument":{"uri":"file:///t.tcl"},"position":{"line":2,"character":0}}}"#;
    ...
    assert!(
        !response.is_empty(),
        "did not receive id=2 completion response within the deadline",
    );
    assert!(
        response.contains("\"greet\""),
        "expected `greet` in completion items, got {response}",
    );
    assert!(
        response.contains("\"shout\""),
        "expected `shout` in completion items, got {response}",
    );
