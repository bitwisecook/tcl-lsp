...
//!
//! Everything the editor has open lives in [`Backend::documents`] and never
//! touches this module. What does touch it is the other half of the picture:
//! the file a `source` edge points at, the sibling the workspace scan indexed,
//! the `config.ini` a session layers under the editor's settings, the line a
//! spec-pack notice names. Natively those are all `std::fs` calls. In a
//! browser worker there is no `std::fs` to call — the host has the bytes and
//! has to hand them over.
//!
//! [`SourceStore`] is that seam, and [`Backend`] holds exactly one of them.
//!
...
//!
//! **Scope.** Only the single-document chokepoints are routed through the
//! store today: the closed-file read behind `read_document`, the decode-report
//! comparison, `scan_disk_file`, the `config.ini` layers, the `exportConfig`
//! write, the notice line-range read, and the recovery-widening package read.
//! The whole-workspace paths — the folder scan, the package database, and
//! spec-pack discovery — still call `std::fs` directly. They are M2 work, and
//! they are safe to leave: each one already `.ok()`s every I/O result and
//! folds a failure into an empty result, so on a target with no filesystem
//! they contribute nothing rather than failing the session.
//!
//! [`Backend`]: crate::Backend
//! [`Backend::documents`]: crate::Backend
...
/// for the path and whether it is a directory, and a browser store has no
/// honest answer for anything more.
...
pub trait SourceStore: std::fmt::Debug + Send + Sync {
    ...
    ///
    /// The counterpart of [`tcl_lsp_core::source_decode::read_source_file`],
    /// and the reason it exists (issue #1326): a closed file with one
    /// ill-formed byte must still contribute its procs, its `source` edges,
    /// and its symbols, so decoding is lossy and only the *read* can fail.
    ///
    /// # Errors
    ///
    ...
    fn read_source(
        &self,
        path: &Path,
    ) -> io::Result<(String, tcl_lsp_core::source_decode::DecodeReport)> {
        Ok(tcl_lsp_core::source_decode::decode_source(
            &self.read(path)?,
        ))
    }
}
