//! Differential tests for the `f5 explain-flow` verb (static path) — the
...
fn libpcap_mixed_flows_match_golden() {
    ...
    assert_unmatched("pcap-remap-sample.pcap");
}

#[test]
fn pcapng_flows_match_golden() {
    assert_unmatched("pcap-remap-sample.pcapng");
}

#[test]
fn libpcap_unknown_trailer_match_golden() {
    assert_unmatched("pcap-remap-unknown.pcap");
}
