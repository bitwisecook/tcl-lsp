...
CASES: dict[str, bytes] = {
    ...
    # -- encoding confusion --------------------------------------------
    "enc_utf16le_bom.tcl": "﻿".encode("utf-16-le")
    + BODY_ASCII.decode().encode("utf-16-le"),
    "enc_utf16be_bom.tcl": "﻿".encode("utf-16-be")
    + BODY_ASCII.decode().encode("utf-16-be"),
    "enc_utf16le_nobom.tcl": BODY_ASCII.decode().encode("utf-16-le"),
    "enc_utf32le_bom.tcl": "﻿".encode("utf-32-le")
    + BODY_ASCII.decode().encode("utf-32-le"),
...
def main() -> int:
    out = pathlib.Path(
        sys.argv[1]
        if len(sys.argv) > 1
        else "rust/tcl-lsp-core/tests/fixtures/encoding"
    )
