...
def test_vscode_diagnostic_settings_match_registry():
    # The `tclLsp.*` sections of package.json are now generated by
    # `cargo xtask gen-vscode-package` from the Rust `DiagCode` catalogue (the
    # rewrite's source of truth), which is a *superset* of this Python registry
    # — it carries a few extra Rust-only user-configurable diagnostics. So every
    # Python-registry code must have a toggle, but package.json may add more.
    vscode_diag = _vscode_codes_from_json("tclLsp.diagnostics.")
    assert diagnostic_codes() <= vscode_diag, (
        f"VS Code package.json is missing diagnostic toggles for registry codes:\n"
...
def test_typescript_catalog_importable_by_node():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check_script.write_text(
            ...
            "  process.exit(1);\n"
            "}\n"
            # diagnosticCatalog.ts is generated from the Rust `DiagCode`
            # catalogue (a superset of this Python registry), so it must carry
            # *at least* every Python diagnostic — extra Rust-only codes are fine.
            f"if (c.DIAGNOSTICS.length < {len(diagnostic_codes())}) {{\n"
            '  console.error("DIAGNOSTICS count too low:", c.DIAGNOSTICS.length);\n'
            "  process.exit(1);\n"
            "}\n"
