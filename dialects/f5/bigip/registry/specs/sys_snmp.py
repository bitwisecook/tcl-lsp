# tcl-lsp — a language server and toolchain for Tcl
# Copyright (C) 2026 James Deucker (bitwisecook) <https://github.com/bitwisecook>
#
# This program is free software: you can redistribute it and/or modify
# it under the terms of the GNU Affero General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
#
# This program is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU Affero General Public License for more details.
#
# You should have received a copy of the GNU Affero General Public License
# along with this program.  If not, see <https://www.gnu.org/licenses/>.
#
# SPDX-License-Identifier: AGPL-3.0-or-later

from __future__ import annotations

from ..models import (
    BigipObjectKindSpec,
    BigipObjectSpec,
    BigipPropertySpec,
)
from ._base import register


@register
def register_spec() -> BigipObjectSpec:
    return BigipObjectSpec(
        kind_spec=BigipObjectKindSpec(
            "sys_snmp",
            module="sys",
            object_types=("snmp",),
        ),
        header_types=(("sys", "snmp"),),
        properties=(
            BigipPropertySpec(
                name="agent-addresses",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "replace-all-with")),
            ),
            BigipPropertySpec(
                name="agent-trap",
                value_type="enum",
                enum_values=("disabled", "enabled"),
                shape_kind="boolean",
                default="enabled",
            ),
            BigipPropertySpec(
                name="allowed-addresses",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "replace-all-with")),
                default="127",
            ),
            BigipPropertySpec(
                name="auth-trap",
                value_type="enum",
                enum_values=("disabled", "enabled"),
                shape_kind="boolean",
                default="disabled",
            ),
            BigipPropertySpec(
                name="bigip-traps",
                value_type="enum",
                enum_values=("disabled", "enabled"),
                shape_kind="boolean",
                default="enabled",
            ),
            BigipPropertySpec(
                name="communities",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "modify", "replace-all-with")),
                block=(
                    BigipPropertySpec(
                        name="access",
                        value_type="enum",
                        in_sections=("communities",),
                        enum_values=("ro", "rw"),
                        default="ro",
                        usage_flags=frozenset(("read_only",)),
                    ),
                    BigipPropertySpec(
                        name="community-name",
                        value_type="string",
                        in_sections=("communities",),
                    ),
                    BigipPropertySpec(
                        name="description",
                        value_type="string",
                        in_sections=("communities",),
                    ),
                    BigipPropertySpec(
                        name="ipv6",
                        value_type="enum",
                        in_sections=("communities",),
                        enum_values=("disabled", "enabled"),
                        shape_kind="boolean",
                        default="disabled",
                    ),
                    BigipPropertySpec(
                        name="oid-subset",
                        value_type="string",
                        in_sections=("communities",),
                    ),
                    BigipPropertySpec(
                        name="source",
                        value_type="string",
                        in_sections=("communities",),
                        shape_kind="ip-address",
                        default="default, which means allow any source address to access the MIB",
                    ),
                ),
            ),
            BigipPropertySpec(
                name="access",
                value_type="enum",
                in_sections=("communities",),
                enum_values=("ro", "rw"),
                default="ro",
                usage_flags=frozenset(("read_only",)),
            ),
            BigipPropertySpec(
                name="community-name",
                value_type="string",
                in_sections=("communities",),
            ),
            BigipPropertySpec(
                name="description",
                value_type="string",
                in_sections=("communities",),
            ),
            BigipPropertySpec(
                name="ipv6",
                value_type="enum",
                in_sections=("communities",),
                enum_values=("disabled", "enabled"),
                shape_kind="boolean",
                default="disabled",
            ),
            BigipPropertySpec(name="oid-subset", value_type="string", in_sections=("communities",)),
            BigipPropertySpec(
                name="source",
                value_type="string",
                in_sections=("communities",),
                shape_kind="ip-address",
                default="default, which means allow any source address to access the MIB",
            ),
            BigipPropertySpec(name="description", value_type="string"),
            BigipPropertySpec(
                name="disk-monitors",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "modify", "replace-all-with")),
                block=(
                    BigipPropertySpec(
                        name="description",
                        value_type="string",
                        in_sections=("disk-monitors",),
                    ),
                    BigipPropertySpec(
                        name="minspace",
                        value_type="integer",
                        in_sections=("disk-monitors",),
                    ),
                    BigipPropertySpec(
                        name="minspace-type",
                        value_type="enum",
                        in_sections=("disk-monitors",),
                        enum_values=("percent", "size"),
                    ),
                    BigipPropertySpec(
                        name="path",
                        value_type="string",
                        in_sections=("disk-monitors",),
                        required=True,
                    ),
                ),
            ),
            BigipPropertySpec(
                name="description",
                value_type="string",
                in_sections=("disk-monitors",),
            ),
            BigipPropertySpec(
                name="minspace",
                value_type="integer",
                in_sections=("disk-monitors",),
            ),
            BigipPropertySpec(
                name="minspace-type",
                value_type="enum",
                in_sections=("disk-monitors",),
                enum_values=("percent", "size"),
            ),
            BigipPropertySpec(
                name="path",
                value_type="string",
                in_sections=("disk-monitors",),
                required=True,
            ),
            BigipPropertySpec(name="include", value_type="string"),
            BigipPropertySpec(
                name="l2forward-vlan",
                value_type="list",
                required=True,
                allow_none=True,
                list_operators=frozenset(("add", "delete", "replace-all-with")),
                default="none",
            ),
            BigipPropertySpec(name="load-max1", value_type="integer"),
            BigipPropertySpec(name="load-max15", value_type="integer"),
            BigipPropertySpec(name="load-max5", value_type="integer"),
            BigipPropertySpec(
                name="process-monitors",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "modify", "replace-all-with")),
                block=(
                    BigipPropertySpec(
                        name="description",
                        value_type="string",
                        in_sections=("process-monitors",),
                    ),
                    BigipPropertySpec(
                        name="max-processes",
                        value_type="integer",
                        in_sections=("process-monitors",),
                        default="1",
                    ),
                    BigipPropertySpec(
                        name="min-processes",
                        value_type="integer",
                        in_sections=("process-monitors",),
                        default="1",
                    ),
                    BigipPropertySpec(
                        name="process",
                        value_type="string",
                        in_sections=("process-monitors",),
                        required=True,
                    ),
                ),
            ),
            BigipPropertySpec(
                name="description",
                value_type="string",
                in_sections=("process-monitors",),
            ),
            BigipPropertySpec(
                name="max-processes",
                value_type="integer",
                in_sections=("process-monitors",),
                default="1",
            ),
            BigipPropertySpec(
                name="min-processes",
                value_type="integer",
                in_sections=("process-monitors",),
                default="1",
            ),
            BigipPropertySpec(
                name="process",
                value_type="string",
                in_sections=("process-monitors",),
                required=True,
            ),
            BigipPropertySpec(
                name="snmpv1",
                value_type="enum",
                enum_values=("disabled", "enabled"),
                shape_kind="boolean",
                default="enabled",
            ),
            BigipPropertySpec(
                name="snmpv2",
                value_type="enum",
                enum_values=("disabled", "enabled"),
                shape_kind="boolean",
            ),
            BigipPropertySpec(
                name="sys-contact",
                value_type="string",
                default='"Customer Name<admin@customer',
            ),
            BigipPropertySpec(name="sys-location", value_type="string", default="Network Closet 1"),
            BigipPropertySpec(name="sys-services", value_type="integer", default="78"),
            BigipPropertySpec(name="trap-community", value_type="string", default="public"),
            BigipPropertySpec(
                name="trap-source",
                value_type="string",
                shape_kind="ip-address",
                default="none",
            ),
            BigipPropertySpec(
                name="traps",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "modify", "replace-all-with")),
                block=(
                    BigipPropertySpec(
                        name="auth-password",
                        value_type="string",
                        in_sections=("traps",),
                    ),
                    BigipPropertySpec(
                        name="auth-protocol",
                        value_type="enum",
                        in_sections=("traps",),
                        allow_none=True,
                        enum_values=("md5", "none", "sha"),
                        default="none",
                    ),
                    BigipPropertySpec(
                        name="community", value_type="string", in_sections=("traps",)
                    ),
                    BigipPropertySpec(
                        name="description", value_type="string", in_sections=("traps",)
                    ),
                    BigipPropertySpec(
                        name="engine-id",
                        value_type="unknown",
                        in_sections=("traps",),
                        allow_none=True,
                        default="none",
                    ),
                    BigipPropertySpec(
                        name="host",
                        value_type="string",
                        in_sections=("traps",),
                        required=True,
                        shape_kind="ip-address",
                    ),
                    BigipPropertySpec(
                        name="port",
                        value_type="integer",
                        in_sections=("traps",),
                        default="162",
                    ),
                    BigipPropertySpec(
                        name="privacy-password",
                        value_type="string",
                        in_sections=("traps",),
                    ),
                    BigipPropertySpec(
                        name="privacy-protocol",
                        value_type="enum",
                        in_sections=("traps",),
                        allow_none=True,
                        enum_values=("aes", "des", "none"),
                        default="none",
                    ),
                    BigipPropertySpec(
                        name="security-level",
                        value_type="enum",
                        in_sections=("traps",),
                        enum_values=("auth-no-privacy", "auth-privacy", "no-auth-no-privacy"),
                        default="no-auth-no-privacy",
                    ),
                    BigipPropertySpec(
                        name="security-name",
                        value_type="string",
                        in_sections=("traps",),
                        required=True,
                        default="none",
                    ),
                    BigipPropertySpec(
                        name="version",
                        value_type="enum",
                        in_sections=("traps",),
                        enum_values=("1", "2c", "3"),
                        default="2c",
                    ),
                ),
            ),
            BigipPropertySpec(name="auth-password", value_type="string", in_sections=("traps",)),
            BigipPropertySpec(
                name="auth-protocol",
                value_type="enum",
                in_sections=("traps",),
                allow_none=True,
                enum_values=("md5", "none", "sha"),
                default="none",
            ),
            BigipPropertySpec(name="community", value_type="string", in_sections=("traps",)),
            BigipPropertySpec(name="description", value_type="string", in_sections=("traps",)),
            BigipPropertySpec(
                name="engine-id",
                value_type="unknown",
                in_sections=("traps",),
                allow_none=True,
                default="none",
            ),
            BigipPropertySpec(
                name="host",
                value_type="string",
                in_sections=("traps",),
                required=True,
                shape_kind="ip-address",
            ),
            BigipPropertySpec(
                name="port",
                value_type="integer",
                in_sections=("traps",),
                default="162",
            ),
            BigipPropertySpec(name="privacy-password", value_type="string", in_sections=("traps",)),
            BigipPropertySpec(
                name="privacy-protocol",
                value_type="enum",
                in_sections=("traps",),
                allow_none=True,
                enum_values=("aes", "des", "none"),
                default="none",
            ),
            BigipPropertySpec(
                name="security-level",
                value_type="enum",
                in_sections=("traps",),
                enum_values=("auth-no-privacy", "auth-privacy", "no-auth-no-privacy"),
                default="no-auth-no-privacy",
            ),
            BigipPropertySpec(
                name="security-name",
                value_type="string",
                in_sections=("traps",),
                required=True,
                default="none",
            ),
            BigipPropertySpec(
                name="version",
                value_type="enum",
                in_sections=("traps",),
                enum_values=("1", "2c", "3"),
                default="2c",
            ),
            BigipPropertySpec(
                name="users",
                value_type="list",
                list_operators=frozenset(("add", "delete", "modify", "replace-all-with")),
                block=(
                    BigipPropertySpec(
                        name="access",
                        value_type="enum",
                        in_sections=("users",),
                        enum_values=("ro", "rw"),
                        default="ro",
                        usage_flags=frozenset(("read_only",)),
                    ),
                    BigipPropertySpec(
                        name="auth-password",
                        value_type="string",
                        in_sections=("users",),
                    ),
                    BigipPropertySpec(
                        name="auth-protocol",
                        value_type="enum",
                        in_sections=("users",),
                        allow_none=True,
                        enum_values=("md5", "none", "sha"),
                        default="none",
                    ),
                    BigipPropertySpec(
                        name="description", value_type="string", in_sections=("users",)
                    ),
                    BigipPropertySpec(
                        name="oid-subset", value_type="string", in_sections=("users",)
                    ),
                    BigipPropertySpec(
                        name="privacy-password",
                        value_type="string",
                        in_sections=("users",),
                    ),
                    BigipPropertySpec(
                        name="privacy-protocol",
                        value_type="enum",
                        in_sections=("users",),
                        allow_none=True,
                        enum_values=("aes", "des", "none"),
                        default="none",
                    ),
                    BigipPropertySpec(
                        name="security-level",
                        value_type="enum",
                        in_sections=("users",),
                        enum_values=("auth-no-privacy", "auth-privacy", "no-auth-no-privacy"),
                        default="no-auth-no-privacy",
                    ),
                    BigipPropertySpec(
                        name="username",
                        value_type="string",
                        in_sections=("users",),
                        required=True,
                    ),
                ),
            ),
            BigipPropertySpec(
                name="access",
                value_type="enum",
                in_sections=("users",),
                enum_values=("ro", "rw"),
                default="ro",
                usage_flags=frozenset(("read_only",)),
            ),
            BigipPropertySpec(name="auth-password", value_type="string", in_sections=("users",)),
            BigipPropertySpec(
                name="auth-protocol",
                value_type="enum",
                in_sections=("users",),
                allow_none=True,
                enum_values=("md5", "none", "sha"),
                default="none",
            ),
            BigipPropertySpec(name="description", value_type="string", in_sections=("users",)),
            BigipPropertySpec(name="oid-subset", value_type="string", in_sections=("users",)),
            BigipPropertySpec(name="privacy-password", value_type="string", in_sections=("users",)),
            BigipPropertySpec(
                name="privacy-protocol",
                value_type="enum",
                in_sections=("users",),
                allow_none=True,
                enum_values=("aes", "des", "none"),
                default="none",
            ),
            BigipPropertySpec(
                name="security-level",
                value_type="enum",
                in_sections=("users",),
                enum_values=("auth-no-privacy", "auth-privacy", "no-auth-no-privacy"),
                default="no-auth-no-privacy",
            ),
            BigipPropertySpec(
                name="username",
                value_type="string",
                in_sections=("users",),
                required=True,
            ),
            BigipPropertySpec(
                name="v1-traps",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "modify", "replace-all-with")),
                block=(
                    BigipPropertySpec(
                        name="community", value_type="string", in_sections=("v1-traps",)
                    ),
                    BigipPropertySpec(
                        name="description",
                        value_type="string",
                        in_sections=("v1-traps",),
                    ),
                    BigipPropertySpec(
                        name="host",
                        value_type="string",
                        in_sections=("v1-traps",),
                        required=True,
                        shape_kind="ip-address",
                    ),
                    BigipPropertySpec(
                        name="port",
                        value_type="integer",
                        in_sections=("v1-traps",),
                        default="162",
                    ),
                ),
            ),
            BigipPropertySpec(name="community", value_type="string", in_sections=("v1-traps",)),
            BigipPropertySpec(name="description", value_type="string", in_sections=("v1-traps",)),
            BigipPropertySpec(
                name="host",
                value_type="string",
                in_sections=("v1-traps",),
                required=True,
                shape_kind="ip-address",
            ),
            BigipPropertySpec(
                name="port",
                value_type="integer",
                in_sections=("v1-traps",),
                default="162",
            ),
            BigipPropertySpec(
                name="v2-traps",
                value_type="list",
                allow_none=True,
                list_operators=frozenset(("add", "delete", "modify", "replace-all-with")),
                block=(
                    BigipPropertySpec(
                        name="community", value_type="string", in_sections=("v2-traps",)
                    ),
                    BigipPropertySpec(
                        name="description",
                        value_type="string",
                        in_sections=("v2-traps",),
                    ),
                    BigipPropertySpec(
                        name="host",
                        value_type="string",
                        in_sections=("v2-traps",),
                        required=True,
                        shape_kind="ip-address",
                    ),
                    BigipPropertySpec(
                        name="port",
                        value_type="integer",
                        in_sections=("v2-traps",),
                        default="162",
                    ),
                ),
            ),
            BigipPropertySpec(name="community", value_type="string", in_sections=("v2-traps",)),
            BigipPropertySpec(name="description", value_type="string", in_sections=("v2-traps",)),
            BigipPropertySpec(
                name="host",
                value_type="string",
                in_sections=("v2-traps",),
                required=True,
                shape_kind="ip-address",
            ),
            BigipPropertySpec(
                name="port",
                value_type="integer",
                in_sections=("v2-traps",),
                default="162",
            ),
        ),
    )
