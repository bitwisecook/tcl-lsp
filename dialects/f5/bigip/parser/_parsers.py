"""Per-kind ``_parse_*`` functions, kind-classification tables, and dispatch tables.

Every kind-specific parser (data-groups, pools, virtuals, profiles,
monitors, persistence, iRules, policies, plus every ``ltm.* / net.* /
sys.* / security.* / apm.* / cm.* / gtm.* / pem.* / auth.*`` typed
or minimal kind) lives here, together with the kind-classification
tables (``_TWO_WORD_TYPES`` / ``_THREE_WORD_TYPES`` / …), the
profile-type map, and the per-module dispatch tables that route
kind labels to ``BigipConfig`` attribute names and parser
functions.

The top-level driver :func:`parse_bigip_conf` in :mod:`._driver`
reads these tables to dispatch each parsed block to the right
sub-parser.  The brace-balanced helpers it depends on live in
:mod:`._helpers`.
"""
...
def _parse_virtual(
    ...
    profile_attachments: BigipList = BigipList()
    profiles_block = props.get("profiles")
    if profiles_block:
        # Build the typed BigipList of ProfileAttachment items
        # directly into the model field — the parser no longer
        # populates a parallel ``tuple[str, ...]`` for
        # back-compat.  Lazy import to avoid pulling the registry
        # in on every parse call.
        ...
        parsed_value = _spec.parse(profiles_block, ParseContext()).value
        if isinstance(parsed_value, BigipList):
            profile_attachments = parsed_value

    persist_attachments: BigipList = BigipList()
    ...
    if persist_block:
        ...
        parsed_value = _spec.parse(persist_block, ParseContext()).value
        if isinstance(parsed_value, BigipList):
            persist_attachments = parsed_value
...
def _parse_typed_field(
    *,
    ...
    :data:`dialects.f5.bigip.registry.pilot.PILOT_PROPERTY_SPECS`, run the
