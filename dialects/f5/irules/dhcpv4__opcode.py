# Enriched from F5 iRules reference documentation.
"""DHCPv4::opcode -- This command returns opcode field from DHCPv4 message."""

from __future__ import annotations

from compiler.registry._base import CommandDef
from compiler.registry.models import CommandSpec, FormKind, FormSpec, HoverSnippet, ValidationSpec
from compiler.registry.namespace_models import EventRequires
from compiler.registry.signatures import Arity
from compiler.side_effects import ConnectionSide, SideEffect, SideEffectTarget

from ._base import _IRULES_ONLY, register

_SOURCE = "https://clouddocs.f5.com/api/irules/DHCPv4__opcode.html"


@register
class Dhcpv4OpcodeCommand(CommandDef):
    name = "DHCPv4::opcode"

    @classmethod
    def spec(cls) -> CommandSpec:
        return CommandSpec(
            name="DHCPv4::opcode",
            dialects=_IRULES_ONLY,
            hover=HoverSnippet(
                summary="This command returns opcode field from DHCPv4 message.",
                synopsis=("DHCPv4::opcode",),
                snippet=(
                    "This command returns opcode field from DHCPv4 message\n"
                    "\n"
                    "Details (syntax):\n"
                    "DHCPv4::opcode"
                ),
                source=_SOURCE,
                examples=(
                    'when CLIENT_DATA {\n        log local0. "Htype [DHCPv4::opcode]"\n    }'
                ),
                return_value="This command returns opcode field from DHCPv4 message",
            ),
            forms=(
                FormSpec(
                    kind=FormKind.DEFAULT,
                    synopsis="DHCPv4::opcode",
                ),
            ),
            validation=ValidationSpec(
                arity=Arity(),
            ),
            event_requires=EventRequires(),
            side_effect_hints=(
                SideEffect(
                    target=SideEffectTarget.NETWORK_IO,
                    reads=True,
                    connection_side=ConnectionSide.BOTH,
                ),
            ),
        )
