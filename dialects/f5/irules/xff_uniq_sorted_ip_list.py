...
"""xff_uniq_sorted_ip_list -- Sorted unique valid IPs from X-Forwarded-For."""
...
_SOURCE = "https://clouddocs.f5.com/api/irules/xff_uniq_sorted_ip_list.html"


@register
class XffUniqSortedIpListProc(CommandDef):
    ...
    def spec(cls) -> CommandSpec:
        return CommandSpec(
            ...
            hover=HoverSnippet(
                ...
                synopsis=(
                    "call xff_uniq_sorted_ip_list",
                    'call xff_uniq_sorted_ip_list "X-Real-IP"',
                ),
                snippet=(
                    "Collects all addresses from the named header (default "
                    "`X-Forwarded-For`), even when the header appears multiple "
                    "times.\n"
                    "\n"
                    ...
                    "\n"
                    "See also: `xff_list` (convenience alias), "
                    "`xff_uniq_ordered_ip_list` (preserves header order)."
                ),
                source=_SOURCE,
                examples=(
            ...
            forms=(
                FormSpec(
                    ...
                    synopsis="xff_uniq_sorted_ip_list ?xff_header_name?",
                ),
            ),
