"""Shared rendering layer for the compiler explorer.

A leaf module that owns the view-rendering machinery used by *both* the flat
``--text`` CLI path (``tooling.explorer.cli``) and the Textual TUI
(``tooling.explorer.tui``).  Keeping it here — rather than in ``cli`` — lets
the TUI import the renderers without forming a ``cli`` <-> ``tui`` import cycle
(the CLI imports ``run_tui`` from the TUI, so the TUI must not import the CLI).

It must stay a leaf: it may import the pipeline, annotations, cfg-layout, and
the lower-level compiler/formatter helpers, but never ``cli`` or ``tui``.
"""
