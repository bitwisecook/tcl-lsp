...
| O9 | **Which tier is E-R2-*untrusted*.** §6.4 keys the workspace half on the **editor's Workspace Trust state** ("in an untrusted workspace … overriding a canonical environment or shipped command requires explicit trusted opt-in"), but nothing on the discovery path is told that state: a `PackFile` carries a discovery `Tier`, not a trust class. The `one-loader` lane made this decidable rather than academic, because the discovery path now passes its real tier into the loader. Resolved *for now* by deriving the predicate from the mapping that already existed — `PackEnvironmentTier::provenance` calls `Tier::Workspace` `Provenance::WorkspaceTrusted` — so a trusted workspace pack may still `-override` a shipped command (the collision policy `install.rs` implements and tests), and the untrusted class is reachable today only through the live Spec Studio override tier. Before the lane, the loader and the environment model simply disagreed about what `Tier::Workspace` meant | Nothing today: the safe direction is already taken (a workspace pack cannot register a *security* fact past the registration-time gate, whatever the loader let it declare). What is blocked is honest enforcement of the untrusted case, which cannot happen until the trust state is plumbed | O8's consent surface, plus a `Tier` (or a trust field beside it) that carries the editor's Workspace Trust state to `pack::load` |
