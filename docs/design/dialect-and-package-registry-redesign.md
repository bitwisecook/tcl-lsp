...
| Q6 family surface composition | **Re-ruled 2026-08-28 (§10.1): `include from`, and jim's surface authored as SpecTcl** — not the ▸ multi-row-availability alternative. P6 authored **no** jim command specs; the family inherits Tcl 8.6 through one `Ancestry` edge, which over-admits threads, coroutines and traces. `include` landed (P2-H); `include from PROVIDER {names…}` is the word this ruling adds; §11 |
...
| Q2 | **Confirmed: the EDA shells move into their `.tclspec`s.** The data move D17 names is now scheduled work, not an option | Closes the "no shipped pack declares an environment" half of D17 once the six `specs/eda_*.tclspec` packs carry `environment` blocks. The naming question (`Environment` vs platform/target/host) was **not answered** and is still open |
...
| Q6 | **`include from`, and jim's surface authored as SpecTcl.** Not the ▸ multi-row-availability alternative | Builds the `include from PROVIDER {names…}` word (§6.2's optional row) and uses it to author jim's 76 commands, killing the duplication and the over-admitting single `Ancestry` edge. Resolves the open Q6 row |
...
| Q8 | **Not understood as put** — restated below and still open | See §10.2 |
...
| O6 | **The assistance-view ordering hint** (Q8's residual). Should the assistance view surface "used before its `package require`" by default, off by default, or not at all? The semantic half is settled and enforced (review B2/H5): the semantic view is position-insensitive and W120 reports a *missing* requirement. **Put to the owner again 2026-08-28 and restated in §10.2; still unanswered.** | One assistance-view default and the diagnostic behind it | An owner ruling on the default, then one diagnostic and its setting |
...
| D17 | **No shipped pack declares an environment** (Q2). The `environment` block, its live registration, its retirement and its detection routing all work and are proved end to end over a workspace pack; not one of the eight bundled `specs/*.tclspec` packs uses one. **Owner ruling 2026-08-28 (Q2): confirmed — the EDA shells move into their `.tclspec`s.** This is now scheduled work rather than an option. The naming question it was asked with — `Environment` vs platform/target/host — was **not** answered and stays open. | The "fully centralised end-state": six EDA catalogue shells are still compiled-in | Adding `environment` blocks to the six `specs/eda_*.tclspec` packs and deleting the compiled shells, with the editor catalogues regenerated from the packs |
