...
(53 of 67 sites are Tk — since closed by `refine`), no `-dialects` scoping
or hosted/keyed spelling
...
lowering). The invocation-refinement descriptor (Q12) has since landed as
`refine NAME { … }`.
...
| `refine NAME { … }` — the invocation-refinement descriptor (**Q12**) — **shipped** | the declarative replacement for `command_forms`/`subcommand_forms`: arity, a literal `selector`, role and option overlays, availability, and the replacement traits / mutator / effects one call shape states, written in the owning scope's own words. The descriptor's native halves (`completion`, `dispatch_dependencies`, `literal_argument_validator`, hook ids) stay Rust-only and are reported rather than thinned. Tk's form sites are the migration test and round-trip |
...
| `include` / surface composition (**Q6**) — **shipped** | `include from SOURCE into TARGET ?-available {WINDOW}? {names…}`: the roster of ancestor command names a *reimplementing* family actually has, with the window on the **target's** own ladder. The row names both ends because a roster is a two-ended fact and the target is a compiled family a pack cannot otherwise claim. Several rows for one pair merge into one roster; a malformed row is dropped whole; an unrostered pair fails **open** (wholesale inheritance, today's behaviour) rather than empty; and only a trusted tier may narrow a compiled family. Jim's roster is the first — and, under Q1, the only planned — user |
...
- **P3 — Tk pilot.** Invocation-refinement descriptor first (Tk's 53
  ...
  **What a surface-to-pack conversion still needs** (the remainder of
  this phase — the invocation-refinement descriptor is no longer on it,
  and Tk's form sites round-trip through `refine`): SpecTcl vocabulary
  for the widget-class and
...
- **P6 — jim rebased.** The jim branch re-lands on the new model: its
  ...
  *hold* jim, so the tax cannot be re-incurred — and then check that
  ...
  - **What P6 deliberately did not do — and Q6 since has.** P6 authored
    no jim command specs, so the inherited Tcl 8.6 surface over-admitted
    what `jim_tcl.txt` says Jim lacks. That was recorded as data
    (`JIM_ABSENT_FROM_THE_INHERITED_SURFACE`) with a test that would fail
    the day the pack landed. **The pack has landed** (Q6, 2026-08-28),
    the constant is retired, and the recorded gap turned out to be an
    understatement: measured rather than cited, the edge was offering a
    `jim` document **seventeen** heads no `jimsh` on the ladder has, not
...
| Q | Outcome |
|---|---|
| Q1 core surface source of truth | **Re-ruled 2026-08-28 (§10.1): the shipped cores stay native Rust**, and the stdlib, tcllib and Tk surfaces with them. AOT is not the endgame, so ruling **R7** is withdrawn and ledger D12 is retired as a non-goal. Jim is the one carve-out (Q6). The `dialect` blocks that landed in P2-H stand — they are how a *pack* declares a family, not a step towards compiling the cores |
...
| Q3 fate of the borderline three | **Confirmed 2026-08-28: `spectcl` and `bpf` stay compiled** as Rust catalogue profiles rather than becoming pack-declared environments, and `f5-bigip` stays off the Tcl availability axis (`f5-bigip` in an `available` row is an error and never translates — U2). U3's finding that neither environment can carry membership as `available` rows, because both surfaces are compiled and neither declares an ambient package, is now the *intended* state rather than a pending gap |
...
| Q22 stub fate (R1) | **Ruled ▸ yes, and enacted (one-vocabulary lane).** `stub_overlay.rs` is deleted. Inline `# tcl-lsp: stub` blocks and `.tcl.stubs` sidecars ingest as provenance-tagged `SurfaceDeclaration`s (`tcl_registry::model::declaration`) — provider `Provider::Document`, the whole `VersionAxisId::document()` axis, `Provenance::Document` for a buffer and `Provenance::WorkspaceUntrusted` for a sidecar — and every consumer asks one door, `DocumentCommandSurface` |
...
### 10.1 The 2026-08-28 re-ruling (Q1–Q14)

The owner re-read Q1–Q14 and ruled on all fourteen. Where a ruling repeats
the ▸ recommendation or the outcome above, it **confirms** it and the row
stands; where it differs, **this section is the current ruling** and the row
above is history. Q15–Q25 were not re-put and are unchanged.

| Q | 2026-08-28 ruling | Effect |
|---|---|---|
| Q1 | **Core surfaces stay native Rust** — `commands/{tcl,irules}` and the stdlib, tcllib and Tk surfaces with them. Not "model first, AOT later": AOT is **not** the endgame. | Retires ruling **R7** and ledger **D12** as non-goals. `tcl spec build --emit rust` is not built and will not be. Jim is the one carve-out — see Q6 |
| Q2 | **Confirmed: the EDA shells move into their `.tclspec`s**, and **`Environment` is the right name** (platform/target/host considered and rejected, 2026-08-28). The data move D17 names is now scheduled work, not an option | Closes the "no shipped pack declares an environment" half of D17 once the six `specs/eda_*.tclspec` packs carry `environment` blocks |
...
The original question asked whether a `package require` has to appear
*before* the first use of a command it provides for that command to count as
available.

The **semantic** half is settled and enforced (review B2/H5): the semantic
view is position-insensitive — a `package require` anywhere in the file makes
its commands available for the whole file — and W120 reports a command used
with no `package require` at all. That is not in question.

What is still open is one **assistance-view** default (ledger O6). When a
file does this:

```tcl
puts [csv::join {a b}]      ;# used here …
package require csv         ;# … required here
```

W120 does not fire, because the requirement is present. The question is
whether the assistance view should nonetheless surface a *hint* — "used
before its `package require`" — **by default**, off by default, or not at
all.

It matters because the two readings are both defensible. Tcl semantics say
the ordering is irrelevant at parse time and only matters at the moment of
the call, so a file that requires at the bottom and never calls before it
runs is correct; but for a human reading top-down the ordering is a real
smell, and most style guides require the `package require` first.

Nothing is built either way. This is a UX default, not a correctness
question, and it costs one diagnostic and its setting whichever way it goes.
...
| O6 | ~~**The assistance-view ordering hint**~~ **RESOLVED (owner, 2026-08-28: on by default) and LANDED.** `H301` — "used above the `package require` that provides it" — is a Hint-severity, default-on assistance diagnostic, disjoint from W120 by construction (W120 fires when the requirement is absent, H301 when it is merely late). One hint per package, not per command; no code fix, because the edit is a move and a code action carries one replacement. Silent for ambient packages, a file that `package provide`s the package, a requirement that is only conditional, a command the document declares itself, and the two gates W120 already takes. KCS article and catalogue rows shipped with it. | Nothing | Nothing — this is closed |
...
| O9 | **Which tier is E-R2-*untrusted*.** ~~Half resolved~~ **— the security half is closed (R12, 2026-08-28).** §6.4's monotone-merge invariant **I6** was never implemented, and the gap was measured, not inferred: a four-line `Tier::Workspace` pack declaring `command exec -override { arity 1.. }` loaded with no error and produced an `exec` carrying neither `TAINT_SINK` nor `TAINT_SOURCE`, because an override replaces the shipped spec wholesale. `tcl-registry::security_floor` now enforces I6 on **every** override from **every** tier — set-valued security facts union, single-valued ones keep the shipped value — and `tcl-spectcl/tests/i6_security_floor.rs` is that probe asserting the opposite. The floor is deliberately not tier-keyed: §6.4 keys its untrusted class on the editor's Workspace Trust state, which nothing on the discovery path is told, so a tier-keyed rule would protect nothing today, and a security fact a *trusted* pack may quietly drop is not much of a security fact. **What remains open** is only the plumbing: `Provenance::WorkspaceUntrusted` is still unreachable from pack discovery, so the editor's trust state cannot gate wholesale environment override. | The consent surface of O8, and any rule that must distinguish a trusted workspace from an untrusted one | Plumbing the LSP client's Workspace Trust state to `discovery`, and deciding the behaviour for clients that do not report it |
...
| D2 | **The invocation-refinement descriptor** (Q12) — **done**: `refine NAME { … }` at command and subcommand scope, the declarative replacement for `command_forms` / `subcommand_forms`. Per-form arity, literal selector, role and option overlays, availability, and replacement traits / mutator / effects, read by the owning scope's own readers. The native halves stay Rust-only and are reported rather than thinned. | — | — |
...
| D9 | **Ledger C8 — done (one-vocabulary lane); ledger C6's tail — open.** The three vocabularies collapsed onto `state_transition.rs`'s `CommandBindingTransition`: `state_transition::command_binding` holds the three stock descriptors (lifted out of `proc_.rs` / `rename_.rs` / `interp.rs`, with the `interp alias` shape guard `tcl_compiler::alias` used to carry), every shipped mutator names one of them and no longer stamps `command_table_effect` beside it, `CommandTableEffect` survives **only** as the pack-authoring selector resolved by `CommandTableEffect::transitions` (a pack cannot supply a Rust resolver), `CommandRegistry::command_table_effect` is deleted in favour of the one door `command_binding_transitions`, and `alias.rs`'s `detect_rename` / `detect_interp_alias` / `detect_interp_alias_delete` / `is_interp_alias_shape` are deleted with it. `LegacyEffectBridge::command_table_effects` — a fourth place the same fact was said — became a `command_table_mutation` bool fed from the resolved transitions. **What remains (C6's tail)** is the analyser's `command_aliases` / `renamed_commands` / `deleted_commands` / offset maps: they are now *populated from* the one vocabulary (so they are indexes, not a parallel derivation) but are still `AnalysisResult`-shaped rather than `state_transition.rs`-shaped values, and `indirection.rs` walks them in that form | D8 (the realm map wants one vocabulary) — no longer blocked; the "no parallel binding tables" half of ruling R10 is gated | Re-typing the tables reaches ~30 files across `tcl-lsp-core`'s navigation providers, which is a lane of its own |
