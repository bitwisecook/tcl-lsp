...
| Q12 | **Confirmed: the declarative descriptor** | Ledger D2 stays the largest unblocking item |
...
| D2 | **The invocation-refinement descriptor** (Q12) — **confirmed by the owner 2026-08-28**: the declarative whole-descriptor replacement for `command_forms` / `subcommand_forms`, not closed native `CommandForm` ids. Per-form word patterns, arity windows and role overlays as data. | Any surface-to-pack conversion; Tk's 53 `subcommand_forms` sites are the measured blocker | Designing and building the descriptor, then porting the form sites onto it |
