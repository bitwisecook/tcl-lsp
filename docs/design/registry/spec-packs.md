...
Beside it, in `rust/tcl-spectcl/tests/`: `golden_packs.rs` holds all 24
shipped packs to checked-in snapshots (regenerated by `cargo xtask
pack-goldens`), so a loader change cannot silently alter what a pack
means; `eval_loader.rs` holds the static fast path and the interpreter
route byte-identical; and `pack_is_real_tcl.rs` sources every shipped
pack with a stock `tclsh9.0` so a pack is always parseable Tcl.
