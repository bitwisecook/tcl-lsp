...
> of the decision. Migration guarantees stand and were **proved**: the
...
> accept them as data). The **CST loader has since been deleted** — the
> `one-loader` lane, centralisation ledger row L1 — because keeping a
> proved-redundant second implementation of "load a pack" is exactly the
> duplication this programme forbids. The proof is kept as the
> golden-snapshot gate over the same 24 packs
> (`tcl-spectcl/tests/golden_packs.rs`, regenerated by
