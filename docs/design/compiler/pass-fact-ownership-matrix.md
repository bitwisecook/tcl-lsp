...
| `value_transfer.rs` (the registry's `value_transfer` module owns the declarations) | one invocation's value transfer: the registry's structural plan, per-domain transfer, and exact evaluation over lattice-proved operands, applied to a definition by `sccp.rs`; the per-module `AnalysisContextKey` every answer is keyed under | `sccp.rs` (the only consumer today; the optimiser and diagnostics read the lattice it feeds), the existence fold's unbind set | `LatticeDriver` (crate-private), `AnalysisContextKey`, `tcl_registry::value_transfer::CommandSemantics` |
