...
| `value_transfer.rs` (the registry's `value_transfer` module owns the declarations and the direct routes: the cell updates behind `incr`, `append` and `lappend`, the cell write behind `set`, the keyed updates of `dict` and their `::tcl::dict::` spellings, `string range`, `list`, `llength` and `string length`; `format` stays a transitional compiler route until slice 3) | one invocation's value transfer: the registry's structural plan, per-domain transfer, and exact evaluation over lattice-proved operands; one evaluated write applied to the call's definition; each statement's route and answer (`SccpResult::explanations`); the per-module `AnalysisContextKey` every answer is keyed under, whose command trust is the lattice's `ObservedBindings` stance and a rewrite's `WholeModule` one. Unavailable is `Overdefined`, with the `DeclineReason` recorded beside it | `sccp.rs`, which feeds the lattice O100, O102 and O109 / O126 read; `optimiser/chain_fold.rs` (O104, O130), the tail fold, `static_loops.rs`, `intervals.rs` (the descriptor's `IntegerAdd`) and W231 (`analyser/bounds_checks.rs`), each reading the resolved cell update rather than a command name; the Explorer's `sccp` view; `tcl-lsp-db`'s memoised `function_lattice`, under the same context and stance; the existence fold's unbind set | `LatticeDriver` (crate-private), `AnalysisContextKey`, `tcl_registry::value_transfer::CommandSemantics` |
