...
| `value_transfer.rs` (the registry's `value_transfer` module owns the declarations and the direct routes: the cell updates behind `incr`, `append` and `lappend`, the cell write behind `set`, the keyed updates of `dict` and their `::tcl::dict::` spellings, `string range`, `list`, `llength`, `string length` and, since slice 3, `format`; the expression route assembles `expr`'s argument words — `ExpressionRoute::assemble` — and runs them through the shared engine, `tcl_expr_eval::ExprServices`, over the same lattice inputs; since slice 4, a pack's own declared implementation runs through the same driver — `DeclaredSemantics::evaluate` in `tcl-registry`'s `pack_hooks`, over the bounded host behind `tcl_engine_api::Engine`) | one invocation's value transfer: the registry's structural plan, per-domain transfer, and exact evaluation over lattice-proved operands; one evaluated write applied to the call's definition; each statement's route and answer (`SccpResult::explanations`) and the run's per-family entry counts (`SccpResult::route_tally`); the per-module `AnalysisContextKey` every answer is keyed under, whose command trust is the lattice's `ObservedBindings` stance and a rewrite's `WholeModule` one, and whose `evaluator_revision` (slice 4, D94) names the declared-implementation evaluators the run served with, so a healthy and a host-absent worker never share a declared implementation's answer. Unavailable is `Overdefined`, with the `DeclineReason` recorded beside it. Since slice 5: each definition's `FoldedType` (`SccpResult::folded_types`) — intrep, shape and representation evidence beside the value, joined at a φ and forgotten at a barrier; `regexp`, `regsub`, `scan`, `binary scan`, `lassign` and `array set` apply their declared `Write` / `Preserve` / `Unbind` / `MayWrite` / `WriteElement` outcomes per place in execution order, so a no-match preserves and a repeated target composes; `dict with`, `dict update` and a loop header answer a structural `PlanAnswer::Body` / `Iterate` plan; one `TemplatePlanRecord { span, command, switches, plan }` per executable `subst` call (`SccpResult::template_plans`, `rebase_function_unit`-shifted), the switch spellings W102's narrowing reads; each definition's `Preserve` version before its statement (`SccpResult::preserved`), which W210 and W213 read through instead of a private no-match prover; the solver's branch facts each carry a `BranchFactKind` (`Applied` / `Proven` / `Selected`) so I230 reads the stored kind rather than re-proving it | `sccp.rs`, which feeds the lattice O100, O102 and O109 / O126 read, and whose `evaluate_branch` decides a condition once every member of its one finite read agrees; `optimiser/chain_fold.rs` (O104, O130), the tail fold, `static_loops.rs`, `intervals.rs` (the descriptor's `IntegerAdd`), `optimiser/helpers/expr_simplify.rs` (O110's regrouping, type-guarded since slice 3) and W231 (`analyser/bounds_checks.rs`), each reading the resolved cell update or expression answer rather than a command name; the Explorer's `sccp` view; `tcl-lsp-db`'s memoised `function_lattice`, under the same context and stance, re-keyed by `tcl_lsp_db::EvaluatorEpoch` on a plan publish or a quarantine (slice 4, D104); since slice 5, `analyser/diagnostics/proven.rs`'s per-function pass, which re-runs the literal-only checks (W121, W127, W137, W138, W145–W147, W152, W200, W202, W230, W232, W303) and W102's narrowing over `value_transfer::proven_word_value(fu, StatementId, word, LexerConfig)` — the exact value a word proves at its statement, addressed by `FunctionUnit::word_at(span)` for a consumer holding a span rather than a statement — and `hover.rs` / `inlay_hints.rs` read the same query for a computed pattern or format argument, while the pattern/format semantic-token families stay literal-only (`a_computed_format_word_falls_back_to_its_plain_classification`) | `LatticeDriver` (crate-private), `AnalysisContextKey`, `tcl_registry::value_transfer::CommandSemantics`, `ExpressionSource`, `RouteTally`, `FoldedType`, `StoreOutcome`, `TemplatePlanRecord`, `BranchFactKind` |
| `sccp.rs` (the existence rung, since slice 8) | a flow-sensitive bound / unbound / may-bound fact per place and per SSA version (`SccpResult::existence`, `existence_reads`, `existence_exits`), fed by storage outcomes, the entry states, the join, the absent-cell release rule and the guard's edge refinement in the existence domain ([value-transfers.md](value-transfers.md) § *Existence*) | W210, W211, W213, W214 (`analyser/diagnostics/dataflow.rs`), O108, O109 (`optimiser/elimination.rs`), I230, O101 (`compiler_checks.rs`, `optimiser/branch_folding.rs`), S100 (`shimmer/phi.rs`); a fast-tier request and a function over the complexity ceiling read `Unavailable`, which is neither bound nor unbound | `SccpResult::existence_at`, `FunctionUnit::existence`, `FunctionUnit::tier`, `EdgeRefinement`, `tcl_registry::value_transfer::Existence` |
| `analyser/diagnostics/usage.rs` (`emit_w100_unbraced_expr`) | W100, the unbraced-expression fact: one finding per EXPR-role argument (`expr`, `if`, `while`, `for`, a nested `[…]`) that is not a braced word, at the expression word's own span | the diagnostic-policy lane's `DP8.1` (`FACT_CODES` keeps W100 computed under every policy) and `DP8.2`'s `brace_expr_hints`, which emits O111 at each produced W100's span in place of the former `with_brace_expr_hints` walk (slice 5, D23: no second representation of the fact) | `emit_w100_unbraced_expr`, `push_w100` |
