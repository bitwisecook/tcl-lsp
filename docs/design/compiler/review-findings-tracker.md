...
| D5-SH-EXPR | HIGH | `_find_expr_shimmers` only analyses `IRAssignExpr`, misses standalone `expr`, `if {…}`, `while {…}`, `for {…}` expr contexts.  All four are real expr lex-promotion sites in tclsh. | ✅ FIXED | `_find_expr_shimmers` extended to walk three expr-context sites: IRAssignExpr bodies (unchanged), IRExprEval bodies (standalone `expr {...}`), and `CFGBranch.condition` on each block's terminator (covers if/while/for/break-condition).  Terminator's SSA uses come from `ssa_block.exit_versions`.  Per-block de-duplication on `(range, var)`. · [FP-SH-07](FP.md#fp-sh-07) |
...
| D5-T104 | MEDIUM | T104 ignores `taint_network_sink_args` argument positions in the registry — fires on any tainted var in the statement (e.g. `http::geturl URL -headers $hdr` fires for `$hdr`, but the URL is positional[0], `$hdr` is an option value). | ✅ FIXED | `TaintSinkInfo.is_network_sink: bool` replaced with `network_sink_args: tuple[int, ...] | None` (None=not a sink, ()=whole-statement scan, non-empty=positional indexes).  `classify_taint_sinks` propagates the tuple; `_find_taint_sinks` adds a `t104_addr_idxs` per-statement cache and position-filters T104 emissions, mirroring the existing T101/T102 filters. · [FP-TNT-05](FP.md#fp-tnt-05) |
...
|---|---|---|---|
| SF-1 | Registry data coverage: add `return_type=TclType.STRING` / `OBJECT` to specific tcllib/Tk commands so D3-P5 / D4-F6 partial closures catch unregistered external factories (`::pkg::plain` style). | ✅ FIXED (variable-as-command tier) | `struct::set` subcommands got `return_type=BOOLEAN/INT/LIST/STRING` (~17 firings).  The census's dominant tier — 99% (4714 / 4755) **variable-as-command** firings — is closed by the existing SCCP CONST/CONSTSET lattice + D3-P2 call-site param seeding (a literal command name flows through the same constant lattice as any other string; no separate `known_command_names` fact is needed).  Hardened with a per-SSA-version refinement in `_diag_var_command.py` (reads the value at the dispatch's exact use-version, removing the reassign-before-dispatch false positive) and locked in by `tests/test_fp_obj_var_as_cmd.py` (9 TP/TN pairs).  Residual registry-data coverage for specific tcllib/Tk factories stays incremental data work (folds into D1-11), and the unregistered-external **cmd-sub-as-command** case (D3-P5/D4-F6) is unknown-return-bound, not an inference gap. |
| SF-2 | Wire `ClassDef.method_purity` into D2-O126-FU's `interproc_pure` set so pure-method RHS (`set unused [my pure_method ...]`) can be safely folded. | ✅ FIXED | Method-body lowering landed: `lowering.py` lifts `oo::class create` / `oo::define` method bodies to `IRMethodDef` entries; `compile_source` lowers those to per-method `FunctionUnit`s in `CompilationUnit.methods`; `analyse_interprocedural_ir` summarises them into `InterproceduralAnalysis.methods` (conservative purity — pure iff no local side effect and every proc callee pure; `my`/`next` self-dispatch ⇒ unknown call ⇒ impure).  The optimiser iterates method bodies with the owning class as `enclosing_class`, activating the SF-2 gate: `set unused [my <pure-method>]` folds to O126, impure-method RHS preserved.  Bytecode identity unchanged (methods are analysis-only; codegen still sees the class-body `IRBarrier`).  Regression: `tests/test_fp_opt.py::test_FP_OPT_12_tcloo_*`. · [FP-OPT-12](FP.md#fp-opt-12) |
