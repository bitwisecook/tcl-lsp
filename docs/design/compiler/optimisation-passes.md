...
| O107–O109, O112, and O126 | SCCP reachability, SSA def-use (a nested cell update's read is a use of the store it reads, and a call observes the names its callee's global-write summary holds), place/alias facts, and effect tests; O112 decides a condition on the shared expression route under the rewrite's whole-module trust, so it never decides past the target's tower or over a math function the module rebinds | Inputs overlap with conservative AOT reasoning, but DCE, dead-store, and structure-elimination results are not AOT evidence. Current AOT does not use an O107 reachability decision to erase a region or an O109 result to erase storage. |
