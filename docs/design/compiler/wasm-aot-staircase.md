...
| **S4** | Inlining of small leaf procs | [s4](wasm-aot-staircase-s4.md) | landed (S4.1 + S4.2 v0/v1/v2) — S4.1 catalogue (e7480b4c), S4.2 v0 empty-body splice (2ce0fbe7), pipeline integration (d97e1c17), S4.2 v1 single-call wrapper splice (5c21fd76) + frame-observation gate (9ff932fa), S4.2 v2 multi-statement wrapper splice (bc5acddd); S4.3 (profile-guided WASM-level inlining) closed as out-of-scope — needs profile infrastructure, no concrete plan in spec |
| **S5** | SSA-driven codegen optimisations | [s5](wasm-aot-staircase-s5.md) | landed — S5.1 first-write (acb851f4), S5.2 alias-skip (0cac52ce), S5.3 LICM for IRFor (099920da) + IRForeach (f111b3c3), S5.4 DCE adapter for pure_leaf procs (0d531219); SCCP/GVN sub-adapters in S5.4 closed as future work — codegen already reaches `i32.const` for literal ints via S6.4, and full SCCP/GVN need multi-emitter coordination that exceeds the adapter scope |
