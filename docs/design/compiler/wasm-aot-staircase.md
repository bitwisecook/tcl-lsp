...
| **S4** | Inlining of small leaf procs | [s4](wasm-aot-staircase-s4.md) | landed (S4.1 + S4.2 v0/v1/v2/v3 + dead-proc DCE + full eligibility) — S4.1 catalogue (e7480b4c), S4.2 v0 empty-body splice (2ce0fbe7), pipeline integration (d97e1c17), v1 single-call wrapper (5c21fd76) + frame-observation gate (9ff932fa), v2 multi-statement wrapper (bc5acddd), v3 parameterised inlining + α-renaming + dead-proc DCE (b2cefa5b), v3 eligibility completion — defaults / variadic / nested control flow / trailing-return at terminal position (e0c1eddd); S4.3 (profile-guided WASM-level inlining) closed as out-of-scope — needs profile infrastructure |
