...
| TclOO construct coverage | _analyser/_oo.py (full) | analyser/oo.rs, handlers.rs | ✅ | Manufacturer layouts and visibility are registry data. C Tcl rejects ordinary `oo::class new` and `oo::class createWithNamespace` dispatch, so the Rust analyser deliberately records neither; derived Tcl 9 metaclasses retain exported `new`. `initialise` and `property -get`/`-set` bodies are walked — see `oo_class_create_with_namespace_is_not_exported`, `oo_initialise_body_is_walked`, and `oo_property_accessor_bodies_are_walked` in `oo.rs`. |
