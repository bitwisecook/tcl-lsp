...
| Per-item walk Phase D (issue #1123) — the rotted corpus gate re-zeroed. **Synthetic identities rebase-stable**: `@dynns@`/`@dynclass@`/`@autoname@` names are minted through `mint_synthetic_offset_name` (recorded in `minted_synthetic_names`), and the graft rewrites exactly those names — wherever a string embeds them — by the body's offset delta (`naming::rebase_synthetic_offset_names`), so the memoised fragment stays offset-invariant while matching the whole-file walk's absolute-offset names. **Instance-creation DFS parity**: shell *and* body creations are captured with their site offset and replayed post-graft in source order against only the classes whose definition precedes the site. **More whole-file facts deferred/carried**: W103/W300 `$var` classification deferred to the tail (`pending_var_literal_checks`); widget-dispatch sites, M7 `$cmd` const-dispatch sites, `$class` instance sites, scoped command regions/sibling defs, and the analyser-side alias/rename/deletion tables all carried in `BodyFragment` and rebased at graft; ensemble `subcommand → target` maps visible to a body attached to `DeferredBody` (sorted + body-filtered, keyed into `ItemBodyKey`) — the G8 trust-snapshot pattern. `proc_declaration_sites`/`class_body_spans`/`scoped_command_regions` added to the canonical order tail; the qualified-read replay no longer double-records `qualified_var_refs`. A body that `source`s a `$var` path falls back (walk-time const chain not reconstructable in isolation), the enclosing-context gate now covers method bodies too, and the isolated pass's structural param rebind records no phantom qualified occurrence. | **shipped** (corpus gate re-zeroed: 0/895 divergences; a non-`#[ignore]`d `samples/**/*.tcl` slice now runs in every `cargo test`, so the gate cannot rot silently again) |
