...
| `tcl_runtime.zig` (root) | 1 | export-aggregation root | `runtime/rust/` lib root | **partial — real link runs** | `runtime/rust` **compiles and links for `wasm32-unknown-unknown`** (the `BrowserHost` capability host stands in for the absent WASI host; the native-only features — coroutines (native stack-swap + threads), the ARE regex FFI, the libtommath bignum tower — are cfg-gated off behind `BrowserHost`/`have_regex`/`have_tommath`, with the wasm-dead native machinery under a `cfg_attr(target_arch = "wasm32", allow(dead_code))`; native is unaffected and `clippy -D warnings` stays clean). It now **exports the codegen-import ABI** (`codegen_abi.rs`: `tcl_obj_new_string`/`tcl_eval`/`tcl_obj_release`/`tcl_expr_bool` + `tcl_runtime_set_current_interp`) the WASM emitter calls, and an **emitted module runs against it end-to-end** (`tcl-compiler/tests/wasm_real_link.rs`): the emitted `::top` evaluates `set x 42` in the real interp and the side effect reads back as `42`, over one shared linear memory. The emitted constant pool is relocated into a reserved gap (`[0x100000, 0x200000)`, runtime built with `--global-base=0x200000`) so it does not collide with the runtime's shadow stack. Remaining: the rest of the `tcl_*`/`obj_*` symbol set + dropping the per-program reserved gap for true PIC (`__memory_base`) |
