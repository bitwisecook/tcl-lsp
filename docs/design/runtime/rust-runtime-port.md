...
| `tcl_runtime.zig` (root) | 1 | export-aggregation root | `runtime/rust/` lib root | **partial — wasm32 build links** | `runtime/rust` **compiles and links for `wasm32-unknown-unknown`** (the `BrowserHost` capability host stands in for the absent WASI host; the native-only features — coroutines (native stack-swap + threads), the ARE regex FFI, the libtommath bignum tower — are cfg-gated off behind `BrowserHost`/`have_regex`/`have_tommath`, with the wasm-dead native machinery under a `cfg_attr(target_arch = "wasm32", allow(dead_code))`; native is unaffected and `clippy -D warnings` stays clean). Remaining: export the `tcl_*`/`obj_*` symbol set the codegen imports (Track A) |
