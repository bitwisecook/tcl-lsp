...
| `cmds/` builtins | 34 (8367) | all builtin commands | `runtime/rust/` cmds | **partial** (T1.4/T1.5/T1.6 + M1–M3) | `make runtime-rust-test` — `set`/`incr`(tower)/`return`/`unset`(`-nocomplain`) + `expr` + `subst` + list cmds + `dict` ensemble + `append` + `string` ensemble (incl. `match`/`map`/`is`) + `rename`/`interp alias`/`namespace` (+ `ensemble`/`code`/`origin`) + `global`/`variable`/`upvar` + `::tcl::mathfunc/mathop::*` + `proc`/control-flow/`puts` + `catch`/`error`/`try`/`throw` + `info`(incl. `level N`/`complete`) + `array`/`switch`/`package` + `source`/`file`/`glob`/channels + `trace` (variable) + **`regexp`/`regsub`** (real Tcl ARE engine, `have_regex`); **drives the unmodified Tcl 9 `init.tcl` + `tcltest.tcl` to `package require tcltest` 2.5.10 and runs real `test` bodies** (M3); per-command parity + tcltest sweep as more land |
