...
- Four new ``tcl_ns.zig`` helpers: ``hidden_put`` /
  ...
  ``hidden_table_cap``.
- Rewritten ``tcl_cmd_info.zig`` with a real walker + pattern
...
- Removed the compiled-proc rename exception from
  ``tcl_rename.zig``.
- New built-in branches in ``tcl_interp.zig``:
