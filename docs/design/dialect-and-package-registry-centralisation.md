...
For `.tclspec` sources "one loader" became literally true with ledger row
**L1**: design E's evaluation loader (`tcl_spectcl::evaluate_pack`, cached
behind `tcl_spectcl::evaluate_pack_cached`) is the only door, and the CST
front end it was proved byte-identical to is deleted.
...
| C12 | `RuntimeExprSurface::for_tcl_version` and the duplicated operator/word tables; free-function `binary_bp` | `ExprGrammar` per core profile (precedence, symbolic operators, mathfunc sets, arity, substitution) with `for_profile`-only derivation | P1 *(partial: the **data** side is complete for every modelled family — P6 filled the Jim ladder from the upstream `OPRINIT` tables, so `ExprGrammar` now carries the whole binary binding-power table (release-keyed: Jim moved `**` at 0.77), the release each word and symbolic operator arrives at, and the mathfunc set with its per-function build gate. What remains is the **consumer** move: `binary_bp` in `rust/tcl-syntax/src/expr/parser.rs` is still a free function keyed on operator text with no dialect parameter, and `tcl-syntax/src/expr/mathfunc.rs` still keys on `TclVersion` floors — which is exactly the shape that would offer `min`/`max`/`entier`/`bool`/`isqrt` under Jim, all five of which Jim has never had)* |
...
| L1 | The **CST pack-loader front end** — `tcl_spectcl::loader::{load_pack, load_pack_with, load_command, expand_includes, report_extra_speclib_blocks}` and `cache::load_pack_cached` — a second live implementation of "load a pack", still the LSP's production path while the studio, CLI and MCP server used design E's evaluation loader | `tcl_spectcl::loader::evaluate_pack` (one loader), behind the one cache door `tcl_spectcl::cache::evaluate_pack_cached` | **done (`one-loader` lane)** *(§1.1's "one loader" is now literal for `.tclspec`. The row readers — `apply_pack_stmt`, `apply_command_stmt`, `apply_subcommand_stmt`, `command_from_parts`, `subcommand_from_parts`, `PackTables` and the property readers under them — are deliberately **kept**: they are the one `SpecTcl` vocabulary, which the evaluation loader replays every captured registration through, not a second front end. The two caching layers reconciled to one identity, `EvalSnapshotKey` (content × vocabulary × loader-eval version × tier) stamped with the build, serving an in-memory `Pack` tier and the on-disk segmentation tier; `cache::LOADER_BUILD` is gone. The equivalence proof survives as the golden-snapshot gate (§5) plus the static-fast-path gate. Every deleted spelling is in the retired-api gate. Measured on the eight bundled packs, `eda_xilinx` (788 commands, 20,887 lines) included: cold load 961 ms → 952 ms, warm 165 ms → 190 ms, uncached 1,097 ms → 891 ms — the evaluation loader is **faster** uncached once the static fast path covers the file's own top level, which this lane widened it to do. Full record: `docs/design/lanes/one-loader.md`)* |
...
| Golden pack snapshots (`cargo test -p tcl-spectcl --test golden_packs`, regenerated by `cargo xtask pack-goldens`) | a loader change cannot silently alter what a shipped `.tclspec` means: all 24 shipped packs load to their checked-in snapshots or the diff is written down. Replaced the two-loader byte-identity gate when the CST front end was deleted (row L1) | I10 |
| Static fast-path gate (`tcl-spectcl/tests/eval_loader.rs`) | the one loader's shortcut past the interpreter is an *optimisation*: all 24 shipped packs load byte-identically with `static_fast_path` on and off | I10 |
