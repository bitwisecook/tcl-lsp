...
**Two decode tiers, one verdict.** The byte tier (`tcl diag`, the workspace
scan, `cargo xtask fp-sweep`) reads the file and can name the byte offset and
malformation class; the text tier (an editor `didOpen`, where the bytes were
consumed by the editor's own decoder) reports from the residue. Both raise
the same codes on the same file — the difference is precision, in one
direction only. The single exception is stated in
`tcl_lsp_core::source_decode`'s module docs: with the bytes in hand the
decoder can *prove* a `U+FFFD` is genuine content and stay silent, which the
text tier cannot.
