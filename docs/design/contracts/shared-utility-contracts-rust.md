...
| binary field grammar | `rust/tcl-cmd-core/src/binary.rs` | `specifiers`; `Specifier`; `is_specifier`; `signedness_available` | field letters per release; the TIP 275 unsigned suffix (`u` only, after any field letter) per resolved release | none |
