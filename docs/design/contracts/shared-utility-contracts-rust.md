...
| command vocabulary outside the registry | `rust/tcl-registry/src/registry.rs`; `rust/tcl-registry/src/definer.rs`; `rust/tcl-registry/src/traits.rs`; `rust/tcl-registry/src/special_vars.rs` | `CommandRegistry`; `DefinitionBodyGrammar`; `CLAUSE_KEYWORDS_WITHOUT_COMMAND_SPEC`; `CLAUSE_NOISE_KEYWORDS`; `SPECIAL_VARS` | per dialect surface: the vocabulary is every word the registry declares across every loadable dialect and the shipped `.tclspec` packs | `xtask-registry-axes` |
