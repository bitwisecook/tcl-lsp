...
- `display_namespace` and `display_command` are output projections only.
  ...
  explicit parent/child `NsId` edges, and TclOO class/object/provider state
  retains a stable object token across command rename and deletion callbacks.
