...
1. Put reusable Tcl language behaviour in `core/` (no `server/` imports).
2. Put protocol/server lifecycle/feature wiring in `server/`.
...
- Circular dependencies when `core/` imports `server/`.
- Duplicate logic when feature-specific behaviour is implemented in multiple `server/features/*` files instead of a shared `core` module.
- Regressions in VM/explorer behaviour when compiler logic is incorrectly added to `server/` only.
...
- [Pipeline layering](../../../docs/design/contracts/pipeline-lsp-first.md)
- [Compiler KCS index](../../../docs/design/README.md)
