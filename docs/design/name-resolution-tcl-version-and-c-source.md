...
| **9.0 `::tcl::` reorg** (removed `unsupported::inject`; added `tcl::process`, `tcl::idna::*`; `isunordered`/`tm::path` fixes) | 9.0 | `changes.md:85, :103, :209, :261, :264` | **PARTIAL** — `tcl::process`/`tcl::idna`/`tcl::zipfs`/`tcl::build-info`/`tcl::unsupported::corotype`/`tcl::tm::path`/`tcl::tm::roots` are now dialect-gated specs (session update below); `tcl::prefix`/`tcl::clock` remain unsurfaced | **RESOLVED for the surfaced subset** — see the Stage 15.5 correction in `name-resolution-fix-plan.md` (the earlier "already modelled" claim for build-info/corotype was itself wrong: both carried `dialects: None`; `tm::path`/`tm::roots` were never covered at all). `tcl::prefix`/`tcl::clock` are still genuinely unmodelled — low-priority residual. |
...
**Other `::tcl::` sub-namespaces** — `::tcl::tm` (8.5), `::tcl::prefix` (8.5), `::tcl::clock`, `::tcl::unsupported::*`; 9.0 removed `unsupported::inject`, added `tcl::process` and `tcl::idna::*`, fixed `mathfunc::isunordered`/`tm::path` (`tcl9.0.4/changes.md:85, :103, :209, :261, :264`). **Rust — now PARTIALLY MODELLED** (session update, see `name-resolution-fix-plan.md` Stage 15.5): `tcl::tm::path`/`tcl::tm::roots` (`TCL85_PLUS`), `tcl::process`/`tcl::zipfs`/`tcl::idna` (already `TCL90_PLUS`), `tcl::build-info` and `::tcl::unsupported::corotype` (were `dialects: None` despite an earlier note in this document and in the fix-plan claiming otherwise; corrected to `TCL90_PLUS` and `TCL86_PLUS` respectively — `corotype` ships with coroutines in 8.6, empirically confirmed against a real `tclsh 8.6.14`, not 9.0-only). `tcl::prefix` and `tcl::clock`'s Tcl-scripted ensemble-implementation helpers remain unsurfaced — low-priority residual, and arguably correctly so (they are ensemble-backing implementation detail, not part of the documented public `::tcl::` surface the way mathop/mathfunc/process/idna/zipfs/tm/build-info are).
Real Tcl gives `::tcl` **no special read-only/rename protection** at the language level (empirically verified against `tclsh 8.6.14`: `rename ::tcl::mathop::+ ::foo`, `namespace delete ::tcl::mathop`, and `namespace delete ::tcl` all succeed silently — unlike the *global* namespace `::`, which cannot be deleted). It is merely unusually load-bearing: it hosts `::tcl::UnknownPending` (the `unknown` auto-load recursion guard) and the Tcl-scripted implementations backing several C ensembles, so deleting it cascades into breaking most of the interpreter — a footgun Tcl allows, not a locked door the LSP needs to model.
...
| D7 | **expr math-function set dialect-blind** — 8.5+/9.x functions accepted/folded under 8.4 | CONFIRMED | medium | `tcl8.4.20/tclExecute.c:427, 3934-3949` (no min/max/is*); 8.5 `init.tcl:79,95`; 8.6 legacy `tclExecute.c:3095-3111` | `mathfunc.rs:69-82` (no dialect param); `tcl_expr_eval.rs:299-332`; VM `expr.rs:686-698`; unmodelled in registry (mathop is gated) | **NEW** (expr-function dialect gating) — *partially stale*: the analyser's `emit_expr_function_dialect_diagnostics` (`analyser/diagnostics/validity.rs`) already draws W002 for a too-new function under a pinned dialect today (verified: `tcl8.4` + `min(1,2)` → W002 citing the real floor); whether the const-folder/codegen still silently applies the fold alongside that diagnostic is unverified and may be a narrower residual. |
| D8 | **expr-function ↔ user-proc link missing** — `proc ::tcl::mathfunc::f` invisible to `f(...)`; no def/refs/arity, flagged unused | CONFIRMED | medium | `tcl8.6.16/tclCompExpr.c:2276`; `tclExecute.c:3092` | codegen faithful `expressions.rs:325`; static: `mathfunc.rs` closed table; `commands.rs:2225` only `TokenType::Cmd`; `usage.rs:1758`. Admitted `command-resolution.md:184-188` | **NEW** — *user-proc linking itself works* (`rust/tcl-lsp-server/tests/e2e/name_resolution.rs`'s `mathfunc_expr` suite, issue #958), but a **separate, more severe, dialect-*independent*** symptom of the same "mathfunc unmodelled in the registry" root cause was found verifying this: an *unshadowed* built-in (`sin`, `abs`, `round`, …) draws a spurious W123 in *every* dialect including 9.0, since the W123 check only recognises an expr-function call via a coincidental proc-tail-name match. Reported as [issue #968](https://github.com/bitwisecook/tcl-lsp/issues/968); fixed on the not-yet-merged sibling branch `claude/awesome-curie-rxyttd` (commit `4b38405`) by teaching the W123 check to also consult `tcl_syntax::expr::mathfunc`'s existing version table for the settled qualified name — reusing D7's table rather than adding registry specs. |
...
| N4 | **9.0 `::tcl::` reorg deltas unmodelled** (`unsupported::inject`, `tcl::process`, `tcl::idna`, `isunordered`/`tm::path`) | PLAUSIBLE | low | C `tcl9.0.4/changes.md:85,103,209,261,264`; Rust: only mathop surfaces | **RESOLVED** for `tcl::process`/`tcl::idna`/`tcl::zipfs`/`tcl::build-info`/`tcl::unsupported::corotype`/`tcl::tm::*` — see `name-resolution-fix-plan.md` Stage 15.5. `tcl::prefix`/`tcl::clock` still unmodelled (low-priority residual; likely correctly so, per §3). |
...
- `::tcl::mathop` gating + operator inline-compile: FAITHFUL — `mathop.rs:22-35`, `command_binding.rs:481-498`. (A related but distinct bug lived on the *profile* axis, not this registry gate: the plain `tcl8.4` `DialectProfile` had `operators_as_commands: true`, so the mathop heads resolved under `tcl8.4` via profile shape even though the per-spec gate above was always correct. Fixed — see `name-resolution-fix-plan.md` Stage 15.5.)
