...
- **M16** — VM behavioural parity: `TclPreventAliasLoop`, cross-interp
  aliases and `interp eval`/`invokehidden` in any direction (parent↔child,
  sibling↔sibling, arbitrary-depth grandchild paths) via an interpreter-tree
  engine that reaches a target interp by switching arena-held state on the
  shared native stack rather than suspending — so alias calls compose with
  every native re-entry (coroutine resume, `lsort -command`, trace/event
  callbacks) exactly as C Tcl does — command + execution traces with
  C-faithful shapes including step-trace visibility into opcode-inlined
  commands (deoptimising recompile on demand, matching C's
  `DONT_COMPILE_CMDS_INLINE`), and the command-name epoch cache. See
  [issue #946](https://github.com/bitwisecook/tcl-lsp/issues/946) for the
  full differential verification this closed out.
...
- [x] Cross-file go-to-definition: `cross_file_method_definition` — **reworked for issue #945 faults 4 + 6** — resolves the receiver class's C-faithful **dispatch chain** (`WorkspaceIndex::method_dispatch_chain` over the canonical `tcl_syntax::mro` linearisation, visibility-filtered by the call's `MethodAccess` context) and returns the **entry implementation only**, never the whole override family; an externally-uncallable (unexported/private) method resolves to nothing, mirroring C's `unknown method`. Wired into `compute_definition` before the command-head gate (a method call's head is `$obj`, not the method token). Resolves the common inherited-method-declared-in-another-file case.
...
**Stage 6.3 — class references from `superclass` / `mixin` / `inherit` (issue #923 real-project follow-up)** ✅ **DONE**
- [x] Empirical verification against georgtree's `SpiceGenTcl` (a deeply-namespaced, one-class-per-file `TclOO`/`oo::configurable` project) surfaced the gap the per-stage tests missed: a class named by a `superclass` / `mixin` argument (`superclass ::SpiceGenTcl::Device`, 64 sites) was **not** a reference.  Find All References on the class returned only its declaration, and rename left every `superclass` site dangling — silent corruption of the inheritance graph.  Root cause: `superclass`/`mixin` populated `ClassDef::superclass_refs`/`mixin_refs` (used by the hierarchy graph and a single-document band-aid in `references::class_references`) but were never recorded in `command_invocations`, so the workspace oracle (`invocations_of`), `class_reference_spans`, and `rename_class` — which all read `command_invocations` — never saw them.
- [x] **Fix (registry-driven, general).** The class-body walker now records each command-naming member argument as a `command_invocation` via `record_member_command_references`, dispatching on registry data — `MemberSpec::all_args_ref == MemberRefKind::Class` (`superclass`, `mixin`, `[incr Tcl]` `inherit`) and member `ArgRole::CommandName` positions (`forward` TARGET, generalised off its former hardcoded `if texts[0] == "forward"` special-case) — never a member keyword.  The redundant `superclass_refs`/`mixin_refs` band-aid was removed from `references::class_references` so references, rename, and the code-lens count read one source of truth and can never diverge.  Single-document go-to-definition now skips a document-local `via_define` extension stub so a `superclass` cursor falls through to the cross-file creation site (Stage 6.2's preference).  Covered by `superclass`/`mixin`/`inherit`/`forward` analyser TP/FP/TN/FN (`analyser::oo::tests`), lsp-core references + rename (`references_residual.rs`, `references_rename.rs`), native e2e (`issue923_class_refs.rs`), and VS Code (`references.test.ts`, `renameSymbol.test.ts`).
...
**Stage 7.2 — resolve constants** ✅ **REWORKED (issue #945 faults 1–2)** — a `$cmd` head is recorded as a pending dispatch site (`ConstDispatchSite`: variable + head span + resolution namespace + head-expansion flag) and settled in the CFG/SSA phase against the compiler's **flow-sensitive value model** (`value_provenance::const_contributors` — the SSA use-version's reaching constant definitions through φ-joins and pure copy chains), never the lexical `const_strings` map, whose last-write-wins view collapsed `if`/loop joins into the lexically last assignment.  Every may-target of a branch join is referenced; each contributing literal with a source-exact span becomes a **writable literal-anchored reference** (so rename rewrites `set cmd target` → `set cmd renamed` and the output executes under tclsh 9.0.4), while the `$cmd` head invocation stays **`indirect: true`** and is never rewritten.  An expanded `{*}$cmd` head narrows the writable span to the value's first list element.  Unprovable shapes — computed values, proc parameters, `upvar`/trace-reachable writes, opaque `catch` bodies — abstain soundly, and a contributor with no writable span blocks the whole rename (`rename_safe`).  Tests: analyser TP/FP/TN/FN (`*_945` in `diagnostics/tests.rs` — branch/switch/loop/try joins, copy chains, traced heads, list prefixes), lsp-core references + rename (renamed output executed under a real tclsh), native e2e `issue945.rs`, VS Code `issue945.test.ts`.
...
**Stage 15.1 — cross-interp isolation** ✅ **REWORKED (issue #945 faults 7–8)** — the namespace approximation grew into an **interpreter-domain model**: `interp create ?-safe?` / `delete` / `hide` / `expose` maintain per-path state via registry hooks (`AnalyserHookId::InterpCreate`/`InterpDelete`/`InterpHide`/`InterpExpose`); eval bodies home under the collision-free synthetic `@interp@<path>` domain (a real parent namespace named like the child can no longer merge), `interp delete` bumps a per-path **epoch** so a recreated interpreter is a fresh domain, paths inside child bodies qualify against the enclosing interpreter (`interp create t` inside `interp eval s {…}` names `s t`), multi-word eval scripts are consumed without walking (the old fall-through analysed them in the *parent* scope), an eval into a never-created literal path draws **W140**, a safe child's body walks with the registry's `Traits::SAFE_INTERP_HIDDEN` set hidden (**W129**, and no effect edges from calls that never execute), and literal cross-domain aliases record domain-qualified targets.  See [resolution-soundness-945.md](resolution-soundness-945.md).  Tests: `interp_eval_child_isolates_definitions_from_the_parent` (now runnable Tcl — the child is created first), `deleted_and_recreated_interp_is_a_fresh_domain_945`, `nested_interp_ops_qualify_against_the_enclosing_child_945`, `cross_domain_interp_alias_links_child_calls_to_the_parent_target_945`, `safe_interp_hides_unsafe_commands_and_expose_restores_945`, `interp_eval_into_uncreated_interp_warns_and_multiword_stays_isolated_945`, `interp_eval_empty_path_is_the_current_interpreter`, `rename_parent_proc_does_not_edit_a_child_interp_body`.
...
**Stage 15.5 — the "already modelled" claim above was itself wrong (::tcl:: namespace dialect handling)** ✅ **DONE** — a follow-up audit (prompted by a user asking for `::tcl::` handling to be checked per-dialect, since the namespace does not exist at all in Tcl 8.4 / F5 iRules — a real Tcl 8.4.6 core) found that Stage 15.4's "already modelled with dialect gating" claim for `tcl::build-info`, `tcl::unsupported::corotype`, and `tcl::tm::path`/`tcl::tm::roots` was **not actually true**: all four carried `dialects: None` (universal — the exact bug class Stage 15.4 fixed for `zipfs`), silently leaking them into `tcl8.4` (iRules was accidentally masked by two ad hoc `IRULES_DISABLED_COMMANDS` entries for two of the four names, which is precisely the "version-gating in the ban list, not the version axis" anti-pattern that same list's own contract test warns against). Fixed:
- `tcl::build-info` / `::tcl::build-info` (two separate specs, one per spelling) → `TCL90_PLUS`.
- `::tcl::unsupported::corotype` → `TCL86_PLUS`, not `TCL90_PLUS` — empirically verified present against a real `tclsh 8.6.14` (coroutines are TIP 396, an 8.6 feature); the file's own doc comment claiming "Tcl 9 ships" was wrong. Also gained a bare `tcl::unsupported::corotype` registration alongside the fully-qualified one (matching the `tcl::process`/`tcl::zipfs`/`tcl::idna` sibling pattern) — needed because `tcl-registry`'s lookup has no bare→qualified fallback (only the reverse), so a qualified-only spec silently failed to resolve under its *own* valid dialect once `known_in_any_dialect` was fixed (below).
- `tcl::tm::path` / `tcl::tm::roots` (Tcl Modules, TIP 189) → `TCL85_PLUS`; these were never covered by Stage 15.4's parenthetical at all.
- `tcl::idna::decode` / `tcl::idna::encode` — a second, independent `stdlib/` registration of these two names (distinct from the `tcl_idna.rs` ensemble's own gated subcommands, attributed to the bundled `cookiejar` package) → `TCL86_PLUS`.
- **A related, adjacent bug on the *profile* axis, not the per-spec `dialects` axis:** the plain `"tcl8.4"` `DialectProfile` had `operators_as_commands: true` — so `::tcl::mathop::+`/bare `+`/etc. (already correctly `TCL85_PLUS`-shaped data, no gap on that axis) resolved under `tcl8.4` anyway, the same K36322151-style profile-shape gap `f5-irules` already avoids. Fixed to `false`; the `tcl-dialect` invariant test asserting "operators everywhere but iRules" was itself encoding the bug and is renamed/corrected.
- **A registry bug the above surfaced, not specific to `::tcl::`:** `CommandRegistry::known_in_any_dialect` bare-normalises its query but not the cached spec-name set it checks against, so a spec registered *only* under its `::`-qualified spelling (`::tcl::unsupported::corotype`, before the bare form above was added) could never satisfy the W002 "disabled in dialect" check's `known_in_any_dialect` guard — it silently drew **no diagnostic at all** in *any* dialect rather than the correct W002. Fixed by bare-normalising at insertion too.
- `IRULES_DISABLED_COMMANDS` gained the seven `tcl::Opt*` "opt" package names (`OptKeyRegister`/`OptKeyParse`/`OptKeyDelete`/`OptKeyError`/`OptParse`/`OptProc`/`OptProcArgGiven`) — these correctly stay `dialects: None` (the `opt` library package predates namespaces and ships across every real Tcl 8.x/9.x, so it is *not* a version-gating case), but iRules' TMM sandbox has no `package`/`source`/`auto_load` (already banned in the same list), so `opt` can never actually load there either — the same reasoning as the pre-existing `package`/`source` bans.
- Investigated (per the same user request) whether real Tcl's `::tcl` namespace gets any special "readonly" / rename-protection treatment that the LSP would need to account for: empirically verified against a real `tclsh 8.6.14` that it does not — `rename ::tcl::mathop::+ ::foo`, `namespace delete ::tcl::mathop`, and even `namespace delete ::tcl` all succeed with no special-cased error (unlike the *global* namespace `::`, which cannot be deleted). `::tcl` is merely unusually **load-bearing**: it hosts the `::tcl::UnknownPending` recursion guard the core `unknown` auto-loading proc uses, plus the Tcl-scripted implementations backing several C ensembles (`string`/`dict`/`array`/`file`/`info`/`namespace`/`clock`/`chan`/`binary`/`encoding`/`zlib`), so deleting it cascades into breaking almost everything in the interpreter — a footgun, not a locked door. No LSP change follows from this (nothing to protect against that Tcl itself doesn't already allow); noted here since it was explicitly asked about.
- New tests: `rust/tcl-compiler/tests/tcl_namespace_probe.rs` (dialect-negative cases for all of the above), `rust/tcl-registry/tests/dialect_profile.rs` (`tcl8.4` added to the operator-shape TN block), `rust/tcl-registry/tests/registry_commands.rs` (`tcl_namespace_commands_gated_to_their_real_introduction_version`).
- **Separately, and NOT fixed by this stage:** verifying the above surfaced that `expr` math-function calls (`sin($x)`, `abs($x)`, …) draw a spurious W123 "unknown command" in *every* dialect whenever the function isn't shadowed by a matching user `proc ::tcl::mathfunc::NAME` — a much larger, pre-existing, dialect-*independent* bug (`::tcl::mathfunc` carries zero registry specs at all, unlike `::tcl::mathop`). Reported by a user as [issue #968](https://github.com/bitwisecook/tcl-lsp/issues/968); a fix already exists on the sibling branch `claude/awesome-curie-rxyttd` (not yet merged as of this writing) reusing `tcl_syntax::expr::mathfunc`'s existing version table rather than adding a parallel registry surface. Left alone here to avoid duplicating/conflicting with that in-flight work.
...
**Stage 16.2** ✅ **DONE — full C parity, [issue #946](https://github.com/bitwisecook/tcl-lsp/issues/946) faults 1–2 closed.** The VM's interpreter model is an engine (`Vm`) driving a tree of interpreters in one arena (`interp::InterpState` per node, addressed by a stable `InterpId` that is never reused), with the currently-executing interp's state held directly (`Vm: Deref<Target = InterpState>`). Cross-interp evaluation — `interp eval`, an alias crossing in *any* direction (parent→child, child→parent, sibling→sibling routed through the shared parent, and arbitrary-depth grandchild paths), `interp invokehidden` — is `Vm::in_interp`: a plain nested native call that swaps which arena slot is "current" and swaps back on return, exactly mirroring C's `Tcl_EvalObjv(targetInterp, …)` on one shared C stack. This replaced the previous suspend/resume "hosted pump" model entirely (no more `YieldReq::ParentCall` / `DriveMode::ChildHosted` / `HostedExit` — cross-interp calls need no coroutine-style suspension at all once the target is reached by switching state rather than freezing the caller). Consequences: a parent-target alias now works from *any* native re-entry — a resumed coroutine, an `lsort -command` comparator, a variable-write trace callback, an `after`/`vwait` event callback — because the target interpreter's state stays in the arena, addressable, whichever native call reaches it (fault 1: no more `cannot invoke parent-interp alias: C stack busy`); a parent-alias target may re-enter the child that invoked it, or a sibling child, or a multi-level grandchild path, because every interpreter's state is always live in its arena slot whether or not it is "current" (fault 2: no more could-not-find on re-entry). Interpreter lifecycle is now explicit: an `InterpSlot` tracks liveness (`dying`) and an in-flight-evaluation count (`active`), so `interp delete` on a currently-executing interpreter defers state teardown until the last nested call unwinds (C's `Tcl_Preserve`/`Tcl_Release`), and deleting a target interpreter sweeps cross-interp aliases that pointed at it out of their source interpreters (`alias_backrefs`). `yield` itself keeps its own, still-genuine boundary (`activation_depth`; `cannot yield: C stack busy` for a bare `yield` crossing a native re-entry — unlike alias calls, C Tcl has this boundary too). A pre-existing, unrelated VM bug surfaced by the new differential coverage was fixed alongside this: a `break`/`continue` reaching a proc's own boundary (`return -level 0 -code break`, or a bare `break`/`continue`) now converts to the error `invoked "break" outside of a loop` with `-errorcode {TCL RESULT UNEXPECTED}`, matching C's `InterpProcNR2` epilogue — previously it silently propagated as `TCL_BREAK`/`TCL_CONTINUE`. Tests: `rust/tcl-vm/tests/cross_interp_reentry_e2e.rs` (21 vectors: native re-entry paths, completion-code/errorcode/errorinfo propagation, suspended-child/grandchild/sibling re-entry, frame/interpreter identity, target-death alias cleanup, safe children) plus the existing `cross_interp_alias_e2e.rs`, both VM + real tclsh8.6/9.0.

**Stage 16.3** ✅ **DONE — full C parity, [issue #946](https://github.com/bitwisecook/tcl-lsp/issues/946) fault 3 closed.** Fire command/execution traces (previously accepted no-op).  Command traces (`rename`/`delete`) fire with fully-qualified names, follow the command through `rename`, fire on redefinition-overwrite, and ignore callback errors; `trace add command` requires the command to exist.  Execution traces: `enter`/`leave` (`cmd-string op` / `cmd-string code result op` shapes), `enterstep`/`leavestep` via a step-scope stack; an enter-trace error **aborts** the command, a leave/leavestep error **replaces** its result; deferred bodies settle their leave when the frame unwinds (contexts ride the frame, transfer across `tailcall`). Step traces now observe opcode-inlined commands too (`set`, `incr`, `return`, and every structured control-flow form — `if`/`while`/`foreach`/`eval`/`uplevel`/`catch`/`try`/…), matching C's `DONT_COMPILE_CMDS_INLINE` (`tclTrace.c`): `Vm::step_trace_active` tracks whether any `enterstep`/`leavestep`-capable trace exists anywhere in the interp; `ProcDef::compiled_epoch` + `Vm::trace_deopt_epoch` detect a stale compile on next entry and recompile the proc's body from its retained source (`ProcDef::body_src`) via `CompileService::compile_traced` — a new compiler-side pass (`Lowerer::trace_visible`, `lower_to_ir_traced`) that suppresses every registry-driven inline/structured lowering hook so the whole body becomes plain runtime dispatches, reverting the same way once the last such trace is removed. `Vm::compile_cached` (the shared body of `eval_source`/`compile_source_cached`) applies the identical choice, so every runtime-compiled body a traced proc's control-flow builtins evaluate (`if`/`while`/`foreach`/`eval`/`uplevel`/`catch`/`try`'s bodies all run through this path) is trace-visible too — not just the directly-traced proc. TclOO method bodies and coroutine suspension/resume are covered the same way. A C re-entrancy guard (`INTERP_TRACE_IN_PROGRESS`) was added alongside this (`Vm::trace_in_progress`): a trace callback's own commands are never step-observed, preventing infinite recursion into a callback that itself dispatches commands (e.g. `puts` inside the callback). All tclsh8.6+9.0-pinned in `rust/tcl-vm/tests/command_traces_e2e.rs` and the new `rust/tcl-vm/tests/step_trace_inline_e2e.rs` (18 vectors: inlined forms, nested control flow, command substitution, coroutine suspension, TclOO methods, callback errors/mutations, trace add/remove mid-execution, redefinition, rename). One narrow, explicitly-pinned residual divergence remains (`step_trace_over_observes_a_tailcall_target_vm_divergence`): C's interp-wide step trace is torn down when the traced proc's own `leave` event matches the level/command-string it was registered at, and a `tailcall` — which replaces the traced proc's activation at the *same* level — trips that teardown early in C, so the tail target's body runs unobserved; the VM's step-scope model is a stack tied to the traced proc's own activation frame rather than a level/command-string match, so it keeps observing through the tail-call replacement (a broader, not narrower, observation than C).
