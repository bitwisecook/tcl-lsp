...
**Stage 2.2 — the analyser link model** — instance variables ✅ **DONE**
- [x] TclOO instance variables now unify across methods: `references::linked_var_reference_spans` (in `definition.rs`) walks the scope tree and unions the uses of every `VarDef` that shares one `variable v` declaration span, wired into `references`, `rename`, and `document_highlight`. Find-References/Rename/Highlight on `$v` in one method now reach the declaration and every sibling method's use as one variable (verified unit + e2e; the shared declaration span from the corruption fix is the natural link key, so no new `VarDef` field was needed; the zero-width implicit-var fallback is excluded so declaration-less seeds never over-merge).
- [ ] **Remaining:** cross-frame/namespace aliasing — `upvar` / `namespace upvar` / `global` / `variable` link a local to a caller-frame or namespace cell ([`handle_upvar_command`](https://github.com/bitwisecook/tcl-lsp/blob/6a6bc87e94c67416cdca6954ba2ad0ec6937bd63/rust/tcl-compiler/src/analyser/handlers.rs#L1483) et al. record only the local alias, not the target). Needs a target-recording edge, distinct from the shared-span key used for instance vars.
- [ ] **Remaining:** for a non-`#0` `uplevel N { … }` (target frame statically unknown) **abstain** rather than mis-attribute body vars.
