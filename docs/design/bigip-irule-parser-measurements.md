...
| A **`matches`** re-probe alongside `matches_glob`/`matches_regex` | `e_matches` answered `1` in all three F5 contexts, but the word-operator table carries nine operators and not the bare `matches`. One confirming probe (and its precedence behaviour) closes a recorded corpus divergence. |
...
| The **21 recorded event-context divergences** re-probed per cell | The corpus pins where the shipping model and the appliance disagree (§8's matrix). Six are the deliberate `RULE_INIT` compile-acceptance difference; the other fifteen are open registry-data gaps, and each needs its cell confirmed before the data is changed. |
