...
Fifteen of these are **fixed**: idx 61 (critical, §3's `d825d1d`), idx 9
...
idx 70 (high, §3's `a8971ac`); the other 70 main-wave findings are
clustered by feature/root-cause with a priority-ordered table in
...
REFUTED.** Of the 85 CONFIRMED: **15 fixed** (idx 61, critical — §3's
...
#### Priority tier 1 — critical + high (24 findings, 15 already fixed)
...
4. Pick the next finding to fix — two ready queues, both fully triaged:
   ...
   - §6b: 70 remaining main-wave findings (idx 61, idx 9, idx 10, idx 18,
     idx 29, idx 31, idx 32, idx 33, idx 39, idx 46, idx 52, idx 56, idx
     63, idx 68, and idx 70 are fixed — idx 46 and idx 63 only partially,
     see their §3/§6b rows for what's still open — so far), fully
     triaged into a priority-ordered critical/high table (9 remaining,
     start here) and a feature-clustered medium/low table (61 findings,
     group by feature when fixing). Likely the higher-leverage queue
     given its size and the presence of several zero-results
...
7. Both queues (§6a's 6 tcllib findings, §6b's 70 main-wave findings) are
