...
Four of these are **fixed**: idx 61 (critical, §3's `d825d1d`), idx 9
(high, §3's `26e4ea3`), idx 10 (high, §3's `9a55b00`), idx 18 (high, §3's
`b3cbdb1`); the other 81 main-wave findings are clustered by
feature/root-cause with a priority-ordered table in §6b, ready for a
future session to pick up efficiently. Nothing is lost — the raw data,
...
REFUTED.** Of the 85 CONFIRMED: **4 fixed** (idx 61, critical — §3's
`d825d1d`; idx 9, high — §3's `26e4ea3`; idx 10, high — §3's `9a55b00`;
idx 18, high — §3's `b3cbdb1`), **81 remaining**.
...
#### Priority tier 1 — critical + high (24 findings, 4 already fixed)
...
4. Pick the next finding to fix — two ready queues, both fully triaged:
   ...
   - §6b: 81 remaining main-wave findings (idx 61, idx 9, idx 10, and idx
     18 are fixed so far), fully triaged into a priority-ordered
     critical/high table (20 remaining, start here) and a
