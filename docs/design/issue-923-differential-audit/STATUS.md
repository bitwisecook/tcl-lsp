...
| 44 (namespaces) | `${ns}::setdef` resolved to nothing — go-to-definition, find-references, and call-hierarchy silently dropped every call site. | Two halves. (1) **Wrong source bytes**: for a braced composite word the lexer merges the whole word into one `Var` token, so its raw text is `ns}::setdef`, not the variable name — `resolve_dynamic_word` looked that up as a variable and always missed. It now truncates at the brace the lexer left in place (the same rule `record_var_or_cmd_command_site` already used for its W307 head reading) and folds the literal tail. (2) The command **head** itself now goes through that folding before `resolve_command_qualified_name`, via `resolve_dynamic_command_head`, using the *dominating*-constant lattice so a branch-conditional binding still abstains. **2026-07-31 (issues #1096 / #1097):** the `TclOO` one-level-removed spelling of the same idiom — `set ns [namespace qualifiers [self class]]` — now folds to its constant at the optimiser level too (`namespace qualifiers`/`tail` gained registry `const_fold` callbacks; method bodies gained an instance-variable escaping model so method-local variables propagate). Row state unchanged: the resolution-layer fix this row records is what it always was, and the two remaining hops of the full chain — the analyser's constant-string lattice folding a constant `[…]` `set` RHS, and `definition()` consuming an already-resolved `indirect` invocation instead of falling back to a bareword proc lookup — are tracked as their own issues. |
