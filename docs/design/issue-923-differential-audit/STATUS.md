...
Twenty of these are **fixed**: idx 61 (critical, §3's `d825d1d`), idx 9
...
partial — §3's `e178f29`), idx 86 (high, §3's `cf5720c`); the other 65
...
| 86 | high | tk | rename | **FIXED** (`cf5720c`) — `tk/library/accessibility.tcl`'s `foreach wtype {...} { rename ::$wtype ::tk::ac... }` loop-generated rename/proc targets weren't tracked (nor was a plain `proc ::$wtype {...}` outside any loop — `proc`'s name never attempted constant-folding at all); go-to-definition on a post-rename call fell through to the stale original, and the outline showed a garbled `${wtype}`-named entry. |
