...
toolchain reads the file itself (`tcl diag`, the editor's workspace scan) it
has the bytes, so it names the exact offset and class — and it can *prove* a
`U+FFFD` you typed on purpose is genuine content, and stays silent. When your
editor sends an already-decoded buffer over the language-server protocol the
original bytes are gone, so the check reports on the replacement characters
alone and says "most likely". The same code fires either way; only the
precision differs.
