...
The three verbs answer three different shapes of question.  Pick by shape, not by feature list.
...
**`f5 rename` answers the precise rename question.**  "Swap full-path X for Y everywhere it appears."  It is the simplest verb for that case: an old name, a new name, and the config, printing a dry-run unified diff by default, with `--in-place` to persist.  Reach for it when you already know both names and you need no filter or property edit.

A useful shorthand: if the question is just two names, use `rename`.  If the answer is a list of objects, use `grep` for relationship-shaped questions and `query` for property-shaped ones.  If you want to update a field on many objects in one expression, use `query`.
