...
The BIG-IP object registry is the single source of truth for what every
TMSH object kind looks like to `f5`: how the parser turns config text
into typed values, how the query DSL projects each property, how the
edit planner renders it back, and which references each property
carries for the graph and LSP layers.
...
Run `f5 query` or open a `.conf` file in any supported editor — the
registry runs in the background.  The user-facing payoff is consistent
behaviour across `f5 grep`, `f5 query`, `f5 explain`, `f5 rename`,
document links, definition jump, references, rename, and semantic
tokens: every place that asks "what does this property reference?"
hits the same dispatch.

## Object kind catalogue

Every object kind the registry understands is a curated entry in
`core/bigip/registry/specs/`.  743 distinct kinds across:
