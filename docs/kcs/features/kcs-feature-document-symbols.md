...
- `rust/tcl-compiler/src/analyser/oo.rs` — `expand_wrapper_block_members` (`self { … }` / `private { … }` block forms), `apply_oo_self`, `retract_named_members` / `apply_visibility_member` (registry-driven member effects, scoped by `MemberSide`)
- `rust/tcl-lsp-core/src/workspace_index.rs` — `WorkspaceClass::retracted_members` + `method_dispatch_chain` (cross-document retraction tombstones and the effective-export union)
