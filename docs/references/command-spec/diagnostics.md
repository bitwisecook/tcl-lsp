...
| W102 / W309 | `subst` risks | `Traits::PERFORMS_SUBSTITUTION` gates, and the switch rows' option effects (projected by `CommandRegistry::substitutions_performed`) say which kinds this call still runs, so a switch never suppresses by spelling; `EVALUATES_CODE \| TAINT_SINK` for the outer half |
