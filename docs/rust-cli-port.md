...
| `callgraph` (`call-graph`) | ✅ wired | ports `build_call_graph` + `_find_call_sites_in_scope` / `_find_top_level_calls` / `_is_inside_proc` / `_effect_region_str` (`analyser/semantic_graph.py`) onto `tcl-compiler` `CompilationUnit` (`ir_module` + `interproc`) for nodes/edges plus the analyser's `command_invocations` for call-site resolution. Added `ProcSummary::direct_calls` — the *direct* (non-transitive) local call set the doc called for (Python's `local.calls`) — so the edge list carries no spurious `A→C` transitive edges; nodes sorted by qname, top-level edges in first-seen order, `<top-level>` root prepended. Text + JSON. **Interproc-engine gaps** (output tracks the engine, converges as it does): the call scanner does **not** detect calls nested in `[cmd …]` substitutions inside a proc body (so proc→proc edges through `return [f …]` / `set x [f …]` are missed — **top-level** edges resolve via the analyser and are unaffected); a proc that calls another proc gets the callee's command-effect applied locally, diverging from Python's `pure` (Python skips the effect for a resolved internal call); and global-variable writes land in `effect_writes` (`GLOBAL_STATE`) rather than Python's separate `writes_global` flag, so `effects` diverges for global-mutating procs. Closing these is the interproc-engine workstream (broad blast radius across the optimiser/analyser). **Faithful subset golden-tested** (`callgraph.tcl`: pure leaf procs invoked from the top level + a namespace proc — locks node/edge/roots/leaves serialisation + top-level call-site resolution byte-for-byte) |
