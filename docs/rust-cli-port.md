...
| `query` | ✅ read-only parity / 🚧 mutations | **Read-only `f5 query` runs end-to-end byte-identical to the Python CLI.** `tcl-bigip-query` has the front-end, value model, output render modes, evaluator (full jq core + all 29 special forms), **~204/244 builtins** (every byte-deterministic one: value/stream/string/regex/math/net-ip-CIDR/time/encoding), the **BIG-IP projection** layer (Container/ObjectRef/PathRef over the typed model — core LTM kinds), and a read-only **runner + verb** (`commands::query`, output modes auto/json/raw/paths/scf/table, multi-file banners + json envelope, `--strict` exit codes). 10+ cookbook examples verified byte-identical end-to-end (`f5-cli` `query_parity.rs`, 26 goldens; plus per-layer golden suites: front_end/output/eval/math/net/regex_str/time/value2/projection). **Remaining:** `refs`/`referenced_by` + rule `.refs` (graph-backed, in progress), **edit-plan/rename** (mutating queries + `--write`/`--in-place`, needs field-slot byte-ranges), live **probes** (dns/ping/http/x509/tls), **side-inputs** (`--input-*`), **renderers** (`--render`), the `--help-*` actions, and the long-tail object kinds in projection |
