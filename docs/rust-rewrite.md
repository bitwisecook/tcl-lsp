...
* **Semantic tokens**: full classification taxonomy
  ...
  full stream).
* **`call PROC_NAME` first-arg completion** — same shape as
  the `when EVENT` enumeration that landed this session,
  but needs a `Traits::INVOKES_USER_PROC` bit (or
  equivalent registry hint) so the completion provider can
  identify the command without hard-coding the name
  `"call"`.
