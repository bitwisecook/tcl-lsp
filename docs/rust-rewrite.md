...
| C41-default-on-followups-cmdsubst | **Nested `[cmd ...]` substitution invocation walking.** Python's `_AnalyserCommandsMixin._record_command_invocation` records every `[…]` substitution it encounters inside a command argument as its own `CommandInvocation`. The Rust analyser only records the head of each top-level command. Without nested-cmdsubst walking, workspace usage counts, references, rename, and call-hierarchy under-count cross-references like `set x [helper $foo]` (which references `helper`). Port: extend `commands.rs::process_command` (or add a sibling helper) to scan each argument token's text for `\[…\]` substitutions, segment the inner text via `segment_commands_local`, and recursively dispatch each inner command head through the invocation-recording path. Mirrors `_record_command_invocation`'s behaviour around the `_iter_nested_invocations` helper. Combined with the recovery port above, lets `command_invocations` graduate from `rust.X = python.X` to no override. | planned |
...
| C41-default-on-followups-param-traits | **`infer_param_traits` port.** Free-standing port of `core/analysis/proc_arg_traits.py::infer_param_traits` (~525 LOC, called out in the original C41 handoff as its own chunk). Adds `param_traits: HashMap<String, HashSet<ProcArgTrait>>` to `analyser::types::ProcDef`; populates it in `handle_proc_command` from a body walk that records each parameter's role (`EVAL` / `BODY` / `VAR_WRITE` / `VAR_READ` / `EXPR` / `LOOP_LIST`). Fixes the Rust W214 over-emit on `[expr {$param}]` patterns (the analyser currently can't see the param being read inside the expr), and unblocks downstream optimisations that consult `param_traits` (`_branch_folding`, `_propagation`, taint inter-procedural). Threading: `ProcDef.param_traits` field already exists Python-side; the PyO3 binding emits an empty dict today; the materialiser plumbs it through. | planned |
