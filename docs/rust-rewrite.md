...
| 3 | `C43` (residual: per-form runtime dispatch migration) | **Partial landed via PR #389 + follow-up sub-commits** — `_barrier_gate.py` ported as `body_has_dynamic_barrier`; all 14 structured-command `LoweringHookId` variants (`Proc`, `When`, `NamespaceEval`, `If`, `Switch`, `For`, `While`, `Foreach`, `Lmap`, `Catch`, `Try`, `Dict`, `Eval`, `Uplevel`) stamped on every relevant `CommandSpec`; `Lowerer::try_dispatch_structured_hook` routes calls through `resolve_call().lowering_hook` for migrated forms.  **Migrated forms:** `eval`, `uplevel` (sub-strip 1, first-form pilot — simple dialect-agnostic single-method dispatch).  **Remaining:** `proc`, `when`, `namespace eval`, `if`, `switch`, `for`, `while`, `foreach`, `lmap`, `foreachLine`, `catch`, `try`, `dict` — each lands as its own focused sub-commit.  The `when` migration is blocked on unifying the registry-load path across test and production harnesses (`build_default()` doesn't `load_irules()`, so a registry-driven `when` would silently fall through). |
...
* **Priority 3** (`C43` residual — runtime dispatch migration)
  — first sub-strip landed: `eval` + `uplevel` now route
  through `Lowerer::try_dispatch_structured_hook` calling
  `resolve_call().lowering_hook`.  Remaining structured forms
  (`proc`, `when`, `namespace eval`, `if`, `switch`, `for`,
  `while`, `foreach`, `lmap`, `foreachLine`, `catch`, `try`,
  `dict`) migrate as their own sub-commits.  The `when`
  migration is gated on unifying the registry-load path
  across test and production harnesses.
