...
| **S-references** (covers `S-document-highlight`) | **Wire find-references and document-highlight end-to-end in `tcl-lsp-server` with `spawn_blocking` worker offload.** Adds `tcl-lsp-core::references` (~140 LOC + 4 unit tests) covering: `$var` references → `VarDef.definition_span` plus every span in `VarDef.references`; proc references → `ProcDef.name_span` plus every command invocation in `analysis.command_invocations` whose head matches the proc's simple, qualified, or unprefixed-qualified name; class references → `ClassDef.name_span` plus every command invocation matching the class name. Output is deduplicated by `(start_line, start_character, end_line, end_character)`. The server's `LanguageServer::references` lifts to `tower_lsp::lsp_types::Location` arrays. Advertises `referencesProvider` and `documentHighlightProvider`. **`S-document-highlight-rich` landed via `4c153fd`:** new `document_highlights(source, line, character, analysis) -> Vec<(LspRange, HighlightKind)>` entry point.  Variables surface their `definition_span` as `Write` and each span in `VarDef.references` as `Read`; procs / classes surface their `name_span` as `Write` and matching command-invocation heads as `Text` (the analyser doesn't currently distinguish read vs write semantics on call-head matches).  `dedup_kinded` keeps the highest-kind label per span (Write > Read > Text).  4 new unit tests pin the tagging; the synthetic-`VarDef` test verifies the kind logic in isolation from the analyser's body-walk gap (`$x` reads in arg positions aren't tracked in `VarDef.references` today). **Still deferred to `S-references-rich` follow-up:** resolved-qualified-name matching for command invocations (`invocation.resolved_qualified_name` — the Rust analyser doesn't populate that field today), method-name references inside a class body, and cross-document references (workspace-index integration that lands alongside `S-workspace-symbols`).  Tracking `$x` reads in `VarDef.references` for global scope is its own analyser-side follow-up — once landed, the `document_highlights` provider will surface every `$x` site as `Read` without further plumbing. | landed (document-highlight-rich landed via 4c153fd; references-rich follow-ups deferred) |
| **S-rename** | **Wire the rename provider end-to-end in `tcl-lsp-server` with `spawn_blocking` worker offload.** Adds `tcl-lsp-core::rename` (~150 LOC + 3 unit tests) covering: `$var` references → rewrite `VarDef.definition_span` and every `VarDef.references` span to the new name; proc references → rewrite `ProcDef.name_span` and every matching command invocation in `analysis.command_invocations`. Output is deduplicated by edit range. The server's `LanguageServer::rename` lifts the core `TextEdit { range, new_text }` shape onto `tower_lsp::lsp_types::TextEdit` and wraps the result in `WorkspaceEdit { changes: { uri: edits } }`. Advertises `renameProvider`. **`S-rename-rich` first piece landed via `19aed1d`:** symbol-validity gating — `is_safe_symbol_name(name)` matches `^[A-Za-z_][A-Za-z0-9_]*$`; `is_builtin_command_name(name, registry)` consults `registry.command_names()`.  `rename()` grows an `Option<&CommandRegistry>` parameter.  Shape gate applies to every rename target (variable + proc); built-in-shadow gate applies only to proc renames.  7 new unit tests pin the gating behaviour. **Still deferred to further `S-rename-rich` sub-strips:** namespace-aware proc renames (`_namespace_prefix` / `_tail_name` machinery that rewrites short-form call sites when renaming a qualified proc), variable-name escaping for `${name}` braced references, class / method rename, and cross-document rename. | landed (safety gating landed via S-rename-rich first piece; other -rich sub-strips deferred) |
...
| **S-document-links / S-inlay-hints / S-code-lens / S-code-actions** | **Wire four LSP capability surfaces with stub `tcl-lsp-core` providers.** Each adds a small core module (~30-50 LOC + 1 unit test) that returns an empty list — none of these features have rich behaviour available yet, but advertising the capabilities lets editors stop displaying "feature unavailable" errors and gives the rich-port follow-ups a concrete server-side hook to extend. Server side advertises `documentLinkProvider`, `inlayHintProvider`, `codeLensProvider`, `codeActionProvider`; implements the four `LanguageServer` methods; lifts each core record onto its `tower_lsp::lsp_types` counterpart. **`S-inlay-hints-rich` landed via `09bd3c2`:** parameter-name hints at user-proc call sites.  `inlay_hints(source, range, analysis)` re-segments on each request, looks up each call's command head against `analysis.all_procs`, and emits a `param_name:` hint anchored at each positional argument's first character.  Filters by request range; skips `args` tail-collector parameter; stops at the proc's declared parameter count (no hints for extra args).  7 new unit tests (replacing the single empty-return stub test). **Still deferred** to per-feature `*-rich` follow-ups: document-link `source` / `package require` resolution against a workspace path-resolver; inlay hints for built-in commands (`set` / `lassign` — needs the registry's argument-name surface) and type / inferred-trait annotations; code-lens reference counts above defs (needs workspace-wide reference index); code-action quick-fixes for analyser diagnostics (needs the cached-analysis surface that `S-diagnostics` lands later). | landed (inlay-hints-rich landed via 09bd3c2; document-links / code-lens / code-actions rich follow-ups deferred) |
