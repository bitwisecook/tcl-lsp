...
| C27c  | **rendered_properties — string content lattice.** `RenderedProperties` bitflags (6 may-bits + 3 provenance-bits + 2 must-bits), `RenderedValueProps { may, must }` with `bottom()` / `top()` / default. `rendered_join(a, b)` (may ∪, must ∩), `analyse_literal(text)` for slash/backslash/CRLF/null + starts-with-slash/dash, `apply_unescape` / `apply_normalised` provenance transitions. 11 unit tests. Full SSA walk integration deferred to the taint follow-up. | landed |
...
| C32   | **Python shim retirement.** Plan: add PyO3 entry points in `tcl-lsp-rust` for `CompilationUnit::build_for`, `run_all_checks`, `find_redundancies` / `find_partial_redundancies` / `find_loop_invariants`, and (when their bodies finish) `find_shimmer_warnings` / `find_taint_warnings` / `run_passes`. Flip the Python `core.compiler.compiler_checks.find_all_diagnostics` and the analyser's SCCP / GVN / shimmer / taint entry points to call through PyO3, with a Python-pure fallback gated on an environment flag. Pattern already established by L11 (lexer flip). Blocked on the per-analysis PyO3 surface, which lands after the stubbed C27d/C29/C30 bodies are filled in. | planned |
