...
| — | `CST-CONSUMERS` (candidate a — give the CST descent / ranges accessors a production caller) | **Opened 2026-06-04 (the #538 L1 finding); strips 1-3 LANDED — L1 resolved for the descent.**  `descend_token` / `descend_command` and the `tcl-lexer::ranges` accessors landed with the CST but had **no production caller**.  The nested cmd-sub invocation walker in `analyser/commands.rs` (consumed by the LSP server's references / call-hierarchy via `analysis.command_invocations`) was a hand-rolled first-head-only scan; rebased onto the CST descent across three strips: **(1)** `descend_token` + `segments_from_tree` records every command in a substitution (`[foo; bar]` → `{foo, bar}`) — first caller of `descend_token`; **(2)** head fidelity via `texts[0]` (`$cmd`→`${cmd}`, `"q"`→`q`); **(3)** `descend_command` resolves body args so a control-flow command in a substitution surfaces its body's commands (`[if {$c} {puts hi}]` → `if, puts`) — first caller of `descend_command`; `switch` list-bodies are skipped (mirror of `_switch_list_body_index`) so a pattern is never mis-recorded.  Validated **Rust ⊆ main** over a 14-case battery (no false invocations).  **Strip 4** (the `ranges` accessors) is **blocked**: no faithful Rust caller until the irules quick-fixes / `_style.py` W108 land.  Tracked subset-gap follow-ups: switch-arm recording, expr-arg cmd-subs (+ the dual braced-data over-record).  See the `CST-CONSUMERS` section. |
