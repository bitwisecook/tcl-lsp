...
| C18   | **startCommand placement precision.** Close the layout-precision gaps in C17's `generate.rs` so per-command boundaries match tclsh byte-for-byte: foreach startCommand wrapping when `cmd_index > 0` (end label deferred to `foreach_end_N`); for-body startCommand when the body block has a Branch terminator (end label deferred to `if_end_N` pop); complex-foreach if-condition branch wrapping (per-body startCommand in `foreach_body_blocks`); constant-folded `if {1}` startCommand in non-proc scripts (so the tclsh "command boundary survives even when the branch is dead" behaviour matches); `<cond>` synthetic statement handling (defer startCommand end label to cover the `ExprCommand` inside the branch condition in `emit_branch`). | planned |
| C19   | **Value emission extras.** Extend `CodegenCtx::emit_value` (in `codegen/cmd_subst.rs`) with three specialised paths ported from the Python `_values.py`: `try_list_expand_concat` for `[list {*}$a {*}$b]` → load/load/listConcat; `try_inline_list_with_break_continue` for `[list arg [break]]`/`[list arg [continue]]` inline jump compilation; wire the existing `helpers::try_format_fold` into `emit_value` so `[format "..." args]` with literal args is constant-folded. Each path adds ~40 LOC and a handful of unit tests. | planned |
| C20   | **Differential codegen test harness.** `rust/tcl-compiler/tests/differential_codegen.rs` (or a standalone binary) feeds a corpus of Tcl scripts through both the Python emitter (via the existing `tcl_lsp_rust` PyO3 bridge or a subprocess call to `core.compiler.codegen.codegen_module`) and the Rust `codegen_module`; asserts byte-equal `FunctionAsm` output with a disassembly-diff report on the first divergence. Drives test-coverage for C17 + C18 + C19. Landed corpus: `tests/fixtures/*.tcl`, plus the harvested tclsh reference corpus under `tmp/tcl-*/`. | planned |
