...
| 6 | `SYNC-MAY19-dialect-contextvar` | Thread the per-folder dialect into the **lexer config** so mixed-dialect workspaces tokenise correctly.  **Half-landed, gap pinpointed (2026-06-04):** the `LexerConfig::for_dialect(&str)` constructor exists in `tcl-lexer` (sets `expand_syntax` off for 8.4 / iRules and `irules_brace_separator` on for iRules) **but has zero callers**.  The analyser's segmenter (`tcl-compiler/src/segmenter.rs::segment_commands_local`) lexes with `LexerConfig::default()`, and the server resolves the per-folder dialect only into `registry_for_dialect` — so every analyser-based LSP feature (document symbols, hover, diagnostics, …) tokenises dialect-blind.  Earlier note ("pure server-side, no analyser-crate changes") was **wrong**: the lex site is in the analyser crate, so the fix threads a dialect / `LexerConfig` through the `segment_commands*` API (used by `analyser::state`, `specialise_factories`, `place_bridge`, + ~30 tests).  That cross-cutting reach is why it sits under the `S*` umbrella.  W108's `non_ascii_mode` still has no Rust mirror (closes with the `_style.py` port). |
