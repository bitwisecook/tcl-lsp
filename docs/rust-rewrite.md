...
| C18   | **startCommand placement precision (cases 1–3 landed; cases 4–5 deferred).** `GenerateState` gains three new per-run maps: `foreach_end_labels` (keyed by `foreach_end_N` block), `for_body_end_labels` (keyed by `if_end_N` join), and `pending_join_labels` (applied when the join block emits its trailing pop). **Case 1** — foreach startCommand wrapping: when the foreach is not the first command in its script (`cmd_index > 0`), emit `startCommand` with `fresh_label("cmd_end")` before loading the list args, and defer the end label into `foreach_end_labels` so it is placed just before the `foreach_end` block's trailing pop (handled by `place_label_before_trailing_pop`). **Case 2** — for-body startCommand: a `for_body_*` block whose terminator is a Branch (first statement is an `if`) gets a startCommand whose end label is threaded through the true-branch successor to the `if_end_*` join block's pop. **Case 3** — complex-foreach if-condition wrapping: body blocks inside a complex foreach with a Branch terminator also get per-body startCommand, with the end label deferred to the `if_end_*` or `if_next_*` join. **Cases 4** (constant-folded `if {1}` startCommand in non-proc scripts) **and 5** (`<cond>` synthetic statement handling in `emit_branch`) are deferred to a follow-up: neither is exercised by the current fixture corpus, both require threading a new `pending_cond_end_label` piece of `CodegenCtx` state and aligning the dead-branch elimination in `linearise` with constant-folded conditions. New fixture `foreach-after-set.tcl` exercises case 1; `for-body-break.tcl` exercises case 2. Differential harness now reports 19/19 matching (8 exact, 11 semantic). | partial |
...
| C21   | **Registry codegen hooks.** Extend `tcl-registry::CommandSpec` with a `codegen_hook: Option<CodegenHookId>` field (indexed into a per-registry dispatch table, following the pattern of the existing `LoweringHookId`). Wire `codegen/emitter/bytecoded.rs::try_bytecoded` to look up and call the hook. Port per-command codegen hooks from `core/compiler/codegen/bytecoded/` (commands with command-specific bytecode that isn't already handled by `cmd_subst::emit_inline_cmd_subst` — e.g. `lset`, `lassign`, `array set`, `dict incr` with multi-key paths). Blocked on the registry-crate interface change. | planned |
