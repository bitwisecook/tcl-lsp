...
| C41-default-on | **Flip the C41 default.** Mirrors the `C40-default-on` shape: `core.analysis._analyser.analyse` dispatches to Rust by default whenever the `tcl_lsp_rust` binding is importable; the gate becomes `TCL_LSP_RUST_ANALYSER=0` opt-out. Reuses the existing `rust_shim_enabled` helper with `default=True`. **Status — blocked.** Prerequisites (a) and (b) landed across `C41-default-on-1` … `C41-default-on-6`: the two Python parity bugs (`::::ns::foo` double-prefix quirk; `nested_namespace_eval` outer-namespace-drop) are fixed in `_proc.py` + `_oo.py`, the differential corpus is expanded from 40 shape-only fixtures to 92 (53 shape-only + 34 tcl field-parity + 5 iRules field-parity + 4 dispatcher gating), and a strict `_compare_fields` helper compares per-fixture `all_procs` / `all_classes` / `all_variables` keys, the bare `name` / `params` / `name_range` / `body_range` fields, the `methods` / `class_methods` / `superclasses` / `mixins` of each class, and the recursive scope-tree shape. Diagnostics are deliberately excluded from `_compare_fields` (Python's `Analyser.analyse` integrates `run_compiler_checks` for style + SSA dead-store W110 / W220 / W304 …; Rust's `analyser_analyse` is analyser-only; layering alignment is its own follow-up). **The polarity flip itself was attempted and reverted — running `make prep-pr` with `default=True` produces ≈535 fast-suite failures.** The two blockers are: (i) **materialiser gaps** in `core/analysis/_analyser/__init__.py::_materialise_rust_analysis` — the Rust dict already contains `command_invocations`, `command_aliases`, `package_requires`, `namespace_imports`, `source_targets`, but the Python materialiser only extracts `global_scope` / `all_procs` / `all_classes` / `all_variables` / `diagnostics` / `unknown_proc_info`, dropping every side-channel field. Workspace-index, `interp alias`-driven role propagation, and `package require` -keyed feature gating all break as a result. (ii) **Rust-side parity gaps** — `source_targets`, `namespace_imports`, `regex_patterns`, `auto_path_entries`, `stub_commands`, `stub_expr_defs`, `package_provides`, plus the Rust analyser's lack of an integrated post-pass equivalent to `run_compiler_checks` (which is what populates W220 / W304 / W110 via the Python `analyse` shim). Other Rust-side per-feature gaps the differential surfaced but isn't blocked on: `Scope.classes` not threaded back per-scope; variables defined inside `when` event bodies not surfaced into the enclosing scope's variable set; `unset` arguments not recorded as scope variables; `for`-loop init bodies' `set` not surfaced into the enclosing scope. Recommended sequencing: a `C41-default-on-materialiser` chunk extends `_materialise_rust_analysis` to populate every `AnalysisResult` field the Rust dict already carries; a `C41-default-on-rust-fields` chunk lands the Rust-side ports of the missing emitters; and only then does the polarity flip become a single-line change. | blocked (corpus + bug-fix prerequisites landed; default-on flip blocked on materialiser + Rust-emitter parity gaps) |
