...
|---|---|---|
| 1 | `C41-default-on-followups-regex-vars` | Moderate.  Threads a ``const_strings`` map through ``analyser::state::Analyser`` so ``regexp $p`` resolves to the literal stored in ``set p "..."``.  Finishes the ``regex_patterns`` retirement (combined with the just-landed ``switch-regexp``). |
| 2 | `C41-default-on-followups-recovery` | Moderate segmenter heuristic port.  Partially closes the ``command_invocations`` merge.  Lift Python's ``_find_command_break_offset`` from ``core/parsing/command_segmenter.py``. |
| 3 | `C41-default-on-followups-noop-guards` | Audit + cleanup.  Probe whether the conservative-fallback guards ever fire on the corpus / sample-fixtures; delete the ones that don't. |
| 4 | `C41-default-on-followups-param-traits` (full) | 525 LOC — the original handoff item.  Adds ``ProcDef.param_traits`` populated by a body walk recording each parameter's role.  Unblocks the optimiser passes that consult traits.  Plumb through PyO3 + the materialiser. |
