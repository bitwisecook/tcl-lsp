...
**Remaining follow-ups (clean subset gaps — never over-records — tracked):**
...
`_recurse_switch_list_body`); the list body is currently skipped (clean
subset).  (b) Cmd/Str **head names inside a deeper substitution**
(`[[x] hi]` → main also records the literal `[x]` head name) — the
recursion's head filter skips `Cmd`/`Str` heads; an esoteric subset gap.
(c) optional behaviour-preserving folds of `analyse_body` / `rename.rs`'s
body re-segmentation onto the descent (not needed for L1, which strips 1 +
3 close).
