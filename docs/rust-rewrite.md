...
**Remaining follow-ups (esoteric clean-subset gaps — never over-records):**
(a) Cmd/Str **head names inside a deeper substitution** (`[[x] hi]` → main
also records the literal `[x]` head name) — the recursion's head filter
skips `Cmd`/`Str` heads.  (b) optional behaviour-preserving folds of
