...
|---|---|---|
| 1 | `C41-default-on-followups-alias-chains` | Small fixpoint pass after the main walk (~80 LOC + tests).  Closes the ``command_aliases`` supplement guard.  Mirrors ``core/analysis/_analyser/_handlers.py::_resolve_alias_chain``. |
| 2 | `C41-default-on-followups-switch-regexp` | Moderate body walker addition to ``handle_regex_pattern_capture``.  Partially closes the ``regex_patterns`` unconditional override.  Recognise ``switch ?-flag…? -regexp ARG BODY`` and segment the braced body to emit a ``RegexPattern`` per arm. |
| 3 | `C41-default-on-followups-regex-vars` | Moderate.  Threads a ``const_strings`` map through ``analyser::state::Analyser`` so ``regexp $p`` resolves to the literal stored in ``set p "..."``.  Finishes the ``regex_patterns`` retirement (combined with #2). |
| 4 | `C41-default-on-followups-recovery` | Moderate segmenter heuristic port.  Partially closes the ``command_invocations`` merge.  Lift Python's ``_find_command_break_offset`` from ``core/parsing/command_segmenter.py``. |
| 5 | `C41-default-on-followups-noop-guards` | Audit + cleanup.  Probe whether the conservative-fallback guards ever fire on the corpus / sample-fixtures; delete the ones that don't. |
| 6 | `C41-default-on-followups-param-traits` (full) | 525 LOC — the original handoff item.  Adds ``ProcDef.param_traits`` populated by a body walk recording each parameter's role.  Unblocks the optimiser passes that consult traits.  Plumb through PyO3 + the materialiser. |
| 7 | `C41-default-on-followups-postpass` (= ``C42``) | Largest analyser-followup chunk.  Port ``run_compiler_checks`` integration (W110 / W220 / W304 / W101 / W105 / W116 / W117) so ``rust.diagnostics = python.diagnostics`` retires.  Depends on the IR / CFG / SSA pipeline being reachable from the Rust analyser — already true via ``compilation_unit::CompilationUnit``. |
| 8 | `C41-default-on-followups-structural` | Naturally closes with the ``S*`` LSP-server port.  Lower priority for direct work — the structural-triple override survives only because Python consumers depend on object identity; rewriting them in Rust eliminates the question. |
...
The first seven rows are the remaining C41 followup work and
should land before the wider Python-retirement chunks
(`S*`, `VM*`, …) so the analyser hybrid retires first.  Item
1 is small enough to land in a single session; items 2-7 are
PR-sized.
