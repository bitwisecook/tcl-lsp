...
| C22i  | **iRules string operators + list membership.** Completes the `BinOp::Contains` / `StartsWith` / `EndsWith` / `StrEquals` / `MatchesGlob` / `In` / `Ni` surface in `tcl_expr_eval.rs` under the `f5-irules` dialect. New module-private helpers: `strip_string_delimiters`, `eval_as_string` (mirrors Python `_eval_as_string` — strip delimiters on ExprString, raw text on ExprLiteral, env lookup on ExprVar, fold-via-eval for numerics), `split_tcl_list` (space-separated + brace-grouped + quoted), and `glob_match` (inline matcher for `*`/`?`/`[abc]`, avoids a `glob` crate dependency). `matches_regex` stays deferred (needs a regex engine). Test helpers `eval_irules(expr)` / `eval_irules_env(expr, env)` parse under `Some("f5-irules")`. 12 new unit tests. | landed |
