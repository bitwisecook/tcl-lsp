...
|---|---|---|
| 1 | `C41-default-on-followups-switch-regexp` | Moderate body walker addition to ``handle_regex_pattern_capture``.  Partially closes the ``regex_patterns`` unconditional override.  Recognise ``switch ?-flag…? -regexp ARG BODY`` and segment the braced body to emit a ``RegexPattern`` per arm. |
| 2 | `C41-default-on-followups-regex-vars` | Moderate.  Threads a ``const_strings`` map through ``analyser::state::Analyser`` so ``regexp $p`` resolves to the literal stored in ``set p "..."``.  Finishes the ``regex_patterns`` retirement (combined with #1). |
| 3 | `C41-default-on-followups-recovery` | Moderate segmenter heuristic port.  Partially closes the ``command_invocations`` merge.  Lift Python's ``_find_command_break_offset`` from ``core/parsing/command_segmenter.py``. |
| 4 | `C41-default-on-followups-noop-guards` | Audit + cleanup.  Probe whether the conservative-fallback guards ever fire on the corpus / sample-fixtures; delete the ones that don't. |
| 5 | `C41-default-on-followups-param-traits` (full) | 525 LOC — the original handoff item.  Adds ``ProcDef.param_traits`` populated by a body walk recording each parameter's role.  Unblocks the optimiser passes that consult traits.  Plumb through PyO3 + the materialiser. |
...
The first six rows are the remaining C41 followup work and
