...
| C41-default-on | **Flip the C41 default.** Mirrors the `C40-default-on` shape: `core.analysis._analyser.analyse` dispatches to Rust by default whenever the `tcl_lsp_rust` binding is importable; the gate becomes `TCL_LSP_RUST_ANALYSER=0` opt-out. Reuses the existing `rust_shim_enabled` helper with `default=True`. **Status — blocked on Rust-side body walkers.** Substantial preparation landed across `C41-default-on-1` … `C41-default-on-9`: (a) the two Python parity bugs (`::::ns::foo` double-prefix quirk; `nested_namespace_eval` outer-namespace-drop) are fixed in `_proc.py` + `_oo.py` plus the Rust-side mirror of the `oo::define ::ns::Cls` bug in `analyser/handlers.rs`; (b) the differential corpus is expanded from 40 shape-only fixtures to 92 (53 shape-only + 34 tcl field-parity + 5 iRules field-parity + 4 dispatcher gating), with a strict `_compare_fields` helper comparing the `all_procs` / `all_classes` / `all_variables` qualified-name sets, the bare `name` / `params` / `name_range` / `body_range`, the `methods` / `class_methods` / `superclasses` / `mixins` per class, and the recursive scope-tree shape; (c) the Python `_materialise_rust_analysis` is extended to populate every side-channel field the Rust dict already carries (`command_invocations` with Python-side `resolved_qualified_name` resolution, `command_aliases`, `package_requires`, `source_targets`, `namespace_imports`); (d) a hybrid `_merge_rust_with_python_supplement` step in the `analyse` shim merges Rust structural data with a Python supplementary pass that fills in the side channels Rust doesn't emit yet (`source_targets` / `regex_patterns` / `stub_commands` / …) plus `run_compiler_checks` post-pass diagnostics — opt-in users (`TCL_LSP_RUST_ANALYSER=1`) get a fully populated `AnalysisResult` via the hybrid; (e) `analyser/handlers.rs::handle_for_command` and `handle_foreach_command` now recurse into init / next / body so loop-local variables land in the enclosing scope. **The polarity flip itself remains blocked** on two structural gaps the hybrid can't paper over while preserving the speedup: (1) **Generic body iteration via `ArgRole::Body`** is not wired into `analyser/commands.rs::process_command`; `if` / `while` / `when` / `oo` method bodies are not walked, so var binding, command-invocation tracking, and CFG/SSA-driven diagnostics inside any conditional / event / method body all rely on the supplementary Python pass. Threading a `CommandRegistry` reference through the `Analyser` state and wiring up the registry-driven body loop is the right port (mirrors the Python `iter_body_arguments` loop in `_AnalyserCommandsMixin._process_command`). (2) **Class body parsing gaps**: `oo::abstract` / `oo::singleton` / `oo::configurable` metaclasses are not recognised by the Rust `handle_oo_class_command` cmd-name guard, and `class_def.variables` + per-method `params` are not extracted. (3) Without (1) and (2), running with `default=True` *does* pass the test suite under the hybrid, but at the cost of always running both passes — measured runtime is >5× the default-OFF baseline. The chunk plan's "don't ship LSP regressions" rule applies to performance regressions too, so the polarity flip waits until the body walker port lands and the hybrid can fall back to a Python-skip path. Recommended next chunk: `C41-default-on-body-walkers` — register a `CommandRegistry` field on `Analyser`, port `iter_body_arguments` to `process_command`, port `handle_if_command` / `handle_while_command` / `handle_when_command`, and remove the corresponding Python supplements from `_merge_rust_with_python_supplement`. After that lands, the polarity flip becomes the targeted single-line change the chunk-log originally envisioned. | blocked (corpus + bug-fix + materialiser + hybrid landed; flip blocked on Rust-side body-walker ports — performance regression otherwise) |
