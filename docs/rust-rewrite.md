...
| C17   | **Emitter main loop + public API (MVP).** `core/compiler/codegen/_emitter.py`, `_bytecoded.py` → `rust/tcl-compiler/src/codegen/emitter/` (directory module). Split by responsibility rather than reproducing Python's mixin composition: `ordering.rs` (`fold_const_branch`, `linearise` RPO + dead-branch elimination, `collect_loop_body`, `reorder_bottom_tested`, `build_loop_context`); `terminator.rs` (`emit_term` for CFG Goto/Branch/Return, `emit_proc_return` with startCommand wrapping + dead-code jumps, `try_emit_jump_table` stub); `proc_defs.rs` (`is_static_proc`, `emit_one_proc_def`, `emit_pending_proc_defs`, `flush_proc_defs`); `loop_blocks.rs` (`detect_foreach` + `ForeachInfo` — per-block handlers deferred); `try_blocks.rs` (`detect_try_finally` CFG pattern detection feeding `emit_try_finally_inline` from C16); `generate.rs` (top-level dispatcher + `GenerateState` struct, loop-end handling, join-block pops, arm-result preservation); `bytecoded.rs` (registry-backed codegen hook dispatch stub). Public API: `codegen_function`, `codegen_module` re-exported from `codegen::`. MVP handles straight-line code, if/else diamonds, proc returns, constant branch folding, try/finally CFG patterns. Deferred to follow-up chunks: foreach opcode compilation, switch jump tables, for-init/while startCommand wrapping, complex-foreach bodies, registry codegen hooks. 9 new integration tests under `tests/codegen_integration.rs` + per-module unit tests (~490 total). | landed |
