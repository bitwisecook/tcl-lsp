...
| C*    | **Compiler migration — deferred analysis bodies.** Aggregate row covering deferred bodies on landed chunks.  **Audited 2026-05-07** against the actual Rust tree.  Items closed since the original aggregate was written: full shimmer use-site / phi-shimmer / thunking detection (C27d — see C27d row); interprocedural summary-building pipeline (C28 — see C28 row + C30 narrative for C28x / C28x-return / C28x-param-traits); taint propagation + path-concat + iRules sinks IRULE3001-3004 / 3101 / 3102 (C29 — see C29 row); ten optimiser passes (C30a-j and follow-up sub-strips — see C30 row); `connection_scope` (`rust/tcl-compiler/src/connection_scope.rs` lands `cross_event_defs` / `cross_event_imports` / `racy_static_defs` and is consumed by W220 in `analyser/diagnostics.rs`).  **Items still genuinely outstanding:** (a) C29 follow-up — URI-split / IRULE3103 (`core/compiler/taint/_uri_split.py`, ~959 LOC) plus T103 / T104 / T105 sinks; (b) C28 follow-up — `irules_flow.py` port (`core/compiler/irules_flow.py`, ~1634 LOC) — IRULE1005-1008 / 1201 / 1202 / 4004 / 5002 / 5004; (c) C43 — registry-driven codegen + lowering hooks port (now unblocked since ARCH1 / ARCH2 landed). Each lands as its own row when picked up. | partially landed (closed sub-rows referenced; outstanding sub-rows now listed explicitly) |
