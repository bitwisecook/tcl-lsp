...
**Follow-up strips (tracked):** (2) var/`"quote"`/compound-head fidelity
(record word_piece-normalised heads to fully match main's
command_invocations); (3) fold `analyse_body`'s body segmentation onto
`descend_command` (the `_recurse_body_arguments` mirror) once a
behaviour-identity differential vs `segment_commands_with_offset_and_config`
is pinned; (4) the `ranges` accessors when their faithful Rust callers
land (irules quick-fixes / `_style.py` W108).
...
| — | `CST-CONSUMERS` (candidate a — give the CST descent / ranges accessors a production caller) | **Opened 2026-06-04 (the #538 L1 finding); strip 1 LANDED.**  `descend_token` / `descend_command` and the `tcl-lexer::ranges` accessors landed with the CST but had **no production caller**.  **Strip 1:** rebased `analyser/commands.rs::record_invocations_from_cmd_token` (nested cmd-sub invocation recording, consumed by the LSP server's references / call-hierarchy via `analysis.command_invocations`) off its hand-rolled first-head-only scan onto `descend_token` + `segments_from_tree` — the **first production caller of the descent**.  Fixes a divergence from main: `set x [foo; bar]` now records `{foo, bar}` (was only `foo`; main records both), mirroring `_recurse_nested_commands`.  Discriminating test FAILS pre-rebase.  Follow-ups: var/quote/compound-head word_piece fidelity; fold `analyse_body` onto `descend_command`; wire the `ranges` accessors when their faithful callers (irules quick-fixes / `_style.py` W108) land.  See the `CST-CONSUMERS` section. |
