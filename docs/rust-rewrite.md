...
| `core/bigip/` (BIG-IP object model, APL parser, SCF/config) | 41.5K | new `tcl-bigip` | 🔴 | Largest unported block. Domain data + APL (iApp presentation language) parser + config model. Needed by iRule analysis, XC translation, semantic-token enrichment. **LSP-level slice landed (not the object model):** `tcl-lsp-core::bigip` ports the config-outline path — the native server detects a canonical `bigip.conf` basename on `did_open`, builds a `module → kind → object` document outline from the stanza tree over the existing `tcl-registry::bigip` spec data (longest-prefix object-type resolution; nameless singletons fall back to their kind label, #534), and suppresses general Tcl diagnostics on BIG-IP config text (#571). The full object model / APL parser / SCF config below remain unported, so the row stays 🔴. |
