...
|---|---|---|---|---|
| landed | `b66f8f9d` (#306) + `ceb190fc` | noqa next-line suppression covers orphaned comments and the noqa-before-comment shape; suppression dispatched from `analyse_chunked` and `analyse_commands`. **Audited 2026-05-08:** ported as a new `parse_noqa_line_suppressions` helper in `rust/tcl-compiler/src/analyser/utils.rs` plus a `merge_noqa_line_suppressions` merger in `analyser/state.rs` that runs alongside the file-level `parse_file_suppression` pre-scan in `analyse` / `analyse_chunked` / `analyse_commands` (mirrors the inline merge block Python adds at `_core.py:395-405` post `b66f8f9d` + the duplicate at `_core.py:248-256` / `_core.py:289-297` post `ceb190fc`).  The new helper is additive — it does not replace `apply_preceding_noqa`, which still attributes per-comment-attached noqa codes to the following command's line range; together they cover both the orphan / before-comment cases and the inline form.  7 unit tests on the helper + 3 integration tests on the wiring through each entry point. | `SYNC-JUN-W306-noqa` |
