...
| **EXP\*** | **Compiler explorer rewrite** (`explorer/` — 23 files / 7.4K LOC).  Replace the Pyodide-based GUI with a Rust → WebAssembly web app (using `wasm-bindgen` + the existing `tcl-compiler` / `tcl-vm` crates compiled to `wasm32-unknown-unknown`).  No Python at runtime.  Tests port from `test_compiler_explorer*.py` (~68 cases). | in progress — thin `tcl-explorer` aggregator + serialiser (30/32 contract keys; `greentree`/`eventOrder` blocked on unported backends, `wasm`/`wasmOptimised` degraded to `null` until the emitter chunk), differential parity harness vs Python `serialise_result`, `tcl explore` CLI verb (`--json` / summary), feature-gated `ratatui` TUI (`tcl explore --tui`), the `tcl-explorer-wasm` `cdylib` + Pyodide→wasm-bindgen `worker.js` swap, and **in-webview WASM compile bundled into the VS Code / JetBrains panels** (no LSP `executeCommand` roundtrip; degrades to host-brokered compile when the module is absent). Remaining: rich `--text` ANSI renderer, the 3 blocked keys, and the WASM emitter (`tcl-vm`). |
