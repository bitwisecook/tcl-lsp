...
| landed | `6c7a7c42` | `var_escape`: any non-literal `upvar LEVEL` (i.e. anything that isn't a decimal digit literal or `#N`) is treated as dynamic, blocking slot lifting. Touches `_propagation.py` + `_cfg_propagation.py`. **Audited 2026-05-08:** ported as a new `is_dynamic_upvar_level` helper in `rust/tcl-compiler/src/var_escape/helpers.rs` consumed by both `var_escape/handlers.rs::handle_upvar` and `var_escape/cfg_propagation/handlers.rs::handle_upvar`.  The dynamic-level branch now also calls `record_unbounded_upvar` so the interprocedural pass forces every caller to spill its locals (matching the Python `state.record_unbounded_upvar()` post `mark_pessimistic`).  9 new unit tests covering `[cmd]`, `"#${n}"`, `$::ns::level`, `#$n` shapes across both handlers + the helper's own predicate. | `SYNC-JUN-VARESC-dynamic-upvar` |
