...
rust/
  ...
  tcl-fuzz/               differential fuzzer (seeded generator + any-backend-pair harness — tclvm / tclsh / runtime-rust — + findings)
...
| Differential fuzzer | `tcl-fuzz` | 🟢 | campaign runner + seeded generator + findings registry land (`tclvm` vs `tclsh`); generator grammar broadened to procs/namespaces/dict/`catch`/`try`/`switch` (RT-VM-gated work done, 1.5 K-iter campaign @ 0 findings); WASM-runnability arm landed (`wasm-check`: compile→`wasmtime`, 600-program campaign clean); WASM **value**-differential arms cover both in-process codegen (`wasm-diff`, with fuel-bounded `WasmHang` detection) and a whole-program module linked against the real `runtime/rust` WASI artefact (`linked-wasm-diff`); the subprocess harness is **backend-pair generic** (#1313): `run`/`replay`/`summary` pair any two of `tclvm` / `tclsh` / `runtime-rust`, while `characterise` classifies all three together; every paired run probes the engines' `[info patchlevel]`, warns when the reported patchlevels differ, and records backend identities and versions with its findings; the eBPF arm executes registry-described socket filters through BPF-Tcl lowering, eBPF emission, and the userspace rbpf VM, then compares that VM result with the descriptor-derived oracle (it is not a kernel-load or kernel-execution check) → **TOOL-FUZZ** |
