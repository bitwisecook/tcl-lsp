...
| landed (delete) | n/a (pre-existing breakage from PR #241) | `tests/test_rust_analyser_differential.py` was broken: line 37's `from core.analysis._analyser import _materialise_rust_analysis` raised `ImportError`. The symbol was deleted when `__init__.py` was simplified at `cd7a8441`. The whole differential harness was silently uncovered between 2026-04-30 and 2026-05-08. **Audited 2026-05-08:** deleted outright (the `-delete` arm of the row's two options). Matches the established direction of the C41 family closing with `S*` — the Rust analyser is now consumed only via the `S*` LSP-server port path, so the differential harness has no live consumer to defend. Restoring a `_materialise_rust_analysis` shim was rejected because the symbol it would re-introduce was the very thing PR #241 simplified away; mirroring it would re-couple the analyser module to the Rust binding under a name the rest of the tree no longer references.  No production code depended on the test file; no other call sites referenced `_materialise_rust_analysis`. | `SYNC-JUN-DIFF-harness-delete` |
...
(`SYNC-JUN-DIFF-harness-delete`, landed 2026-05-08) was tracked
alongside but was independent of the `main`-delta audit and
landed on its own schedule.  Each row should
