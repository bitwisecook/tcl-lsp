"""Cross-cutting BIG-IP data-classes shared by every F5 module.

Pools, profiles, monitors, persistence, iRules, virtual servers,
policies, and the generic-stanza fallback all live here.  Per-F5-
module dataclasses (``BigipLtm*`` / ``BigipNet*`` / …) live in
sibling submodules and import from this one only when a field
type crosses module boundaries (rare; almost everything composes
by string PathRef).
"""
