...
# IPv4 / IPv6 literal regexes generated by rgxg (Debian: ``apt install
# rgxg``) so they accept exactly the addresses ipaddress.IPv4Address /
# IPv6Address would parse — no permissive ``\d{1,3}`` slop, and the
# IPv6 pattern correctly handles the ``::`` compressed form and the
# IPv4-embedded form (``::ffff:1.2.3.4``) that's common in F5
# trailers.  The patterns themselves are facts about RFC 4291 / 5952
# grammar; tests/test_f5_regex_drift.py re-runs rgxg and verifies
# these strings match its current output to catch drift.
#
# Generated:
#   rgxg cidr 0.0.0.0/0
#   rgxg cidr ::/0
...
# Boundary guards prevent matches inside larger tokens (e.g. CIDR
# suffix ``/24``, port ``:80``, or a hex blob of cert data).
_IPV4_RE = re.compile(r"(?<![\w.])" + _RGXG_V4 + r"(?![\w.])")
_IPV6_RE = re.compile(r"(?<![A-Za-z0-9:])" + _RGXG_V6 + r"(?![A-Za-z0-9:])")
...
def collect_addresses(
    ...
    for match in _IPV4_RE.finditer(text):
        addr = _normalise_ipv4_token(match.group(0))
        if addr is None:
            continue
    ...
    # Walk v6 in left-to-right order so an IPv4-embedded form
    # ``::ffff:1.2.3.4`` registers as one v6 hit rather than a v4 hit
    # plus a junk v6 prefix.
    v4_starts = {m.start() for m in _IPV4_RE.finditer(text)}
    consumed_v4_at: set[int] = set()
    for match in _IPV6_RE.finditer(text):
        addr = _normalise_ipv6_token(match.group(0))
        if addr is None:
            continue
        if _should_skip(addr, remap_private=remap_private):
            continue
        seen_v6.setdefault(addr, None)
        # If the v6 match swallows a v4 literal (IPv4-embedded form),
        # mark the v4 start so we drop it from the v4 set.
        for s in v4_starts:
            if match.start() <= s < match.end():
                consumed_v4_at.add(s)

    if consumed_v4_at:
        # Re-derive v4 with embedded matches excluded.
        v4_again: dict[ipaddress.IPv4Address, None] = {}
        for match in _IPV4_RE.finditer(text):
            if match.start() in consumed_v4_at:
                continue
            addr = _normalise_ipv4_token(match.group(0))
            if addr is None:
                continue
