...
:mod:`core.bigip.query.output` and are reachable from the CLI via
``f5 q --render NAME`` and from Python via
:func:`core.bigip.query.render`.
...
and ``@_register`` in :mod:`core.bigip.query.builtins`:

.. code-block:: python

    from core.bigip.query.renderers import renderer
...
The first import of :mod:`core.bigip.query` triggers the renderer
...
- *values* — the same flattened list the rest of
  :mod:`~core.bigip.query.output` sees.  Renderers SHOULD treat unknown
...
Renderers raise :class:`~core.bigip.query.errors.RendererError` for
...
def bind_render_sources(sources: dict[str, str] | None):
    ...
    Used by :meth:`core.bigip.query.api.QueryRun.render` to surface
...
def render(name: str, values: list[Any], **opts: Any) -> str:
    ...
    Raises :class:`~core.bigip.query.errors.RendererError` if *name* is
