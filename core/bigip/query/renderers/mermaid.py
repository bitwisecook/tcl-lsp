...
- when every value is an :class:`~core.bigip.query.values.ObjectRef`,
...
- otherwise emits a generic ``graph LR`` flowchart: one node per value
  (rendered via :func:`~core.bigip.query.output._cell_str`),
...
def _recover_source_text(ref: ObjectRef) -> str | None:
    ...
    1. The render-time :data:`~core.bigip.query.renderers.RENDER_SOURCES`
