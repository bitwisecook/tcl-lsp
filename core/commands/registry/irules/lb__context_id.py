# Generated from F5 iRules reference documentation -- do not edit manually.
"""LB::context_id -- Assign the current connection to a named context."""

from __future__ import annotations

from ....compiler.side_effects import ConnectionSide, SideEffect, SideEffectTarget
from .._base import CommandDef
from ..models import CommandSpec, FormKind, FormSpec, HoverSnippet, ValidationSpec
from ..namespace_models import EventRequires
from ..signatures import Arity
from ._base import _IRULES_ONLY, register

_SOURCE = "https://clouddocs.f5.com/api/irules/LB__context_id.html"


@register
class LbContextIdCommand(CommandDef):
    name = "LB::context_id"

    @classmethod
    def spec(cls) -> CommandSpec:
        return CommandSpec(
            name="LB::context_id",
            dialects=_IRULES_ONLY,
            hover=HoverSnippet(
                summary="Assign the current connection to a named context.",
                synopsis=("LB::context_id",),
                snippet="Assign the current connection to a named context.",
                source=_SOURCE,
            ),
            forms=(
                FormSpec(
                    kind=FormKind.DEFAULT,
                    synopsis="LB::context_id",
                ),
            ),
            validation=ValidationSpec(
                arity=Arity(),
            ),
            event_requires=EventRequires(),
            side_effect_hints=(
                SideEffect(
                    target=SideEffectTarget.POOL_SELECTION,
                    reads=True,
                    connection_side=ConnectionSide.SERVER,
                ),
            ),
        )
