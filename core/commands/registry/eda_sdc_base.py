...
def sdc_base_command_specs() -> tuple[CommandSpec, ...]:
    ...
    return (
        # --- Clock constraints ---
        _sdc("create_clock", "Create a clock object.",
             "create_clock ?-period period? ?-name name? ?-waveform edge_list? ?-add? ?source_objects?",
             Arity(1)),
        _sdc("create_generated_clock", "Create a generated clock object.",
        ...
        _sdc("set_output_delay", "Set output delay on ports.",
        ...
        _sdc("set_input_transition", "Set input transition time on ports.",
             "set_input_transition ?-rise | -fall? ?-min | -max? transition port_list"),
        _sdc("set_load", "Set capacitive load on nets or ports.",
             "set_load ?-pin_load | -wire_load? ?-min | -max? ?-subtract_pin_load? value objects"),
        _sdc("set_driving_cell", "Set the driving cell for input ports.",
        ...
        # --- Timing exceptions ---
        _sdc("set_false_path", "Define a false timing path.",
        ...
        _sdc("all_registers", "Return all register cells.",
        ...
        _sdc("all_fanout", "Return all fanout of a pin/port.",
             "all_fanout ?-from objects? ?-flat? ?-endpoints_only? ?-only_cells?"),
        _sdc("all_fanin", "Return all fanin of a pin/port.",
             "all_fanin ?-to objects? ?-flat? ?-startpoints_only? ?-only_cells?"),

        # --- Collection utilities ---
        _sdc("foreach_in_collection", "Iterate over objects in a collection.",
             "foreach_in_collection var collection body", Arity(3, 3)),
        _sdc("get_object_name", "Return the name of an object.",
             "get_object_name object", Arity(1, 1)),
        _sdc("sizeof_collection", "Return the size of a collection.",
             "sizeof_collection collection", Arity(1, 1)),
        _sdc("filter_collection", "Filter a collection by an expression.",
             "filter_collection collection filter_expr", Arity(2, 2)),

        # --- Analysis & reporting ---
        _sdc("report_timing", "Report timing paths.",
        ...
        _sdc("report_power", "Report power consumption.",
             "report_power ?-hierarchy? ?-verbose?"),
        _sdc("report_constraint", "Report design constraints.",
             "report_constraint ?-all_violators? ?-max_delay? ?-min_delay?"),
        _sdc("report_clock", "Report clock definitions.", "report_clock ?clock_list?"),
        _sdc("report_clock_timing", "Report clock timing characteristics.",
             "report_clock_timing ?-type type?"),
        _sdc("check_timing", "Check for unconstrained paths and issues.",
             "check_timing ?-verbose? ?-override_defaults list?"),

        # --- Misc SDC ---
        _sdc("current_design", "Get or set the current design.",
             "current_design ?design_name?"),
        _sdc("link_design", "Link the design to its library cells.",
             "link_design ?-keep_sub_designs?"),
        _sdc("set_units", "Set the unit specifications.",
             "set_units ?-time unit? ?-capacitance unit? ?-resistance unit? ?-voltage unit? ?-current unit? ?-power unit?"),
        _sdc("define_proc_attributes", "Define attributes for a procedure.",
             "define_proc_attributes proc_name ?-info string?", Arity(1)),
        _sdc("set_case_analysis", "Set constant case analysis on a port/pin.",
             "set_case_analysis value port_pin_list"),
        _sdc("set_disable_timing", "Disable timing arcs.",
             "set_disable_timing ?-from from_pin? ?-to to_pin? object_list"),
        _sdc("set_ideal_network", "Mark a network as ideal.",
             "set_ideal_network ?-no_propagate? object_list"),
        _sdc("group_path", "Group paths for multi-scenario analysis.",
             "group_path ?-name group_name? ?-from from_list? ?-through through_list? ?-to to_list? ?-weight weight? ?-default?"),
        _sdc("set_wire_load_model", "Set the wire load model.",
             "set_wire_load_model -name model_name ?-library lib? ?object_list?"),
        _sdc("set_wire_load_mode", "Set the wire load mode.",
             "set_wire_load_mode mode", Arity(1, 1)),
        _sdc("set_dont_touch", "Prevent optimization of cells/nets.",
             "set_dont_touch object_list"),
        _sdc("set_dont_use", "Prevent use of library cells.",
             "set_dont_use lib_cell_list"),
        _sdc("set_size_only", "Mark cells as size-only (no restructuring).",
             "set_size_only object_list ?-all_instances?"),
        _sdc("set_ideal_latency", "Set ideal latency on a clock network.",
