...
def _materialise_rust_summaries(
    ...
    Note: the Rust ``param_traits`` field uses a different enum
    (``Passthrough`` / ``UsedInCondition`` / ``ForwardedToCallee`` /
    ``Unused``) from Python's registry-level ``ProcArgTrait``
    (``EVAL`` / ``BODY`` / ``EXPR`` / …). The two carry semantically
    different information so this shim drops the Rust traits and
    leaves ``ProcSummary.param_traits`` empty — callers that need
    the Python-shape traits should invoke ``infer_param_traits``
    separately. (Bridging the two trait vocabularies is deferred
    work tracked in ``docs/rust-optimiser-parity.md``.)
    """
    procedures: dict[str, ProcSummary] = {}
...
def analyse_interprocedural_source(source: str) -> InterproceduralAnalysis:
    ...
    implementation; otherwise uses the Python pipeline. The gate is
    opt-in so the ~1000 existing interprocedural / taint / shimmer
    tests don't regress during parity verification — once parity is
    confirmed, the default should flip (with ``TCL_LSP_PY_INTERPROC``
    as the Python escape hatch).
    """
    if _rust_interprocedural_summaries is not None and os.environ.get(
        ...
        try:
            ...
            log.debug(
                "Rust interprocedural delegation failed, falling back", exc_info=True
            )
