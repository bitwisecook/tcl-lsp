...
namespace eval ::tclpkg::cmd_venv {
    ...
    proc cmd_info {args_list} {
        set path [_parse_path $args_list]
        set code [catch { set config [::tclpkg::venv::read_config $path] } err]
        if {$code} {
            puts stderr "error: $err"
            exit 1
        }
    ...
    proc cmd_list {args_list} {
        ...
        foreach candidate $candidates {
            ...
            if {[file isfile $cfg]} {
                ...
                set tcl_ver ""
                catch { set tcl_ver [dict get $config tcl_version] }
                set prompt [file tail $candidate]
                catch { set prompt [dict get $config prompt] }
                set active ""
    ...
    proc cmd_run {args_list} {
        ...
        set existing ""
        catch { set existing $::env(TCLLIBPATH) }
