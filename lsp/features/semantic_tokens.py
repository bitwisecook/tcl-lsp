...
# Bare tail names (e.g. "test" from "tcltest::test") are included so that
# commands imported via ``namespace import ::pkg::*`` are highlighted.
_REGISTRY_NAMES = frozenset(REGISTRY.command_names())
_KEYWORD_TAILS = frozenset(name.rsplit("::", 1)[-1] for name in _REGISTRY_NAMES if "::" in name)
_KEYWORDS = (
    _REGISTRY_NAMES
    | _KEYWORD_TAILS
    | frozenset(
