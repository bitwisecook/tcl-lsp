...
iRules security: IRULE3001 (Tainted data in HTTP response body), IRULE3002 (Tainted data in HTTP header or cookie value), IRULE3003 (Tainted data in `log` command — log injection risk), IRULE3101 (`HTTP::uri`/`HTTP::path` set to value not provably starting with `/`), IRULE3102 (`HTTP::path`/`HTTP::uri`/`HTTP::query` getter used without `-normalized`)
