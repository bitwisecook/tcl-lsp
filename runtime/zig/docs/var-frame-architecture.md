...
| 7: Compile-time slot resolution | **landed (runtime API)** | Per-frame `locals_array` of fixed capacity (`LOCALS_ARRAY_CAP = 16` slots) provides direct-indexed local storage parallel to the existing hash-keyed store.  Exports `frame_local_at(idx) i32` and `frame_local_set_at(idx, value) i32` for codegen-emitted compiled-proc accesses; the slot drain on `frame_pop` releases retained values.  Codegen-side wire-up is the follow-up: Python `ProcLocal` records carry the resolved slot for each compile-time-known scalar local, and the bytecode emitter routes simple reads/writes through the indexed accessors instead of the name-keyed `var_resolve` / `local_set`.  Until the codegen lands, the indexed slots are unused; nothing regresses because the hash-keyed path remains the primary store. |
| 8: `info frame` rich metadata | **landed** | Per-frame `FrameInfo` slots (`type`, `script_obj`, `line`, `cmd_text`, `proc_name`) populated by `eval_proc_call_bucket` / `eval_apply` immediately after `frame_push`.  Released on `frame_pop`.  `info frame ?N?` exposes them — no-arg returns the current depth (1-based, counting the synthetic top-level frame); `info frame N` returns a `{type X cmd Y proc Z line N}` dict-list for frame N.  Negative offsets count from current per Tcl 9 convention.  Compiled-proc prologues currently don't populate the FrameInfo (codegen-side hookup is the follow-up); only interpreted procs and `apply` surface metadata today, which covers `info frame` for the interpreted dispatch path. |
| 9: Cross-thread / cross-interp variable channels | **landed (typed shapes + result transfer)** | The typed `Interp` handle the design doc calls for is already what `interp_root` / `child_create` build (`extern struct`).  This phase adds `CrossInterpLink` as the typed shape for a variable in interp A that aliases a variable in interp B (`{target_interp, target_name}`), and `transfer_result(from, to, value)` for moving a TclObj across the interp boundary with refcount bookkeeping.  Cross-interp variable-link resolution requires the per-name `Variable` record refactor the design doc describes for Phase 1 (we used the simpler scope-keyed-name approach instead) — once that lands `var_resolve` will walk `Variable.link` chains across interp boundaries via `CrossInterpLink.target_interp`.  Channels (file descriptors) live in the IO layer and remain single-instance for the WASM runtime today; cross-interp channel transfer is not yet meaningful. |
| 10: Coroutine-aware frame stacks | **landed (API)** | `FrameContext` aggregates the per-frame slot arrays (`frame_stack` / `frame_capacity` / `frame_ns` / `frame_argv` / `frame_info` / depth) into a single snapshot type.  `frame_context_save` captures the live state; `frame_context_restore` writes it back; `frame_context_reset` zeros the depth for an isolated eval.  No driver currently uses the API — the v1 segment-based coroutine driver evaluates each segment inline in the caller's frame stack so it doesn't need isolation, and the asyncify path handles its own wasm-side state via `wasm-opt --asyncify`.  The API is the foundation for proc-level coroutines that need real frame isolation, plus phase 9's `interp transfer`. |
