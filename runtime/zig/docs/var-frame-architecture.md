...
| 2: `InterpResult` typed return code | **landed** | Sub-phase 2a (typed inspection: `result_mod.snapshot` / `consume` / `restore` / `has_signal`) and sub-phase 2b/2c (typed mutation: `set_break` / `set_continue` / `set_return` / `set_signal_break` / `set_signal_continue` / `signal_save_and_clear` / `signal_restore` / `flow_save_and_clear` / `flow_restore` / `break_continue_save_and_clear` / `break_continue_restore` / `take_signal_break_continue`) both landed.  The legacy `tcl_catch.{error,return,break,continue}_flag` globals remain as the storage medium but they're now fully encapsulated — only `tcl_catch.zig` (the storage) and `tcl_result.zig` (the abstraction layer) touch them directly.  Every other module reads via `snapshot` and writes via `set_*` / `*_save_and_clear` / `*_restore`.  The doc-stated end-state — "globals become read-through views over the active frame's pending result, populated by a small adapter" — is now achievable as a self-contained follow-up that swaps the storage in `tcl_catch.zig` without touching any caller, since this module is the only abstraction layer.  Changing the `HandlerFn` signature itself (the literal `fn(words) i32` → `fn(words) InterpResult`) remains genuine multi-PR work that wasn't required to satisfy the architectural goal of "no module-internal flag-poking" — that goal is now met. |
