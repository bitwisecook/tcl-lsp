...
| 2: `InterpResult` typed return code | **2a landed**; 2b/2c deferred | Sub-phase 2a (typed inspection at every site via `result_mod.snapshot`/`consume`) is in. Sub-phases 2b (change `HandlerFn` to return `InterpResult`) and 2c (delete legacy globals) genuinely span 50+ command files and need a dedicated migration window. |
