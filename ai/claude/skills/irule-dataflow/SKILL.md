...
name: irule-dataflow
description: >
  Analyze and visualize data flow in an iRule or Tcl file.
  Extracts def-use chains, memory aliases, and data-flow edges
  from the compiler SSA, then generates a Mermaid diagram showing
  how variables flow through the program.
allowed-tools: Bash, Read
---

# Data Flow Analysis

Analyze data flow in an iRule or Tcl file using compiler SSA analysis.
...
3. Extract memory alias information:
   ...
   ```
4. Using the def-use chains and alias data, generate a Mermaid diagram and analysis
...
2. A structured analysis including:
   ...
   - **Dead definitions**: Variables defined but never used (optimization opportunities)
   ...
   - **Potential issues**: Uninitialized reads, unnecessary stores, alias confusion
