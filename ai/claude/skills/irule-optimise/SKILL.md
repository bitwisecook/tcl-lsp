...
Apply the optimiser's rewrites to an iRule and explain each one.
...
4. For each optimisation, one or two sentences: why it preserves behaviour,
   and what it gains on the hot path. A grouped item (constant propagation
   O100–O105 whose dead store O109 was then eliminated) is one
   transformation — explain it as one.
