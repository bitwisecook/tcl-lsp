...
iRules: IRULE1001 (Command invalid or ineffective in this iRules event), IRULE1002 (Unknown iRules event name), IRULE1003 (Deprecated iRules event), IRULE1004 (Explicit event priority required by the registry policy), IRULE1005 (Data event without its required registered collection operation), IRULE1006 (Payload access without its required registered collection operation), IRULE1007 (Collection without its required registered release operation on the same connection side), IRULE1008 (`*::release` without a matching `*::collect` on the same connection side), IRULE1201 (HTTP command used after `HTTP::respond`/`HTTP::redirect`), IRULE1202 (Multiple `HTTP::respond`/`HTTP::redirect` on different branches), IRULE2001 (Deprecated `matchclass` — use `class match` instead), IRULE2002 (Deprecated iRules command), IRULE2003 (Unsafe iRules command), IRULE2101 (Heavy `regexp` in a high-frequency event — consider `string match` or data-group), IRULE5001 (Ungated `log` in a high-frequency event), IRULE5002 (`drop`/`reject`/`discard` without `event disable all` or `return`), IRULE5004 (`DNS::return` without `return`), IRULE5005 (Direct proc invocation without `call` — use `call proc_name`), IRULE5006 (Top-level-only command used inside a nested body), IRULE5007 (Event-context command used at top level outside a `when` block), BIGIP6001 (iRule references a data group not found in the configuration), BIGIP6002 (iRule references a pool not found in the configuration), BIGIP6003 (iRule contains a dynamic pool reference that cannot be validated statically), BIGIP6004 (Virtual server references an object not found in the configuration), BIGIP6005 (Pool has no members), BIGIP6006 (Data group has duplicate keys), BIGIP6007 (iRule references a virtual server not found in the configuration), BIGIP6008 (iApp presentation and implementation variables do not agree), BIGIP6009 (iRule contains a dynamic virtual-server reference that cannot be validated statically), BIGIP6010 (Virtual server has a missing or invalid source-address-translation pool reference), BIGIP6011 (Virtual server references a persistence profile not found in the configuration), BIGIP6012 (Attached iRules handle the same event at the same effective priority)
