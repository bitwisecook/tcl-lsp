...
static auto has_code(const AnalysisResult& r, std::string_view code) -> bool {
    for (const auto& d : r.diagnostics()) {
        if (to_string(d.code) == code) return true;
    }
    return false;
}
...
TEST_CASE("upstream proc: simple single parameter",
          "[upstream][proc]") {
...
TEST_CASE("upstream proc: multi-parameter",
          "[upstream][proc]") {
...
TEST_CASE("upstream proc: default parameter values",
          "[upstream][proc]") {
    auto r = analyse(
        R"(proc greet {name {greeting Hello}} { puts "$greeting $name" })");
    auto* p = r.find_proc("greet");
    REQUIRE(p != nullptr);
...
TEST_CASE("upstream proc: variadic args parameter",
          "[upstream][proc]") {
...
TEST_CASE("upstream proc: no parameters",
          "[upstream][proc]") {
...
TEST_CASE("upstream proc: creates child scope",
          "[upstream][proc]") {
...
TEST_CASE("upstream proc: namespace-qualified name",
          "[upstream][proc]") {
...
TEST_CASE("upstream proc: nested namespace",
          "[upstream][proc]") {
    auto r = analyse(
        "namespace eval ::a { namespace eval b { proc c {} {} } }");
...
TEST_CASE("upstream proc: too few args produces E002",
          "[upstream][proc]") {
    auto r = analyse("proc greet {name greeting} {}\ngreet Alice");
    CHECK(has_code(r, "E002"));
}

TEST_CASE("upstream proc: too many args produces E003",
          "[upstream][proc]") {
    auto r = analyse("proc greet {name} {}\ngreet Alice Bob");
    CHECK(has_code(r, "E003"));
}

TEST_CASE("upstream proc: correct arity no diagnostic",
          "[upstream][proc]") {
    auto r = analyse("proc greet {name} {}\ngreet Alice");
...
TEST_CASE("upstream proc: default param allows fewer args",
          "[upstream][proc]") {
    auto r = analyse(
...
TEST_CASE("upstream proc: args param allows extra args",
          "[upstream][proc]") {
    auto r = analyse(
...
TEST_CASE("upstream proc: doc from preceding comment",
          "[upstream][proc]") {
