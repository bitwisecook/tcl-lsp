...
constexpr std::array MULTI_OPS = {
    MultiOp{"**", false}, MultiOp{"<<", false}, MultiOp{">>", false},
    MultiOp{"<=", false}, MultiOp{">=", false}, MultiOp{"==", false},
    MultiOp{"!=", false}, MultiOp{"&&", false}, MultiOp{"||", false},
    MultiOp{"ne", true},  MultiOp{"eq", true},  MultiOp{"in", true},
    MultiOp{"ni", true},  MultiOp{"lt", true},  MultiOp{"le", true},
    MultiOp{"gt", true},  MultiOp{"ge", true},
};
...
auto is_bool_literal(std::string_view text) -> bool {
    return text == "true" || text == "false" || text == "yes" ||
           text == "no" || text == "on" || text == "off";
}
...
class ExprLexer {
  public:
    ...
    auto tokenise() -> std::vector<ExprToken> {
        ...
        while (pos_ < len()) {
            ...
            if (ch == ')') {
                tokens.push_back({ExprTokenType::PAREN_CLOSE, src_.substr(pos_, 1), ipos(), ipos()});
                pos_ += 1;
                continue;
            }
            if (ch == ',') {
            ...
            if (try_multi_op(tokens)) continue;
            if (is_single_op(ch)) {
  ...
  private:
    ...
    void read_whitespace(std::vector<ExprToken>& out) {
        auto start = pos_;
        while (pos_ < len() && is_ws(src_[pos_])) pos_ += 1;
        out.push_back({ExprTokenType::WHITESPACE, src_.substr(start, pos_ - start),
                        static_cast<std::int32_t>(start), ipos() - 1});
    }
    ...
    void read_number(std::vector<ExprToken>& out) {
        ...
        if (src_[pos_] == '0' && pos_ + 1 < len() &&
            (src_[pos_ + 1] == 'x' || src_[pos_ + 1] == 'X' ||
             ...
             src_[pos_ + 1] == 'b' || src_[pos_ + 1] == 'B')) {
            pos_ += 2;
            while (pos_ < len() && is_id_char(src_[pos_])) pos_ += 1;
            out.push_back({ExprTokenType::NUMBER, src_.substr(start, pos_ - start),
                            static_cast<std::int32_t>(start), ipos() - 1});
            return;
        }
        // Integer part
    ...
    void read_variable(std::vector<ExprToken>& out) {
        ...
        if (pos_ < len() && src_[pos_] == '{') {
            pos_ += 1;
            while (pos_ < len() && src_[pos_] != '}') pos_ += 1;
            if (pos_ < len()) pos_ += 1;
        } else {
            while (pos_ < len() &&
            ...
            if (pos_ < len() && src_[pos_] == '(') {
                ...
                while (pos_ < len() && level > 0) {
                    if (src_[pos_] == '(') level += 1;
                    else if (src_[pos_] == ')') level -= 1;
                    pos_ += 1;
                }
            }
        }
        out.push_back({ExprTokenType::VARIABLE, src_.substr(start, pos_ - start),
                        static_cast<std::int32_t>(start), ipos() - 1});
    }
    ...
    void read_command(std::vector<ExprToken>& out) {
        ...
        while (pos_ < len() && level > 0) {
            ...
            else if (src_[pos_] == '\\') pos_ += 1; // skip escaped char
            pos_ += 1;
        }
        out.push_back({ExprTokenType::COMMAND, src_.substr(start, pos_ - start),
                        static_cast<std::int32_t>(start), ipos() - 1});
    }
    ...
    void read_string(std::vector<ExprToken>& out) {
        ...
        if (pos_ < len()) pos_ += 1; // skip closing quote
        out.push_back({ExprTokenType::STRING, src_.substr(start, pos_ - start),
                        static_cast<std::int32_t>(start), ipos() - 1});
    }
    ...
    void read_braced(std::vector<ExprToken>& out) {
        ...
        if (level != 0) {
            pos_ = saved;
            out.push_back({ExprTokenType::STRING, src_.substr(start, 1),
                            static_cast<std::int32_t>(start), static_cast<std::int32_t>(start)});
            return;
        }
        out.push_back({ExprTokenType::STRING, src_.substr(start, pos_ - start),
                        static_cast<std::int32_t>(start), ipos() - 1});
    ...
    auto try_multi_op(std::vector<ExprToken>& out) -> bool {
        for (const auto& [text, word_boundary] : MULTI_OPS) {
            if (pos_ + text.size() > len()) continue;
            if (src_.substr(pos_, text.size()) != text) continue;
            if (word_boundary) {
                ...
                if (after < len() &&
                    (std::isalpha(static_cast<unsigned char>(src_[after])) != 0 ||
                     src_[after] == '_'))
                    continue;
            }
            auto s = ipos();
            out.push_back({ExprTokenType::OPERATOR, src_.substr(pos_, text.size()),
                            s, static_cast<std::int32_t>(pos_ + text.size() - 1)});
            pos_ += text.size();
            return true;
        }
        return false;
